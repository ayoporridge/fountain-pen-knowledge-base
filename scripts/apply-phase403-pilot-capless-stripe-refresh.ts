...
import {
  ...
  phase403PilotCaplessStripeRefreshPacks,
} from "./data/phase403-pilot-capless-stripe-refresh";
import {
...
export type ApplyPhase403Options = ApplyPhase22Options;
export type ApplyPhase403Result = ApplyPhase22Result;
...
function rejectRemote(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (env[key]?.trim()) {
      throw new Error(`Phase 403 refuses inherited remote database selection: ${key}.`);
    }
  }
}

async function assertAuthority(client: Client, options: ApplyPhase403Options): Promise<void> {
  rejectRemote(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 403 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 403 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 403 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 403 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 403 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  const result = await client.execute({
    ...
    args: [PHASE403_PILOT_ID, PHASE403_STRIPE_ID],
  });
  const brand = result.rows.find((row) => String(row.id) === PHASE403_PILOT_ID);
  const model = result.rows.find((row) => String(row.id) === PHASE403_STRIPE_ID);
  if (
    !brand ||
    ...
    throw new Error("Phase 403 Pilot Capless Stripe identity mismatch.");
  }
}
...
async function ensureTopology(client: Client): Promise<void> {
  ...
  try {
    await tx.execute({
      ...
      args: [PHASE403_STRIPE_ID, PHASE403_PILOT_ID],
    });
    const maker = await tx.execute({
      sql: "SELECT id,target_id FROM entity_links WHERE source_id=? AND link_type='made_by'",
      args: [PHASE403_STRIPE_ID],
    });
    if (maker.rows.length === 0) {
      await tx.execute({
        sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
        args: [
          "phase403-made-by-pilot-capless-stripe",
          PHASE403_STRIPE_ID,
          PHASE403_PILOT_ID,
          "Phase 403 verified Pilot Capless Stripe maker relation",
        ],
      });
    } else if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE403_PILOT_ID
    ) {
      throw new Error("Phase 403 Pilot Capless Stripe maker relation is ambiguous.");
    }
    ...
    await tx.execute({
      ...
      args: [
        "phase403-reverse-pilot-capless-stripe",
        PHASE403_PILOT_ID,
        PHASE403_STRIPE_ID,
        "Phase 403 Pilot brand navigation to Capless Stripe",
      ],
    });
    ...
    if (
      ...
      String(makerAfter.rows[0]?.target_id) !== PHASE403_PILOT_ID ||
      reverseAfter.rows.length !== 1
    ) {
      throw new Error("Phase 403 Pilot Capless Stripe topology is ambiguous.");
    }
    await tx.commit();
  } catch (error) {
...
async function exactTerminalState(
  ...
  const reverse = await client.execute({
    sql: "SELECT source_id FROM entity_links WHERE source_id=? AND target_id=? AND link_type='reverse'",
    args: [PHASE403_PILOT_ID, pack.entityId],
  });
...
async function republishPilotBrand(client: Client, reviewer: string): Promise<void> {
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE403_PILOT_ID,
      reviewKind,
      reviewer,
      status: "approved",
      notes: "Phase 403 re-approves the unchanged Pilot brand after normalizing the Capless Stripe navigation link.",
    });
  }
  await publishEntity(client, { entityId: PHASE403_PILOT_ID, reviewer });
}

export async function applyPhase403PilotCaplessStripeRefresh(
  client: Client,
  options: ApplyPhase403Options,
): Promise<ApplyPhase403Result> {
  await assertAuthority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const pack = loadCuratedEntityPack(workspaceRoot, phase403PilotCaplessStripeRefreshPacks[0]!);
  validatePack(workspaceRoot, pack);
  if (Array.from(pack.bodyMd).length < 8_000) {
    throw new Error("Phase 403 reviewed body must contain at least 8,000 Unicode characters.");
  }
  if (
    pack.sources.length < 10 ||
    pack.variants?.length !== 3 ||
    pack.variants.filter((variant) => variant.variantKind === "edition_group").length !== 1 ||
    pack.variants.filter((variant) => variant.variantKind === "market_sku").length !== 2
  ) {
    throw new Error("Phase 403 pack lacks the expected source or SKU coverage.");
  }
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase403-pilot-capless-stripe-refresh.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase403PilotCaplessStripeRefresh(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase403-pilot-capless-stripe-refresh",
