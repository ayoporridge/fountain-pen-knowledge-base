...
export const PHASE565_TARGET_ID = "phase519-wancher-kyoto-urushi-asagao";
export const PHASE565_REVIEWER = "phase565-wancher-kyoto-urushi-asagao";

export type ApplyPhase565Options = {
...
export type ApplyPhase565Result = {
  entityId: string;
...
function rejectRemote(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (env[key]?.trim()) throw new Error(`Phase 565 refuses inherited remote database selection: ${key}.`);
  }
}

async function assertAuthority(client: Client, options: ApplyPhase565Options): Promise<void> {
  ...
  ) throw new Error("Phase 565 requires an owned, non-symlink catalog copy.");
  ...
  if (database === protectedPath || (owned.dev === protectedStat.dev && owned.ino === protectedStat.ino)) {
    throw new Error("Phase 565 refuses the protected catalog and hard-link aliases.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 565 client is not bound to the authorized owned copy.");
  }
  const migration = await client.execute({
    sql: "SELECT 1 AS ok FROM migrations WHERE name=? AND checksum IS NOT NULL",
    args: ["032_taxonomy_identity.sql"],
  });
  if (migration.rows.length !== 1) throw new Error("Phase 565 owned copy must be migrated through 032.");
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  ...
  ) throw new Error("Phase 565 Wancher brand identity is not the approved canonical row.");
  ...
  ) throw new Error(`Phase 565 refuses an identity collision: ${pack.entityId}.`);
}
...
async function prepareTopology(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  ...
  try {
    ...
    await transaction.execute({
      ...
      args: [
        `phase565-made-by-${pack.entityId}`,
        pack.entityId,
        PHASE519_WANCHER_BRAND_ID,
        `Phase 565 verified Wancher maker relation for ${pack.canonicalName}`,
      ],
    });
    await transaction.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
      args: [
        `phase565-reverse-${pack.entityId}`,
        PHASE519_WANCHER_BRAND_ID,
        pack.entityId,
        `Phase 565 Wancher brand navigation to ${pack.canonicalName}`,
      ],
    });
    ...
    if (maker.rows.length !== 1 || String(maker.rows[0]?.target_id) !== PHASE519_WANCHER_BRAND_ID) {
      throw new Error(`Phase 565 maker topology is ambiguous: ${pack.entityId}.`);
    }
    if (reverse.rows.length !== 1) throw new Error(`Phase 565 reverse navigation is ambiguous: ${pack.entityId}.`);
    ...
    if (publication.rows.length !== 1 || !["draft", "published"].includes(String(publication.rows[0]?.status))) {
      throw new Error(`Phase 565 expects an existing publication row: ${pack.entityId}.`);
    }
...
async function applyPack(
  client: Client,
  options: ApplyPhase565Options,
  pack: LoadedCuratedEntityPack,
): Promise<ApplyPhase565Result> {
  ...
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      ...
      status: "approved",
      notes: `${pack.sourceMarker}; ${reviewKind} review of the deepened Asagao copy.`,
    });
  }
...
export async function applyPhase565WancherKyotoUrushiAsagao(
  client: Client,
  options: ApplyPhase565Options,
): Promise<ApplyPhase565Result> {
  await assertAuthority(client, options);
  const rawPack = phase519WancherDreamPenPacks.find((pack) => pack.entityId === PHASE565_TARGET_ID);
  if (!rawPack) throw new Error("Phase 565 Asagao pack is missing.");
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error("Usage: tsx scripts/apply-phase565-wancher-kyoto-urushi-asagao.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]");
  }
  const resolvedDatabase = path.resolve(database);
  const resolvedProtected = path.resolve(protectedCatalog);
  const client = createClient({ url: `file:${resolvedDatabase}` });
  try {
    const result = await applyPhase565WancherKyotoUrushiAsagao(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? PHASE565_REVIEWER,
