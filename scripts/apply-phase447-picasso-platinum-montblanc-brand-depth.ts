...
} from "./data/phase447-picasso-platinum-montblanc-brand-depth";

export type ApplyPhase447Options = ApplyPhase22Options;
export type ApplyPhase447Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase447Options): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 447 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function authority(client: Client, options: ApplyPhase447Options): Promise<void> {
  ...
  if (
    ...
    throw new Error("Phase 447 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 447 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 447 client is not bound to the authorized owned copy.");
  }
  const migration = await client.execute({
    sql: "SELECT 1 FROM migrations WHERE name=? AND checksum IS NOT NULL",
    args: ["032_taxonomy_identity.sql"],
  });
  if (migration.rows.length !== 1) throw new Error("Phase 447 owned copy must be migrated through 032.");
}

async function identity(client: Client, packs: LoadedCuratedEntityPack[]): Promise<void> {
  if (packs.length !== 3 || new Set(packs.map((pack) => pack.entityId)).size !== 3) {
    throw new Error("Phase 447 must load three unique brand packs.");
  }
  for (const [label, id] of Object.entries(PHASE447_BRAND_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack || pack.expectedType !== "brand" || pack.expectedSlug !== label) {
      throw new Error(`Phase 447 ${label} pack identity mismatch.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 447 ${label} brand identity mismatch: ${JSON.stringify(existing)}`);
    }
  }
}
    ...
    for (const [label, brandId] of Object.entries(PHASE447_BRAND_IDS)) {
      ...
      for (const row of models.rows) {
        ...
        await tx.execute({
          ...
          args: [
            `phase447-reverse-${label}-${modelId}`,
            brandId,
            modelId,
            `Phase 447 ${label} brand navigation for published pen ${modelId}`,
          ],
        });
        ...
        if (Number(reverse.rows[0]?.n) !== 1) throw new Error(`Phase 447 ${label} reverse navigation is ambiguous for ${modelId}.`);
      }
    }
    await tx.commit();
  } catch (error) {
...
async function applyBrandPacks(
  client: Client,
  options: ApplyPhase447Options,
  packs: LoadedCuratedEntityPack[],
): Promise<ApplyPhase447Result> {
  ...
  const entities: ApplyPhase447Result["entities"] = [];
...
export async function applyPhase447BrandDepth(client: Client, options: ApplyPhase447Options): Promise<ApplyPhase447Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 447 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase447PicassoPlatinumMontblancBrandPacks.map((pack) => loadCuratedEntityPack(workspaceRoot, pack));
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) throw new Error("Usage: tsx scripts/apply-phase447-picasso-platinum-montblanc-brand-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]");
  const resolvedDatabase = path.resolve(database);
  const resolvedProtected = path.resolve(protectedCatalog);
  const client = createClient({ url: `file:${resolvedDatabase}` });
  try {
    const result = await applyPhase447BrandDepth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase447-brand-depth",
