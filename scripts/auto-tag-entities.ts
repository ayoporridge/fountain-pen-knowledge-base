...
// Group by entity
const entityMap = new Map<string, { id: string; slug: string; name: string; attrs: Record<string, string> }>();
for (const row of entities) {
  if (!entityMap.has(row.id)) {
    entityMap.set(row.id, { id: row.id, slug: row.slug, name: row.name, attrs: {} });
  }
  if (row.key && row.value) {
    entityMap.get(row.id)!.attrs[row.key] = row.value;
  }
}
...
for (const [entityId, entity] of entityMap) {
  ...
  if (attrs.body_material) {
    ...
    if (matchKeyword(v, ["pmma", "亚克力", "透明树脂"])) tagSlug = "mat-pmma";
    else if (matchKeyword(v, ["赛璐璐"])) tagSlug = "mat-celluloid";
    else if (matchKeyword(v, ["漆艺", "莳绘", "漆器", "生漆"])) tagSlug = "mat-lacquer";
    ...
    else if (matchKeyword(v, ["金属", "不锈钢", "钢"])) tagSlug = "mat-steel";
    ...
    if (tagSlug) {
      ...
      if (!unmatched.body_material) unmatched.body_material = [];
      unmatched.body_material.push(v);
    }
  }
  ...
  if (attrs.price_range && attrs.price_range !== "—") {
    ...
    if (tagSlug) {
      ...
      if (!unmatched.price) unmatched.price = [];
      unmatched.price.push(v);
    }
  }
  ...
  if (attrs.writing_style) {
    ...
    else if (matchKeyword(v, ["钢尖", "金尖", "活塞"])) tagSlug = "nib-standard";
    ...
    if (tagSlug) {
      ...
      if (!unmatched.writing_style) unmatched.writing_style = [];
      unmatched.writing_style.push(v);
    }
  }
  ...
  if (attrs.nib_size) {
    ...
    if (tagSlug) {
      ...
      if (!unmatched.nib_size) unmatched.nib_size = [];
      unmatched.nib_size.push(v);
    }
  }
  
  // === origin_country → origin (already done, but let's verify) ===
...
console.log("\nBy dimension:");
for (const [dim, count] of Object.entries(stats).sort((a, b) => b[1] - a[1])) {
  console.log(`  ${dim}: ${count} entities`);
}

// Show unmatched values
if (Object.keys(unmatched).length > 0) {
  console.log("\n=== Unmatched values (need manual mapping) ===");
  for (const [key, values] of Object.entries(unmatched)) {
    const uniqueValues = [...new Set(values)];
    console.log(`\n${key} (${uniqueValues.length} unique):`);
    for (const v of uniqueValues.slice(0, 10)) {
      console.log(`  - "${v}"`);
    }
