...
import {
  ...
  PHASE129_TARGET_IDS,
  PHASE129_TARGET_SLUGS,
  PHASE129_TOKIWA_ID,
  PHASE129_TOKIWA_SLUG,
  PHASE129_WANCHER_ID,
} from "./data/phase129-wancher-dream-pen-tokiwa-bokashi";
import type { LoadedCuratedEntityPack } from "./lib/curated-content-pack";

export type ApplyPhase129Options = ApplyPhase22Options;
export type ApplyPhase129Result = ApplyPhase22Result;
...
const BASELINE = [
  [PHASE129_DREAM_ARTICLE_ID, "article", "wancher-dream-pen", ""],
  [
    "phase107-wancher-true-ebonite-matte-black",
    "pen",
    "wancher-dream-pen-true-ebonite-matte-black",
    "curated-content:phase107-",
  ],
  [
    "phase112-wancher-dream-pen-titanium-black",
    "pen",
    "wancher-dream-pen-titanium-black",
    "curated-content:phase112-",
  ],
  [
    "phase113-wancher-dream-pen-true-urushi-aka-tamenuri",
    "pen",
    "wancher-dream-pen-true-urushi-aka-tamenuri",
    "curated-content:phase113-",
  ],
  [
    "phase119-wancher-puchico",
    "pen",
    "wancher-puchico",
    "curated-content:phase119-",
  ],
  [
    "phase120-wancher-shizuku-glass-nib",
    "pen",
    "wancher-shizuku-glass-nib",
    "curated-content:phase120-",
  ],
  [
    "phase128-wancher-dream-pen-true-urushi-black",
    "pen",
    "wancher-dream-pen-true-urushi-black",
    "curated-content:phase128-",
  ],
  [
    "phase128-wancher-dream-pen-byakudan-nuri",
    "pen",
...
function rejectRemote(env: NodeJS.ProcessEnv) {
  for (const key of [
    ...
    if (env[key]?.trim())
      throw new Error(`Phase 129 refuses inherited remote selection: ${key}.`);
}
...
async function authority(client: Client, options: ApplyPhase129Options) {
  rejectRemote(options.env ?? process.env);
  if (!options.reviewer.trim())
    throw new Error("Phase 129 reviewer must not be empty.");
  if (!ALLOWED_REPOS.has(options.workspaceRoot))
    throw new Error("Phase 129 requires the verified CodeBuddy/Documents repo pair.");
  ...
  if (workspaceRoot !== CANONICAL_REPO || gitRoot !== CANONICAL_REPO)
    throw new Error("Phase 129 verified repo aliases must resolve identically.");
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  if (fs.lstatSync(options.databasePath).isSymbolicLink())
    throw new Error("Phase 129 owned database must not be a symlink.");
  const ownedRoot = fs.realpathSync.native(options.ownedRoot);
  const databasePath = fs.realpathSync.native(options.databasePath);
  const protectedPath = fs.realpathSync.native(options.protectedCatalogPath);
  if (!fs.statSync(ownedRoot).isDirectory() || !inside(databasePath, ownedRoot))
    throw new Error("Phase 129 requires a database inside caller-owned root.");
  ...
  if (
    ...
    throw new Error("Phase 129 refuses protected catalog or hard-link aliases.");
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath)
    throw new Error("Phase 129 client/path mismatch.");
  const migration = await client.execute({
    sql: "SELECT 1 FROM migrations WHERE name=? AND checksum IS NOT NULL",
    args: ["032_taxonomy_identity.sql"],
  });
  if (migration.rows.length !== 1)
    throw new Error("Phase 129 owned copy must be migrated through 032.");
...
async function baseline(client: Client) {
  ...
  if (
    brand?.type !== "brand" ||
    brand?.slug !== "wancher" ||
    !String(brand?.source).startsWith("curated-content:phase107-wancher-brand-v1:") ||
    brand?.status !== "published" ||
    Number(brand?.is_public) !== 1
  )
    throw new Error("Phase 129 requires the exact published Wancher baseline.");
  for (const [id, type, slug, marker] of BASELINE) {
    ...
    if (
      ...
      (id !== PHASE129_DREAM_ARTICLE_ID && row?.status !== "published") ||
      Number(row?.is_public) !== 1
    )
      throw new Error(`Phase 129 prerequisite baseline is invalid: ${id}.`);
  }
}
...
async function state(
  ...
  if (found.length !== packs.length)
    throw new Error("Phase 129 target state is partial or colliding.");
  ...
  for (const pack of packs) {
    ...
    if (
      ...
      throw new Error("Phase 129 terminal identity is partial or alternate.");
  }
  return "terminal";
}
...
async function collisionPreflight(
  ...
  if (aliasOwners.length > 0 || markerOwners.length > 0)
    throw new Error("Phase 129 target alias/source collision; repair forbidden.");
}
...
async function terminal(
  ...
  for (let index = 0; index < packs.length; index += 1) {
    ...
    if (
      ...
      throw new Error("Phase 129 terminal payload/topology/publication is invalid.");
  }
  ...
  if (brand?.status !== "published" || brand?.approved_content_hash !== brandHash)
    throw new Error("Phase 129 terminal Wancher publication is stale.");
  return hashes;
}

export async function applyPhase129WancherTokiwaBokashiContent(
  client: Client,
  options: ApplyPhase129Options,
): Promise<ApplyPhase129Result> {
  const workspaceRoot = await authority(client, options);
  const packs = loadPhase129WancherPacks(workspaceRoot);
  ...
  const brandBefore = await entityDigest(client, PHASE129_WANCHER_ID, false);
  const reverseBefore = (
    await rows(
      client,
  ...
  try {
    for (let index = 0; index < packs.length; index += 1) {
      ...
      await tx.execute({
        ...
        args: [
          PHASE129_MADE_BY_IDS[index],
          pack.entityId,
          PHASE129_WANCHER_ID,
          `Phase 129 exact ${pack.canonicalName} maker relation`,
        ],
      });
    }
  ...
  if ((await entityDigest(client, PHASE129_WANCHER_ID, false)) !== brandBefore)
    throw new Error("Phase 129 changed Wancher non-topology payload.");
  const reverseAfter = (
    await rows(
      client,
      "SELECT target_id FROM entity_links WHERE source_id=? AND link_type='reverse' ORDER BY target_id",
  ...
  if (
    ...
    JSON.stringify(reverseBefore.concat([...PHASE129_TARGET_IDS]).sort())
  )
    throw new Error("Phase 129 reverse delta is not exact add-two.");
  for (const id of PROTECTED_IDS)
    if ((await entityDigest(client, id)) !== protectedBefore.get(id))
      throw new Error(`Phase 129 changed protected Wancher entity ${id}.`);
  ...
  await reviewPublish(
    ...
    PHASE129_WANCHER_ID,
    reviewer,
    "Phase 129 post-topology current hash; no Wancher pack replay.",
  );
  const hashes: string[] = [];
  for (const pack of packs)
    hashes.push(
  ...
  if ((await entityDigest(client, PHASE129_WANCHER_ID, false)) !== brandBefore)
    throw new Error("Phase 129 replayed Wancher non-topology payload.");
  for (const id of PROTECTED_IDS)
    if ((await entityDigest(client, id)) !== protectedBefore.get(id))
      throw new Error(
        `Phase 129 changed protected Wancher entity ${id} after publication.`,
      );
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return {
    entities: packs.map((pack, index) => ({
...
async function main() {
  ...
  const reviewer = value("--reviewer") ?? "phase129-wancher-tokiwa-bokashi";
  if (!databasePath || !ownedRoot || !protectedCatalogPath)
    throw new Error(
      "Usage: tsx scripts/apply-phase129-wancher-dream-pen-tokiwa-bokashi-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db>",
    );
  const client = createClient({ url: `file:${path.resolve(databasePath)}` });
  try {
    process.stdout.write(
      `${JSON.stringify(
        await applyPhase129WancherTokiwaBokashiContent(client, {
