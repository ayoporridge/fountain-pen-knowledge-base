...
} from "./data/phase592-pineider-alba-classic-mini";
import {
...
export type ApplyPhase592Options = ApplyPhase22Options;
export type ApplyPhase592Result = ApplyPhase22Result;
...
const ACCEPTED_INITIAL_BRAND_SOURCE_PREFIX =
  "curated-content:phase591-pineider-brand-depth-refresh-v1:";
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of [
    ...
    if (env[key]?.trim()) {
      throw new Error(`Phase 592 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function assertOwnedCatalog(
  client: Client,
  options: ApplyPhase592Options,
): Promise<string> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) {
    throw new Error("Phase 592 reviewer must not be empty.");
  }
  ...
  if (
    ...
    throw new Error(
      "Phase 592 requires a non-symlink catalog file inside the caller-owned root.",
    );
  }
  ...
  if (owned.nlink !== BigInt(1)) {
    throw new Error("Phase 592 refuses catalog files with more than one hard link.");
  }
  if (
    databasePath === protectedPath ||
    (owned.dev === protectedCatalog.dev && owned.ino === protectedCatalog.ino)
  ) {
    throw new Error("Phase 592 refuses the protected catalog or a hard-link alias.");
  }
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) {
    throw new Error("Phase 592 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 592 owned copy must be migrated through 032.");
  }
  return workspaceRoot;
}

function loadAndValidatePackSet(workspaceRoot: string): LoadedCuratedEntityPack[] {
  const packs = phase592PineiderPacks.map((pack) =>
  ...
  if (
    ...
    throw new Error(
      "Phase 592 requires the exact Pineider brand plus Alba Classic and Alba Mini packs.",
    );
  }
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 592 pack identity mismatch: ${target.id}.`);
    }
  }
  ...
  for (const pack of packs.slice(1)) {
    for (const value of [
      ...
      if (existing && existing !== pack.entityId) {
        throw new Error(`Phase 592 pack-set name or alias collision: ${value}.`);
      }
...
async function assertIdentityPreflight(
  ...
  if (
    !brandPack ||
    brand.length !== 1 ||
    brand[0]?.id !== PHASE592_PINEIDER_BRAND_ID ||
    brand[0]?.type !== "brand" ||
    ...
    throw new Error(
      `Phase 592 Pineider brand identity mismatch: ${JSON.stringify(brand)}.`,
    );
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 592 entity, slug, or name collision: ${target.slug}.`);
    }
    ...
    if (canonicalAliasCollisions.length > 0) {
      throw new Error(
        `Phase 592 canonical name collides with an alias: ${target.name}.`,
      );
    }

    const pack = packs.find((candidate) => candidate.entityId === target.id);
    if (!pack) throw new Error(`Phase 592 missing loaded pack: ${target.id}.`);
    for (const alias of pack.aliases) {
      ...
      if (collisions.length > 0) {
        throw new Error(
          `Phase 592 alias collision for ${alias.alias}: ${JSON.stringify(collisions)}.`,
        );
      }
    }
  }
  ...
  try {
    for (const target of TARGETS) {
      ...
      if (existing.length === 0) {
        ...
        throw new Error(`Phase 592 refuses a conflicting identity: ${target.slug}.`);
      }
      ...
      if (maker.length === 0) {
        await transaction.execute({
          ...
          args: [
            stableId("phase592-made-by", target.id),
            target.id,
            PHASE592_PINEIDER_BRAND_ID,
            "Phase 592 verified exact Pineider maker relation",
          ],
        });
      } else if (
        maker.length !== 1 ||
        maker[0]?.target_id !== PHASE592_PINEIDER_BRAND_ID
      ) {
        throw new Error(`Phase 592 ${target.slug} maker topology is ambiguous.`);
      }
      ...
      if (reverse.length === 0) {
        await transaction.execute({
          ...
          args: [
            stableId("phase592-reverse", target.id),
            PHASE592_PINEIDER_BRAND_ID,
            target.id,
            "Phase 592 Pineider brand public model navigation",
          ],
        });
      } else if (
        reverse.length !== 1 ||
        reverse[0]?.source_id !== PHASE592_PINEIDER_BRAND_ID
      ) {
        throw new Error(`Phase 592 ${target.slug} reverse topology is ambiguous.`);
      }
      ...
      if (
        ...
        throw new Error(`Phase 592 publication row is missing: ${target.id}.`);
      }
    }
...
async function assertTerminalState(
  ...
  for (const pack of packs) {
    ...
    if (
      ...
      throw new Error(`Phase 592 terminal publication mismatch: ${pack.entityId}.`);
    }
    ...
    if (
      ...
      throw new Error(
        `Phase 592 current-hash approved review set is incomplete: ${pack.entityId}.`,
      );
    }
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 592 terminal topology mismatch: ${target.id}.`);
    }
  }
}

export async function applyPhase592PineiderAlbaClassicMiniContent(
  client: Client,
  options: ApplyPhase592Options,
): Promise<ApplyPhase592Result> {
  ...
  for (const [id, before] of protectedBefore) {
    if ((await entityDigest(client, id)) !== before) {
      throw new Error(`Phase 592 changed protected existing Pineider model: ${id}.`);
    }
  }
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return result;
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase592-pineider-alba-classic-mini-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase592PineiderAlbaClassicMiniContent(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase592-pineider-alba-classic-mini",
      databasePath,
