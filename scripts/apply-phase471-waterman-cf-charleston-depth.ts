...
import {
  PHASE471_IDS,
  phase471WatermanCfCharlestonDepthPacks,
} from "./data/phase471-waterman-cf-charleston-depth";

export type ApplyPhase471Options = ApplyPhase22Options;
export type ApplyPhase471Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase471Options): void {
  for (const key of [
    "TURSO_DATABASE_URL",
    "TURSO_AUTH_TOKEN",
    "FPKG_DATABASE_URL",
    ...
    if (options.env?.[key]?.trim()) {
      throw new Error(
        `Phase 471 refuses inherited remote database selection: ${key}.`,
      );
    }
  }
}
...
async function authority(
  ...
  if (
    ...
    throw new Error("Phase 471 requires an owned, non-symlink catalog copy.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 471 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 471 owned copy must be migrated through 032.");
  }
}
...
async function identity(
  ...
  const ids = Object.values(PHASE471_IDS);
  if (
    packs.length !== ids.length ||
    new Set(packs.map((pack) => pack.entityId)).size !== ids.length
  ) {
    throw new Error("Phase 471 must load Waterman C/F and Charleston exactly once.");
  }
  for (const [label, id] of Object.entries(PHASE471_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack?.expectedSlug) {
      throw new Error(`Phase 471 ${label} pack identity mismatch.`);
    }
    ...
    if (
      ...
      throw new Error(
        `Phase 471 ${label} identity mismatch: ${JSON.stringify(existing)}`,
      );
    }
  }
}
...
async function repairModelNavigation(
  ...
  try {
    for (const pack of packs) {
      ...
      if (!brandId) {
        throw new Error(`Phase 471 ${pack.expectedSlug} has no Waterman maker identity.`);
      }
      ...
      if (Number(maker.rows[0]?.n) !== 1) {
        throw new Error(
          `Phase 471 ${pack.expectedSlug} must have exactly one made_by relation.`,
        );
      }
      await tx.execute({
        sql: "INSERT OR IGNORE INTO entity_links (id,source_id,target_id,link_type,reason) VALUES (?,?,?,'reverse',?)",
        args: [
          `phase471-reverse-${pack.entityId}`,
          brandId,
          pack.entityId,
          `Phase 471 Waterman navigation for ${pack.expectedSlug}`,
        ],
      });
      ...
      if (Number(reverse.rows[0]?.n) !== 1) {
        throw new Error(
          `Phase 471 ${pack.expectedSlug} reverse navigation is ambiguous.`,
        );
      }
    }
    await tx.commit();
...
async function applyPacks(
  ...
  const entities: ApplyPhase471Result["entities"] = [];
  for (const pack of packs) {
    for (const reviewKind of ["fact", "language", "media"] as const) {
      await recordEntityContentReview(client, {
        ...
        status: "approved",
        notes: `${pack.sourceMarker}; ${reviewKind} review of Waterman C/F and Charleston copy.`,
      });
    }
...
export async function applyPhase471WatermanCfCharlestonDepth(
  client: Client,
  options: ApplyPhase471Options,
): Promise<ApplyPhase471Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 471 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase471WatermanCfCharlestonDepthPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase471-waterman-cf-charleston-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase471WatermanCfCharlestonDepth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase471-waterman-cf-charleston",
