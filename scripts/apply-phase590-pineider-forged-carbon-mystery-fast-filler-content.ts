...
} from "./data/phase590-pineider-forged-carbon-mystery-fast-filler";
import {
...
export type ApplyPhase590Options = ApplyPhase22Options;
export type ApplyPhase590Result = ApplyPhase22Result;
...
const ACCEPTED_INITIAL_BRAND_SOURCE_PREFIX =
  "curated-content:phase589-pineider-brand-depth-refresh-v1:";
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of [
    ...
    if (env[key]?.trim()) {
      throw new Error(`Phase 590 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function assertOwnedCatalog(
  client: Client,
  options: ApplyPhase590Options,
): Promise<string> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) {
    throw new Error("Phase 590 reviewer must not be empty.");
  }
  ...
  if (
    ...
    throw new Error(
      "Phase 590 requires a non-symlink catalog file inside the caller-owned root.",
    );
  }
  ...
  if (owned.nlink !== BigInt(1)) {
    throw new Error("Phase 590 refuses catalog files with more than one hard link.");
  }
  if (
    databasePath === protectedPath ||
    (owned.dev === protectedCatalog.dev && owned.ino === protectedCatalog.ino)
  ) {
    throw new Error("Phase 590 refuses the protected catalog or a hard-link alias.");
  }
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) {
    throw new Error("Phase 590 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 590 owned copy must be migrated through 032.");
  }
  return workspaceRoot;
}

function loadAndValidatePackSet(workspaceRoot: string): LoadedCuratedEntityPack[] {
  const packs = phase590PineiderPacks.map((pack) =>
  ...
  if (
    ...
    throw new Error(
      "Phase 590 requires the exact Pineider brand plus Forged Carbon and Mystery Fast Filler packs.",
    );
  }
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 590 pack identity mismatch: ${target.id}.`);
    }
  }
  return packs;
}
...
async function assertIdentityPreflight(
  ...
  if (
    !brandPack ||
    brand.length !== 1 ||
    brand[0]?.id !== PHASE590_PINEIDER_BRAND_ID ||
    brand[0]?.type !== "brand" ||
    ...
    throw new Error(
      `Phase 590 Pineider brand identity mismatch: ${JSON.stringify(brand)}.`,
    );
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 590 entity, slug, or name collision: ${target.slug}.`);
    }

    const pack = packs.find((candidate) => candidate.entityId === target.id);
    if (!pack) throw new Error(`Phase 590 missing loaded pack: ${target.id}.`);
    for (const alias of pack.aliases) {
      ...
      if (collisions.length > 0) {
        throw new Error(
          `Phase 590 alias collision for ${alias.alias}: ${JSON.stringify(collisions)}.`,
        );
      }
    }
  }
  ...
  try {
    for (const target of TARGETS) {
      ...
      if (existing.length === 0) {
        ...
        throw new Error(`Phase 590 refuses a conflicting identity: ${target.slug}.`);
      }
      ...
      if (maker.length === 0) {
        await transaction.execute({
          ...
          args: [
            stableId("phase590-made-by", target.id),
            target.id,
            PHASE590_PINEIDER_BRAND_ID,
            "Phase 590 verified exact Pineider maker relation",
          ],
        });
      } else if (
        maker.length !== 1 ||
        maker[0]?.target_id !== PHASE590_PINEIDER_BRAND_ID
      ) {
        throw new Error(`Phase 590 ${target.slug} maker topology is ambiguous.`);
      }

      const reverse = await queryRows(
        transaction,
        "SELECT id FROM entity_links WHERE source_id=? AND target_id=? AND link_type='reverse'",
        [PHASE590_PINEIDER_BRAND_ID, target.id],
      );
      if (reverse.length === 0) {
        await transaction.execute({
          sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
          args: [
            stableId("phase590-reverse", target.id),
            PHASE590_PINEIDER_BRAND_ID,
            target.id,
            "Phase 590 Pineider brand public model navigation",
          ],
        });
      } else if (reverse.length !== 1) {
        throw new Error(`Phase 590 ${target.slug} reverse topology is ambiguous.`);
      }
      ...
      if (
        ...
        throw new Error(`Phase 590 publication row is missing: ${target.id}.`);
      }
    }
...
async function assertTerminalState(
  ...
  for (const pack of packs) {
    ...
    if (
      ...
      throw new Error(`Phase 590 terminal publication mismatch: ${pack.entityId}.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 590 approved review set is incomplete: ${pack.entityId}.`);
    }
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 590 terminal topology mismatch: ${target.id}.`);
    }
  }
}

export async function applyPhase590PineiderForgedCarbonMysteryFastFillerContent(
  client: Client,
  options: ApplyPhase590Options,
): Promise<ApplyPhase590Result> {
  ...
  for (const [id, before] of protectedBefore) {
    if ((await entityDigest(client, id)) !== before) {
      throw new Error(`Phase 590 changed protected existing Pineider model: ${id}.`);
    }
  }
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return result;
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase590-pineider-forged-carbon-mystery-fast-filler-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result =
      await applyPhase590PineiderForgedCarbonMysteryFastFillerContent(client, {
        workspaceRoot: process.cwd(),
        reviewer:
          value("--reviewer") ??
          "phase590-pineider-forged-carbon-mystery-fast-filler",
        databasePath,
