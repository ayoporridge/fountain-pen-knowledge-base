...
} from "./data/phase589-pineider-classic-tempi-moderni";
import {
...
export type ApplyPhase589Options = ApplyPhase22Options;
export type ApplyPhase589Result = ApplyPhase22Result;

const TARGETS = [
  {
    id: PHASE589_IDS.classic,
    slug: PHASE589_SLUGS.classic,
    name: "Pineider Classic Palladium Fountain Pen",
  },
  {
    id: PHASE589_IDS.tempi,
    slug: PHASE589_SLUGS.tempi,
    name: "Pineider Tempi Moderni Fountain Pen",
  },
] as const;

const ACCEPTED_INITIAL_BRAND_SOURCE_PREFIX =
  "curated-content:phase588-pineider-brand-depth-refresh-v1:";
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of [
    ...
    if (env[key]?.trim()) {
      throw new Error(`Phase 589 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function assertOwnedCatalog(
  client: Client,
  options: ApplyPhase589Options,
): Promise<string> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) {
    throw new Error("Phase 589 reviewer must not be empty.");
  }
  ...
  if (
    ...
    throw new Error(
      "Phase 589 requires a non-symlink catalog file inside the caller-owned root.",
    );
  }
  ...
  if (owned.nlink !== BigInt(1)) {
    throw new Error("Phase 589 refuses catalog files with more than one hard link.");
  }
  if (
    databasePath === protectedPath ||
    (owned.dev === protectedCatalog.dev && owned.ino === protectedCatalog.ino)
  ) {
    throw new Error("Phase 589 refuses the protected catalog or a hard-link alias.");
  }
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) {
    throw new Error("Phase 589 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 589 owned copy must be migrated through 032.");
  }
  return workspaceRoot;
}

function loadAndValidatePackSet(workspaceRoot: string): LoadedCuratedEntityPack[] {
  const packs = phase589PineiderPacks.map((pack) =>
  ...
  if (
    ...
    throw new Error(
      "Phase 589 requires the exact Pineider brand plus Classic Palladium and Tempi Moderni packs.",
    );
  }
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 589 pack identity mismatch: ${target.id}.`);
    }
  }
  return packs;
}
...
async function assertIdentityPreflight(
  ...
  if (
    !brandPack ||
    brand.length !== 1 ||
    brand[0]?.id !== PHASE589_PINEIDER_BRAND_ID ||
    brand[0]?.type !== "brand" ||
    ...
    throw new Error(
      `Phase 589 Pineider brand identity mismatch: ${JSON.stringify(brand)}.`,
    );
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 589 entity, slug, or name collision: ${target.slug}.`);
    }
    const pack = packs.find((candidate) => candidate.entityId === target.id);
    if (!pack) throw new Error(`Phase 589 missing loaded pack: ${target.id}.`);
    for (const alias of pack.aliases) {
      ...
      if (collisions.length > 0) {
        throw new Error(
          `Phase 589 alias collision for ${alias.alias}: ${JSON.stringify(collisions)}.`,
        );
      }
    }
  }
  ...
  try {
    for (const target of TARGETS) {
      ...
      if (existing.length === 0) {
        ...
        throw new Error(`Phase 589 refuses a conflicting identity: ${target.slug}.`);
      }
      ...
      if (maker.length === 0) {
        await transaction.execute({
          ...
          args: [
            stableId("phase589-made-by", target.id),
            target.id,
            PHASE589_PINEIDER_BRAND_ID,
            "Phase 589 verified exact Pineider maker relation",
          ],
        });
      } else if (
        maker.length !== 1 ||
        maker[0]?.target_id !== PHASE589_PINEIDER_BRAND_ID
      ) {
        throw new Error(`Phase 589 ${target.slug} maker topology is ambiguous.`);
      }

      const reverse = await queryRows(
        transaction,
        "SELECT id FROM entity_links WHERE source_id=? AND target_id=? AND link_type='reverse'",
        [PHASE589_PINEIDER_BRAND_ID, target.id],
      );
      if (reverse.length === 0) {
        await transaction.execute({
          sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
          args: [
            stableId("phase589-reverse", target.id),
            PHASE589_PINEIDER_BRAND_ID,
            target.id,
            "Phase 589 Pineider brand public model navigation",
          ],
        });
      } else if (reverse.length !== 1) {
        throw new Error(`Phase 589 ${target.slug} reverse topology is ambiguous.`);
      }
      ...
      if (
        ...
        throw new Error(`Phase 589 publication row is missing: ${target.id}.`);
      }
    }
...
async function assertTerminalState(
  ...
  for (const pack of packs) {
    ...
    if (
      ...
      throw new Error(`Phase 589 terminal publication mismatch: ${pack.entityId}.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 589 approved review set is incomplete: ${pack.entityId}.`);
    }
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 589 terminal topology mismatch: ${target.id}.`);
    }
  }
}

export async function applyPhase589PineiderClassicTempiModerniContent(
  client: Client,
  options: ApplyPhase589Options,
): Promise<ApplyPhase589Result> {
  ...
  for (const [id, before] of protectedBefore) {
    if ((await entityDigest(client, id)) !== before) {
      throw new Error(`Phase 589 changed protected existing Pineider model: ${id}.`);
    }
  }
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return result;
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase589-pineider-classic-tempi-moderni-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase589PineiderClassicTempiModerniContent(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase589-pineider-classic-tempi-moderni",
      databasePath,
