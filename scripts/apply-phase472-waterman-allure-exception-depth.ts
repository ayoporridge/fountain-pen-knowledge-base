...
import {
  PHASE472_IDS,
  phase472WatermanAllureExceptionDepthPacks,
} from "./data/phase472-waterman-allure-exception-depth";

export type ApplyPhase472Options = ApplyPhase22Options;
export type ApplyPhase472Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase472Options): void {
  for (const key of [
    "TURSO_DATABASE_URL",
    "TURSO_AUTH_TOKEN",
    "FPKG_DATABASE_URL",
    ...
    if (options.env?.[key]?.trim()) {
      throw new Error(
        `Phase 472 refuses inherited remote database selection: ${key}.`,
      );
    }
  }
}
...
async function authority(
  ...
  if (
    ...
    throw new Error("Phase 472 requires an owned, non-symlink catalog copy.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 472 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 472 owned copy must be migrated through 032.");
  }
}
...
async function identity(
  ...
  const ids = Object.values(PHASE472_IDS);
  if (
    packs.length !== ids.length ||
    new Set(packs.map((pack) => pack.entityId)).size !== ids.length
  ) {
    throw new Error("Phase 472 must load Waterman Allure and Exception exactly once.");
  }
  for (const [label, id] of Object.entries(PHASE472_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack?.expectedSlug) throw new Error(`Phase 472 ${label} pack identity mismatch.`);
    ...
    if (
      ...
      throw new Error(
        `Phase 472 ${label} identity mismatch: ${JSON.stringify(existing)}`,
      );
    }
  }
}
...
async function repairModelNavigation(
  ...
  try {
    for (const pack of packs) {
      ...
      if (!brandId) throw new Error(`Phase 472 ${pack.expectedSlug} has no Waterman maker identity.`);
      const maker = await tx.execute({
        sql: "SELECT count(*) AS n FROM entity_links WHERE source_id=? AND target_id=? AND link_type='made_by'",
        args: [pack.entityId, brandId],
      });
      if (Number(maker.rows[0]?.n) !== 1) {
        throw new Error(`Phase 472 ${pack.expectedSlug} must have exactly one made_by relation.`);
      }
      await tx.execute({
        sql: "INSERT OR IGNORE INTO entity_links (id,source_id,target_id,link_type,reason) VALUES (?,?,?,'reverse',?)",
        args: [
          `phase472-reverse-${pack.entityId}`,
          brandId,
          pack.entityId,
          `Phase 472 Waterman navigation for ${pack.expectedSlug}`,
        ],
      });
      ...
      if (Number(reverse.rows[0]?.n) !== 1) {
        throw new Error(`Phase 472 ${pack.expectedSlug} reverse navigation is ambiguous.`);
      }
    }
    await tx.commit();
  } catch (error) {
...
async function applyPacks(
  ...
  options: ApplyPhase472Options,
  packs: LoadedCuratedEntityPack[],
): Promise<ApplyPhase472Result> {
  for (const pack of packs) validatePack(options.workspaceRoot, pack);
  const current = await client.execute({
    sql: `SELECT entity.id, entity.source, publication.status,
  ...
  const entities: ApplyPhase472Result["entities"] = [];
  for (const pack of packs) {
    for (const reviewKind of ["fact", "language", "media"] as const) {
      await recordEntityContentReview(client, {
        ...
        status: "approved",
        notes: `${pack.sourceMarker}; ${reviewKind} review of Waterman Allure and Exception copy.`,
      });
    }
...
export async function applyPhase472WatermanAllureExceptionDepth(
  client: Client,
  options: ApplyPhase472Options,
): Promise<ApplyPhase472Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 472 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase472WatermanAllureExceptionDepthPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase472-waterman-allure-exception-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase472WatermanAllureExceptionDepth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase472-waterman-allure-exception",
