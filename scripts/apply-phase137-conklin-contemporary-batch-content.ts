...
import { applyPhase136ConklinDuragraphContent } from "./apply-phase136-conklin-duragraph-content";
import {
...
} from "./data/phase137-conklin-contemporary-batch";
import type { LoadedCuratedEntityPack } from "./lib/curated-content-pack";

export type ApplyPhase137Options = ApplyPhase22Options;
export type ApplyPhase137Result = ApplyPhase22Result;
...
async function inspectState(client: Client, packs: LoadedCuratedEntityPack[]) {
  ...
  for (const pack of packs) {
    ...
    const found = await rows(
      ...
      `SELECT DISTINCT entity.id,entity.type,entity.slug,entity.name,entity.source
        ...
        WHERE entity.id=? OR entity.slug=?
           ...
           OR entity.source=?`,
      [pack.entityId, pack.expectedSlug, ...names, ...names, pack.sourceMarker],
    );
    if (found.length === 0) {
      states.add("empty");
      continue;
    ...
    if (
      found.length !== 1 ||
      found[0]?.id !== pack.entityId ||
      found[0]?.type !== "pen" ||
      found[0]?.slug !== pack.expectedSlug ||
      found[0]?.name !== pack.canonicalName ||
      found[0]?.source !== pack.sourceMarker
    )
      throw new Error(`Phase 137 target identity is partial, alternate or colliding: ${pack.entityId}.`);
    states.add("terminal");
  }
  if (states.size !== 1)
    throw new Error("Phase 137 refuses mixed empty/terminal target state.");
  return [...states][0]!;
}

async function collisionPreflight(client: Client, packs: LoadedCuratedEntityPack[]) {
  const aliases = packs.flatMap((pack) => pack.aliases.map((item) => item.alias));
  const owners = await rows(
    client,
    `SELECT entity_id,alias FROM entity_aliases
      WHERE lower(alias) IN (${aliases.map(() => "lower(?)").join(",")})
        AND entity_id NOT IN (${packs.map(() => "?").join(",")})`,
    [...aliases, ...packs.map((pack) => pack.entityId)],
  );
  const markerOwners = await rows(
    client,
    `SELECT id FROM entities WHERE source IN (${packs.map(() => "?").join(",")})
      AND id NOT IN (${packs.map(() => "?").join(",")})`,
    [...packs.map((pack) => pack.sourceMarker), ...packs.map((pack) => pack.entityId)],
  );
  if (owners.length > 0 || markerOwners.length > 0)
    throw new Error("Phase 137 target alias/source collision; repair forbidden.");
}
...
async function terminal(client: Client, packs: LoadedCuratedEntityPack[]) {
  ...
  for (const pack of packs) {
    ...
    if (
      ...
      JSON.stringify(links.map((item) => String(item.id))) !==
        JSON.stringify([phase137MadeById(pack.entityId), phase137ReverseId(pack.entityId)].sort()) ||
      ...
      throw new Error(`Phase 137 terminal payload is invalid: ${pack.entityId}.`);
  }
  const brandHash = await computePublicationContentHash(client, PHASE137_CONKLIN_BRAND_ID);
  const brand = (await rows(client, "SELECT status,approved_content_hash FROM entity_publications WHERE entity_id=?", [PHASE137_CONKLIN_BRAND_ID]))[0];
  if (brand?.status !== "published" || brand?.approved_content_hash !== brandHash)
    throw new Error("Phase 137 terminal Conklin publication is stale.");
  return hashes;
}

export async function applyPhase137ConklinContemporaryBatchContent(
  client: Client,
  options: ApplyPhase137Options,
): Promise<ApplyPhase137Result> {
  await applyPhase136ConklinDuragraphContent(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = loadPhase137ConklinPacks(workspaceRoot);
  ...
  const brandBefore = await entityDigest(client, PHASE137_CONKLIN_BRAND_ID, false);
  const reverseBefore = (await rows(client, "SELECT target_id FROM entity_links WHERE source_id=? AND link_type='reverse' ORDER BY target_id", [PHASE137_CONKLIN_BRAND_ID])).map((row) => String(row.target_id));
  ...
  try {
    for (const pack of packs) {
      ...
      await tx.execute({ sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)", args: [phase137MadeById(pack.entityId), pack.entityId, PHASE137_CONKLIN_BRAND_ID, `Phase 137 verified ${pack.canonicalName} maker relation.`] });
    }
  ...
  for (const [id, before] of protectedBefore)
    if ((await entityDigest(client, id)) !== before)
      throw new Error(`Phase 137 changed protected Conklin pen: ${id}.`);
  if ((await entityDigest(client, PHASE137_CONKLIN_BRAND_ID, false)) !== brandBefore)
    throw new Error("Phase 137 changed Conklin non-topology payload.");
  const expectedReverse = reverseBefore.concat(packs.map((pack) => pack.entityId)).sort();
  const reverseAfter = (await rows(client, "SELECT target_id FROM entity_links WHERE source_id=? AND link_type='reverse' ORDER BY target_id", [PHASE137_CONKLIN_BRAND_ID])).map((row) => String(row.target_id));
  if (JSON.stringify(reverseAfter) !== JSON.stringify(expectedReverse))
    throw new Error("Phase 137 Conklin reverse delta is not exact add-four.");

  const reviewer = options.reviewer.trim();
  await reviewPublish(client, PHASE137_CONKLIN_BRAND_ID, reviewer, "Phase 137 post-topology current hash; Conklin brand content was not replayed.");
  const hashes: string[] = [];
  for (const pack of packs)
    hashes.push(await reviewPublish(client, pack.entityId, reviewer, `${pack.sourceMarker}; current family, rejected evidence and dated sample scopes.`));
  await terminal(client, packs);
  for (const [id, before] of protectedBefore)
    if ((await entityDigest(client, id)) !== before)
      throw new Error(`Phase 137 changed protected Conklin pen during publication: ${id}.`);
  if ((await entityDigest(client, PHASE137_CONKLIN_BRAND_ID, false)) !== brandBefore)
    throw new Error("Phase 137 replayed Conklin non-topology content.");
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
...
async function main() {
  ...
  const reviewer = value("--reviewer") ?? "phase137-conklin-contemporary-batch";
  if (!databasePath || !ownedRoot || !protectedCatalogPath)
    throw new Error("Usage: tsx scripts/apply-phase137-conklin-contemporary-batch-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db>");
  const client = createClient({ url: `file:${path.resolve(databasePath)}` });
  try {
    process.stdout.write(`${JSON.stringify(await applyPhase137ConklinContemporaryBatchContent(client, { workspaceRoot: process.cwd(), reviewer, databasePath: path.resolve(databasePath), ownedRoot: path.resolve(ownedRoot), protectedCatalogPath: path.resolve(protectedCatalogPath), protectedCatalogSnapshot: snapshotCatalogFiles(path.resolve(protectedCatalogPath)), env: process.env }), null, 2)}\n`);
  } finally {
    client.close();
  }
}
