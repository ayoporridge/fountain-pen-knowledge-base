...
import {
  ...
  PHASE584_SLUGS,
  phase584LamyPacks,
} from "./data/phase584-lamy-accent-joy-nexx-scala";
import {
...
export type ApplyPhase584Options = ApplyPhase22Options;
export type ApplyPhase584Result = ApplyPhase22Result;
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of [
    ...
    if (env[key]?.trim()) {
      throw new Error(`Phase 584 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function assertOwnedCatalog(
  client: Client,
  options: ApplyPhase584Options,
): Promise<string> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) {
    throw new Error("Phase 584 reviewer must not be empty.");
  }
  ...
  if (
    ...
    throw new Error(
      "Phase 584 requires a non-symlink catalog file inside the caller-owned root.",
    );
  }
  ...
  if (owned.nlink !== BigInt(1)) {
    throw new Error("Phase 584 refuses catalog files with more than one hard link.");
  }
  if (
    databasePath === protectedPath ||
    (owned.dev === protectedCatalog.dev && owned.ino === protectedCatalog.ino)
  ) {
    throw new Error("Phase 584 refuses the protected catalog or a hard-link alias.");
  }
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) {
    throw new Error("Phase 584 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 584 owned copy must be migrated through 032.");
  }
  return workspaceRoot;
}

function loadAndValidatePackSet(workspaceRoot: string): LoadedCuratedEntityPack[] {
  const packs = phase584LamyPacks.map((pack) =>
    loadCuratedEntityPack(workspaceRoot, pack),
  );
  const expected = [PHASE584_LAMY_BRAND_ID, ...TARGETS.map((target) => target.id)];
  if (
    ...
    throw new Error("Phase 584 requires the exact LAMY brand plus four pen packs.");
  }
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 584 pack identity mismatch: ${target.id}.`);
    }
  }
  return packs;
}
...
async function assertIdentityPreflight(
  ...
  if (
    !brandPack ||
    brand.length !== 1 ||
    brand[0]?.id !== PHASE584_LAMY_BRAND_ID ||
    brand[0]?.type !== "brand" ||
    brand[0]?.slug !== "lamy" ||
    ...
    throw new Error(`Phase 584 LAMY brand identity mismatch: ${JSON.stringify(brand)}.`);
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 584 entity, slug, or name collision: ${target.slug}.`);
    }
    const pack = packs.find((candidate) => candidate.entityId === target.id);
    if (!pack) throw new Error(`Phase 584 missing loaded pack: ${target.id}.`);
    for (const alias of pack.aliases) {
      ...
      if (collisions.length > 0) {
        throw new Error(
          `Phase 584 alias collision for ${alias.alias}: ${JSON.stringify(collisions)}.`,
        );
      }
    }
  }
  ...
  try {
    for (const target of TARGETS) {
      ...
      if (existing.length === 0) {
        ...
        throw new Error(`Phase 584 refuses a conflicting identity: ${target.slug}.`);
      }
      ...
      if (maker.length === 0) {
        await transaction.execute({
          ...
          args: [
            stableId("phase584-made-by", target.id),
            target.id,
            PHASE584_LAMY_BRAND_ID,
            "Phase 584 verified exact LAMY maker relation",
          ],
        });
      } else if (
        maker.length !== 1 ||
        maker[0]?.target_id !== PHASE584_LAMY_BRAND_ID
      ) {
        throw new Error(`Phase 584 ${target.slug} maker topology is ambiguous.`);
      }

      const reverse = await queryRows(
        transaction,
        "SELECT id FROM entity_links WHERE source_id=? AND target_id=? AND link_type='reverse'",
        [PHASE584_LAMY_BRAND_ID, target.id],
      );
      if (reverse.length === 0) {
        await transaction.execute({
          sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
          args: [
            stableId("phase584-reverse", target.id),
            PHASE584_LAMY_BRAND_ID,
            target.id,
            "Phase 584 LAMY brand public model navigation",
          ],
        });
      } else if (reverse.length !== 1) {
        throw new Error(`Phase 584 ${target.slug} reverse topology is ambiguous.`);
      }
      ...
      if (
        ...
        throw new Error(`Phase 584 publication row is missing: ${target.id}.`);
      }
    }
...
async function assertTerminalState(
  ...
  for (const pack of packs) {
    ...
    if (
      ...
      throw new Error(`Phase 584 terminal publication mismatch: ${pack.entityId}.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 584 approved review set is incomplete: ${pack.entityId}.`);
    }
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 584 terminal topology mismatch: ${target.id}.`);
    }
  }
}

export async function applyPhase584LamyAccentJoyNexxScalaContent(
  client: Client,
  options: ApplyPhase584Options,
): Promise<ApplyPhase584Result> {
  ...
  const result = await applyCuratedContentPacks(client, options, phase584LamyPacks);
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase584-lamy-accent-joy-nexx-scala-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase584LamyAccentJoyNexxScalaContent(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase584-lamy-four-models",
      databasePath,
