...
import {
  ...
  PHASE131_TARGET_IDS,
  PHASE131_WATERMAN_ID,
} from "./data/phase131-waterman-exception";
import type { LoadedCuratedEntityPack } from "./lib/curated-content-pack";

export type ApplyPhase131Options = ApplyPhase22Options;
export type ApplyPhase131Result = ApplyPhase22Result;
...
const BASELINE = [
  [PHASE131_CARENE_ID, "pen", "waterman-carene", "curated-content:phase83-waterman-carene-v1:"],
  [PHASE131_EXPERT_ID, "pen", "waterman-expert", "curated-content:phase83-waterman-expert-v1:"],
  [PHASE131_HEMISPHERE_ID, "pen", "waterman-hemisphere", "curated-content:phase83-waterman-hemisphere-v1:"],
  [PHASE131_ALLURE_ID, "pen", "waterman-allure", "curated-content:phase83-waterman-allure-v1:"],
] as const;
...
function rejectRemote(env: NodeJS.ProcessEnv) {
  for (const key of [
    ...
    if (env[key]?.trim())
      throw new Error(`Phase 131 refuses inherited remote selection: ${key}.`);
}
...
async function authority(client: Client, options: ApplyPhase131Options) {
  rejectRemote(options.env ?? process.env);
  if (!options.reviewer.trim())
    throw new Error("Phase 131 reviewer must not be empty.");
  if (!ALLOWED_REPOS.has(options.workspaceRoot))
    throw new Error("Phase 131 requires the verified CodeBuddy/Documents repo pair.");
  ...
  if (workspaceRoot !== CANONICAL_REPO || gitRoot !== CANONICAL_REPO)
    throw new Error("Phase 131 verified repo aliases must resolve identically.");
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  if (fs.lstatSync(options.databasePath).isSymbolicLink())
    throw new Error("Phase 131 owned database must not be a symlink.");
  const ownedRoot = fs.realpathSync.native(options.ownedRoot);
  const databasePath = fs.realpathSync.native(options.databasePath);
  const protectedPath = fs.realpathSync.native(options.protectedCatalogPath);
  if (!fs.statSync(ownedRoot).isDirectory() || !inside(databasePath, ownedRoot))
    throw new Error("Phase 131 requires a database inside caller-owned root.");
  ...
  if (
    ...
    throw new Error("Phase 131 refuses protected catalog or hard-link aliases.");
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath)
    throw new Error("Phase 131 client/path mismatch.");
  const migration = await client.execute({
    sql: "SELECT 1 FROM migrations WHERE name=? AND checksum IS NOT NULL",
    args: ["032_taxonomy_identity.sql"],
  });
  if (migration.rows.length !== 1)
    throw new Error("Phase 131 owned copy must be migrated through 032.");
...
async function baseline(client: Client) {
  ...
  if (
    brand?.type !== "brand" ||
    brand?.slug !== "waterman" ||
    !String(brand?.source).startsWith("curated-content:phase83-waterman-brand-current-v1:") ||
    brand?.status !== "published" ||
    Number(brand?.is_public) !== 1
  )
    throw new Error("Phase 131 requires the exact published Waterman baseline.");
  for (const [id, type, slug, marker] of BASELINE) {
    ...
    if (
      ...
      throw new Error(`Phase 131 prerequisite baseline is invalid: ${id}.`);
  }
}
...
async function state(
  ...
  if (found.length !== packs.length)
    throw new Error("Phase 131 target state is partial or colliding.");
  ...
  for (const pack of packs) {
    ...
    if (
      ...
      throw new Error("Phase 131 terminal identity is partial or alternate.");
  }
  return "terminal";
}
...
async function collisionPreflight(
  ...
  if (aliasOwners.length > 0 || markerOwners.length > 0)
    throw new Error("Phase 131 target alias/source collision; repair forbidden.");
}
...
async function terminal(
  ...
  for (let index = 0; index < packs.length; index += 1) {
    ...
    if (
      ...
      throw new Error("Phase 131 terminal payload/topology/publication is invalid.");
  }
  ...
  if (brand?.status !== "published" || brand?.approved_content_hash !== brandHash)
    throw new Error("Phase 131 terminal Waterman publication is stale.");
  return hashes;
}

export async function applyPhase131WatermanExceptionContent(
  client: Client,
  options: ApplyPhase131Options,
): Promise<ApplyPhase131Result> {
  const workspaceRoot = await authority(client, options);
  const packs = loadPhase131WatermanExceptionPacks(workspaceRoot);
  ...
  const brandBefore = await entityDigest(client, PHASE131_WATERMAN_ID, false);
  const reverseBefore = (
    await rows(
      client,
  ...
  try {
    for (let index = 0; index < packs.length; index += 1) {
      ...
      await tx.execute({
        ...
        args: [
          PHASE131_MADE_BY_IDS[index],
          pack.entityId,
          PHASE131_WATERMAN_ID,
          `Phase 131 exact ${pack.canonicalName} maker relation`,
        ],
      });
    }
  ...
  if ((await entityDigest(client, PHASE131_WATERMAN_ID, false)) !== brandBefore)
    throw new Error("Phase 131 changed Waterman non-topology payload.");
  const reverseAfter = (
    await rows(
      client,
      "SELECT target_id FROM entity_links WHERE source_id=? AND link_type='reverse' ORDER BY target_id",
  ...
  if (
    ...
    JSON.stringify(reverseBefore.concat([...PHASE131_TARGET_IDS]).sort())
  )
    throw new Error("Phase 131 reverse delta is not exact add-one.");
  for (const id of PROTECTED_IDS)
    if ((await entityDigest(client, id)) !== protectedBefore.get(id))
      throw new Error(`Phase 131 changed protected Waterman entity ${id}.`);
  ...
  await reviewPublish(
    ...
    PHASE131_WATERMAN_ID,
    reviewer,
    "Phase 131 post-topology current hash; no Waterman pack replay.",
  );
  const hashes: string[] = [];
  for (const pack of packs)
    hashes.push(
  ...
  if ((await entityDigest(client, PHASE131_WATERMAN_ID, false)) !== brandBefore)
    throw new Error("Phase 131 replayed Waterman non-topology payload.");
  for (const id of PROTECTED_IDS)
    if ((await entityDigest(client, id)) !== protectedBefore.get(id))
      throw new Error(
        `Phase 131 changed protected Waterman entity ${id} after publication.`,
      );
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return {
    entities: packs.map((pack, index) => ({
...
async function main() {
  ...
  const reviewer = value("--reviewer") ?? "phase131-waterman-exception";
  if (!databasePath || !ownedRoot || !protectedCatalogPath)
    throw new Error(
      "Usage: tsx scripts/apply-phase131-waterman-exception-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db>",
    );
  const client = createClient({ url: `file:${path.resolve(databasePath)}` });
  try {
    process.stdout.write(
      `${JSON.stringify(
        await applyPhase131WatermanExceptionContent(client, {
...
export { PHASE131_EXCEPTION_ID, PHASE131_EXCEPTION_SLUG };
