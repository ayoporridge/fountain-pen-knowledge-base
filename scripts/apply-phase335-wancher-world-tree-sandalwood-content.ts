...
} from "./data/phase335-wancher-world-tree-sandalwood";

export type ApplyPhase335Options = ApplyPhase22Options;
export type ApplyPhase335Result = ApplyPhase22Result;
export {
  PHASE335_WORLD_TREE_SANDALWOOD_ID,
  PHASE335_WORLD_TREE_SANDALWOOD_SLUG,
  PHASE335_WANCHER_BRAND_ID,
...
function assertNoRemote(options: ApplyPhase335Options): void {
  for (const key of [
    "TURSO_DATABASE_URL",
    "TURSO_AUTH_TOKEN",
    "FPKG_DATABASE_URL",
    ...
    if (options.env?.[key]?.trim()) {
      throw new Error(
        `Phase 335 refuses inherited remote database selection: ${key}.`,
      );
    }
  }
}
...
async function authority(
  client: Client,
  options: ApplyPhase335Options,
): Promise<void> {
  assertNoRemote(options);
  if (!options.reviewer.trim()) throw new Error("Phase 335 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 335 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 335 refuses the protected catalog or hard-link alias.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 335 owned copy must be migrated through 032.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 335 client is not bound to the authorized owned copy.");
  }
}
...
async function identity(
  ...
  if (
    ...
    throw new Error(`Phase 335 Wancher brand identity mismatch: ${JSON.stringify(brand)}`);
  }
  const pack = packs.find((item) => item.entityId === PHASE335_WORLD_TREE_SANDALWOOD_ID);
  if (!pack) throw new Error("Phase 335 World Tree Sandalwood pack missing.");
  const existing = await rows(
    client,
    "SELECT id,type,slug,name FROM entities WHERE id=? OR slug=?",
    [PHASE335_WORLD_TREE_SANDALWOOD_ID, PHASE335_WORLD_TREE_SANDALWOOD_SLUG],
  );
  if (
    existing.length > 1 ||
    (existing.length === 1 &&
      (existing[0]?.id !== PHASE335_WORLD_TREE_SANDALWOOD_ID ||
        existing[0]?.type !== pack.expectedType ||
        existing[0]?.slug !== pack.expectedSlug ||
        existing[0]?.name !== pack.canonicalName))
  ) {
    throw new Error(`Phase 335 World Tree Sandalwood identity collision: ${JSON.stringify(existing)}`);
  }
}
...
async function topology(client: Client): Promise<void> {
  ...
  try {
    ...
    await tx.execute({
      ...
      args: [PHASE335_WORLD_TREE_SANDALWOOD_ID, PHASE335_WANCHER_BRAND_ID],
    });
    await tx.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
      args: [
        "phase335-made-by-wancher-world-tree-sandalwood",
        PHASE335_WORLD_TREE_SANDALWOOD_ID,
        PHASE335_WANCHER_BRAND_ID,
        "Phase 335 verified Wancher World Tree Sandalwood maker relation",
      ],
    });
    await tx.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
      args: [
        "phase335-reverse-wancher-world-tree-sandalwood",
        PHASE335_WANCHER_BRAND_ID,
        PHASE335_WORLD_TREE_SANDALWOOD_ID,
        "Phase 335 Wancher brand navigation to World Tree Sandalwood",
      ],
    });
    ...
    if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE335_WANCHER_BRAND_ID
    ) {
      throw new Error("Phase 335 World Tree Sandalwood maker topology is ambiguous.");
    }
    await tx.commit();
  } catch (error) {
...
async function ensurePublicBrandNavigation(client: Client): Promise<void> {
  const pens = await client.execute({
    ...
    args: [PHASE335_WANCHER_BRAND_ID],
  });
  const tx = await client.transaction("write");
  try {
    for (const row of pens.rows) {
      ...
      await tx.execute({
        sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
        args: [
          `phase335-reverse-wancher-${penId}`,
          PHASE335_WANCHER_BRAND_ID,
          penId,
          "Phase 335 public Wancher model navigation repair",
        ],
      });
    }
    await tx.commit();
  ...
  const missing = await client.execute({
    ...
    args: [PHASE335_WANCHER_BRAND_ID, PHASE335_WANCHER_BRAND_ID],
  });
  if (missing.rows.length) {
    throw new Error(
      `Phase 335 Wancher public navigation incomplete: ${missing.rows
...
export async function applyPhase335WancherWorldTreeSandalwoodContent(
  client: Client,
  options: ApplyPhase335Options,
): Promise<ApplyPhase335Result> {
  await authority(client, options);
  const packs = phase335WancherWorldTreeSandalwoodPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase335-wancher-world-tree-sandalwood-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase335WancherWorldTreeSandalwoodContent(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase335-wancher-world-tree-sandalwood",
