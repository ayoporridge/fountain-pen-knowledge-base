...
import {
  ...
  PHASE111_CUSTOM_NS_SLUG,
  PHASE111_ELABO_METAL_ID,
  PHASE111_ELABO_METAL_SLUG,
  PHASE111_ELABO_RESIN_ID,
  PHASE111_ELABO_RESIN_SLUG,
  PHASE111_IDENTITY_MARKERS,
  PHASE111_LEGACY_AMBIGUITY_MARKERS,
  PHASE111_LIGHTIVE_ID,
  PHASE111_LIGHTIVE_SLUG,
  PHASE111_PILOT_ID,
  loadPhase111PilotPacks,
} from "./data/phase111-pilot-elabo-metal-resin-custom-ns-lightive";
import {
  curatedId,
  packId,
...
export type ApplyPhase111Options = ApplyPhase22Options;
export type ApplyPhase111Result = ApplyPhase22Result;

const TARGETS = [
  {
    id: PHASE111_ELABO_METAL_ID,
    slug: PHASE111_ELABO_METAL_SLUG,
    name: "百乐 Pilot Elabo 金属轴 FE-25SR",
    makerId: "phase111-maker-elabo-metal-pilot",
  },
  {
    id: PHASE111_ELABO_RESIN_ID,
    slug: PHASE111_ELABO_RESIN_SLUG,
    name: "百乐 Pilot Elabo 树脂轴 FE-18SR",
    makerId: "phase111-maker-elabo-resin-pilot",
  },
  {
    id: PHASE111_CUSTOM_NS_ID,
    slug: PHASE111_CUSTOM_NS_SLUG,
    name: "百乐 Pilot Custom NS",
    makerId: "phase111-maker-custom-ns-pilot",
  },
  {
    id: PHASE111_LIGHTIVE_ID,
    slug: PHASE111_LIGHTIVE_SLUG,
    name: "百乐 Pilot Lightive",
    makerId: "phase111-maker-lightive-pilot",
  },
] as const;
...
async function brandNonTopologyPayload(client: Client): Promise<string> {
  ...
  for (const sql of queries) {
    const result = await client.execute({ sql, args: [PHASE111_PILOT_ID] });
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of [
    ...
    if (env[key]?.trim()) {
      throw new Error(`Phase 111 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function assertOwnedCatalog(
  client: Client,
  options: ApplyPhase111Options,
): Promise<string> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 111 reviewer must not be empty.");
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);

  const ownedRoot = fs.realpathSync.native(options.ownedRoot);
  const databasePath = fs.realpathSync.native(options.databasePath);
  const protectedPath = fs.realpathSync.native(options.protectedCatalogPath);
  const allowedWorkspaceInputs = new Set([
    "/Users/xz/CodeBuddy/fountain-pen-graph",
    "/Users/xz/Documents/fountain-pen-graph",
  ]);
  if (!allowedWorkspaceInputs.has(options.workspaceRoot)) {
    throw new Error("Phase 111 repo authority only accepts the verified CodeBuddy alias/canonical pair.");
  }
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const expectedWorkspaceRoot = "/Users/xz/Documents/fountain-pen-graph";
  const gitRoot = fs.realpathSync.native(
    execFileSync("git", ["-C", options.workspaceRoot, "rev-parse", "--show-toplevel"], { encoding: "utf8" }).trim(),
  );
  if (workspaceRoot !== expectedWorkspaceRoot || gitRoot !== expectedWorkspaceRoot) {
    throw new Error("Phase 111 repo authority could not prove the verified alias/canonical git-root pair.");
  }
  if (
    ...
    throw new Error(
      "Phase 111 requires a non-symlink catalog file inside the caller-owned root.",
    );
  }
  ...
  if (
    ...
    throw new Error("Phase 111 refuses the protected catalog or a hard-link alias.");
  }
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) {
    throw new Error("Phase 111 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 111 owned copy must be migrated through 032.");
  }
...
function validatePacks(
  ...
  if (
    packs.length !== 4 ||
    packs[0]?.entityId !== PHASE111_ELABO_METAL_ID ||
    packs[1]?.entityId !== PHASE111_ELABO_RESIN_ID ||
    packs[2]?.entityId !== PHASE111_CUSTOM_NS_ID ||
    packs[3]?.entityId !== PHASE111_LIGHTIVE_ID
  ) {
    throw new Error("Phase 111 requires the exact ordered Elabo metal/Elabo resin/Custom NS/Lightive pack set.");
  }
  for (const pack of packs) {
    ...
    if (
      !pack.scopes.some(
        (scope) => scope.productionState === "historical",
      )
    ) {
  ...
  if (currentSpecs.some((spec) => /CON-20|CON-50|standard-international/i.test(spec))) {
    throw new Error("Phase 111 current specs contain historical or regional filling claims.");
  }
  const serialized = packs.map((pack) => JSON.stringify(pack));
  if (!serialized[0]?.includes("traditional-flex guarantee") || !serialized[0]?.includes("FE-25SR"))
    throw new Error("Phase 111 metal pack lacks its soft-nib/FE-25SR boundary.");
  if (!serialized[1]?.includes("custom Namiki video") || !serialized[1]?.includes("CON-70N excluded"))
    throw new Error("Phase 111 resin pack lacks its custom-video/CON-40 boundary.");
  if (!serialized[2]?.includes("professional_sample_2020_pre_refresh") || !serialized[2]?.includes("special-alloy steel"))
    throw new Error("Phase 111 Custom NS pack lacks its current/pre-refresh boundary.");
  if (!serialized[3]?.includes("independent_dry_test") || !serialized[3]?.includes("P-FLT-1"))
    throw new Error("Phase 111 Lightive pack lacks its current/dry-test boundary.");
  return packs;
}

async function assertIdentityPreflight(client: Client): Promise<number> {
  const brand = await client.execute({
    ...
    args: [PHASE111_PILOT_ID],
  });
  const pilot = brand.rows[0];
  if (
    brand.rows.length !== 1 ||
    String(pilot?.id) !== PHASE111_PILOT_ID ||
    String(pilot?.type) !== "brand" ||
    String(pilot?.slug) !== "pilot"
  ) {
    throw new Error("Phase 111 requires the exact Pilot brand identity prerequisite.");
  }

  const legacyNames = PHASE111_LEGACY_AMBIGUITY_MARKERS.names.map((name) => name.toLowerCase());
  const legacy = await client.execute({
    sql: `SELECT e.id,e.type,e.slug,e.name,'entity' AS locator FROM entities e
          WHERE e.id IN (${PHASE111_LEGACY_AMBIGUITY_MARKERS.ids.map(() => "?").join(",")})
             OR lower(e.slug) IN (${PHASE111_LEGACY_AMBIGUITY_MARKERS.slugs.map(() => "?").join(",")})
             OR lower(trim(e.name)) IN (${legacyNames.map(() => "?").join(",")})
          UNION ALL
          SELECT e.id,e.type,e.slug,a.alias AS name,'alias' AS locator
          FROM entity_aliases a JOIN entities e ON e.id=a.entity_id
          WHERE lower(trim(a.alias)) IN (${legacyNames.map(() => "?").join(",")})
          ORDER BY id`,
    args: [
      ...PHASE111_LEGACY_AMBIGUITY_MARKERS.ids,
      ...PHASE111_LEGACY_AMBIGUITY_MARKERS.slugs.map((slug) => slug.toLowerCase()),
      ...legacyNames,
      ...legacyNames,
    ],
  });
  if (legacy.rows.length > 0) {
    const candidate = legacy.rows[0];
    throw new Error(`Phase 111 legacy-ambiguity collision: candidate=${String(candidate?.id)} type=${String(candidate?.type)} slug=${String(candidate?.slug)} locator=${String(candidate?.locator)}:${String(candidate?.name)}.`);
  }

  let existingTargets = 0;
  for (const marker of Object.values(PHASE111_IDENTITY_MARKERS)) {
    const target = TARGETS.find((item) => item.id === marker.id);
    if (!target) throw new Error(`Phase 111 marker has no target: ${marker.id}.`);
    ...
    for (const [criterion, candidates] of [["exact-name", [...entities.rows, ...aliases.rows]], ["exact-SKU", variants.rows], ["exact-source-URL", urls.rows]] as const) {
      for (const candidate of candidates) {
        if (String(candidate.id) !== marker.id) throw new Error(`Phase 111 ${criterion} collision: candidate=${String(candidate.id)} type=${String(candidate.type)} slug=${String(candidate.slug)} locator=${String(candidate.name)}.`);
      }
    }
    const own = entities.rows.filter((row) => String(row.id) === marker.id);
    if (own.length > 0 && (own.length !== 1 || String(own[0]?.type) !== "pen" || String(own[0]?.slug) !== marker.slug || String(own[0]?.name) !== target.name)) throw new Error(`Phase 111 target-ID payload mismatch: ${marker.id}.`);
    if (own.length === 1) {
      existingTargets += 1;
      ...
      if (maker.rows.length !== 1 || String(maker.rows[0]?.id) !== target.makerId || String(maker.rows[0]?.target_id) !== PHASE111_PILOT_ID || String(maker.rows[0]?.link_type) !== "made_by" || maker.rows[0]?.reason !== null) {
        throw new Error(`Phase 111 exact made_by topology mismatch: ${marker.id}.`);
      }
    }
  }
  if (
    ...
    throw new Error("Phase 111 requires the published/readiness Pilot brand prerequisite.");
  }
...
async function exactTerminalHash(
  ...
  return maker.rows.length === 1 &&
    ...
    String(maker.rows[0]?.target_id) === PHASE111_PILOT_ID &&
...
async function assertBrandPublishedAtCurrentHash(client: Client): Promise<string> {
  const currentHash = await computePublicationContentHash(client, PHASE111_PILOT_ID);
  const state = await client.execute({
    sql: `SELECT publication.status,publication.approved_content_hash,
          ...
          LEFT JOIN public_entity_readiness readiness ON readiness.entity_id=publication.entity_id AND readiness.contract_version=3
  ...
  if (
    ...
    throw new Error("Phase 111 Pilot brand is not published at the exact post-topology current hash.");
  }
  const reviews = await client.execute({
    sql: "SELECT review_kind,count(*) AS total FROM entity_content_reviews WHERE entity_id=? AND content_hash=? AND status='approved' GROUP BY review_kind",
    args: [PHASE111_PILOT_ID, currentHash],
  });
  const counts = new Map(reviews.rows.map((review) => [String(review.review_kind), Number(review.total)]));
  if (["fact", "language", "media", "publication"].some((kind) => counts.get(kind) !== 1)) {
    throw new Error("Phase 111 Pilot brand lacks exact current-hash review approvals.");
  }
  return currentHash;
}
  ...
  try {
    for (const target of TARGETS) {
      ...
      await transaction.execute({
        ...
        args: [target.makerId, target.id, PHASE111_PILOT_ID],
      });
    }
  ...
  for (const source of sources.values()) {
    ...
    await transaction.execute({
      ...
      args: [
        ...
        source.author ?? null,
        source.homepageUrl,
        `Phase 111 curated source: ${source.registryKey}`,
        source.retrievedAt,
        source.tier,
...
async function installPack(
  ...
  const sourceId = (key: string) => {
    ...
    if (!value) throw new Error(`Phase 111 source mapping missing: ${key}.`);
    return value;
  };
  ...
  if (updated.rowsAffected !== 1) {
    throw new Error(`Phase 111 entity identity changed during install: ${pack.entityId}.`);
  }
  await transaction.execute({
  ...
  for (const alias of pack.aliases) {
    ...
    if (!source) throw new Error(`Phase 111 alias source missing: ${alias.sourceKey}.`);
    await transaction.execute({
  ...
  for (const claim of pack.claims) {
    ...
    for (const evidence of claim.evidence) {
      ...
      if (!scopeId) throw new Error(`Phase 111 claim scope missing: ${evidence.scopeKey}.`);
  ...
  if (!pack.spec) throw new Error(`Phase 111 model spec missing: ${pack.entityId}.`);
  ...
  for (const evidence of pack.spec.evidence) {
    const scopeId = scopeIds.get(evidence.scopeKey);
    if (!scopeId) throw new Error(`Phase 111 spec scope missing: ${evidence.scopeKey}.`);
...
export async function applyPhase111PilotContent(
  client: Client,
  options: ApplyPhase111Options,
): Promise<ApplyPhase111Result> {
  ...
  const existingTargets = await assertIdentityPreflight(client);
  ...
  if (terminalHashes.every((hash): hash is string => hash !== null)) {
    await assertBrandPublishedAtCurrentHash(client);
    return {
  ...
  if (existingTargets !== 0) {
    throw new Error("Phase 111 terminal drift detected; existing target rows are never repaired, re-reviewed, or reloaded.");
  }

  const brandHashBefore = await computePublicationContentHash(client, PHASE111_PILOT_ID);
  const brandPayloadBefore = await brandNonTopologyPayload(client);
  await createIdentitiesAndTopology(client);
  const brandHashAfter = await computePublicationContentHash(client, PHASE111_PILOT_ID);
  const brandPayloadAfter = await brandNonTopologyPayload(client);
  if (brandPayloadAfter !== brandPayloadBefore) {
    throw new Error("Phase 111 Pilot non-topology payload/source marker changed; companion replay requires replanning.");
  }
  if (brandHashAfter === brandHashBefore) {
    throw new Error("Phase 111 Pilot contract hash did not change for the exact made_by topology.");
  }
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE111_PILOT_ID,
      reviewKind,
      reviewer: options.reviewer,
      status: "approved",
      notes: "Phase 111 topology-only current-hash review; Phase 84 Pilot brand payload was not replayed.",
    });
  }
  await publishEntity(client, { entityId: PHASE111_PILOT_ID, reviewer: options.reviewer });
  await assertBrandPublishedAtCurrentHash(client);
  const transaction = await client.transaction("write");
  try {
  ...
  const entities: ApplyPhase111Result["entities"] = [];
  for (const pack of packs) {
    for (const reviewKind of ["fact", "language", "media"] as const) {
      await recordEntityContentReview(client, {
        entityId: pack.entityId,
        ...
        reviewer: options.reviewer,
        status: "approved",
        notes: `${pack.sourceMarker}; ${reviewKind} review of checked-in Phase 111 sourced copy.`,
      });
    }
...
function requiredArg(name: string): string {
  ...
  if (!value?.trim()) throw new Error(`Phase 111 requires ${name}.`);
  return value;
}
...
async function main(): Promise<void> {
  ...
  const reviewer = process.argv.includes("--reviewer")
    ...
    : "phase111-content-review";
  const client = createClient({ url: `file:${databasePath}` });
  try {
    const result = await applyPhase111PilotContent(client, {
      databasePath,
      ownedRoot,
