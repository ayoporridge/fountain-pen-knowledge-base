...
import {
  PHASE331_WANCHER_BRAND_ID,
  PHASE331_WORLD_TREE_EBONY_ID,
  PHASE331_WORLD_TREE_EBONY_SLUG,
  phase331WancherWorldTreeEbonyPacks,
} from "./data/phase331-wancher-world-tree-ebony";

export type ApplyPhase331Options = ApplyPhase22Options;
export type ApplyPhase331Result = ApplyPhase22Result;
export {
  PHASE331_WANCHER_BRAND_ID,
  PHASE331_WORLD_TREE_EBONY_ID,
  PHASE331_WORLD_TREE_EBONY_SLUG,
...
function assertNoRemote(options: ApplyPhase331Options): void {
  for (const key of [
    "TURSO_DATABASE_URL",
    "TURSO_AUTH_TOKEN",
    "FPKG_DATABASE_URL",
    ...
    if (options.env?.[key]?.trim()) {
      throw new Error(
        `Phase 331 refuses inherited remote database selection: ${key}.`,
      );
    }
  }
}
...
async function authority(
  client: Client,
  options: ApplyPhase331Options,
): Promise<void> {
  assertNoRemote(options);
  if (!options.reviewer.trim()) throw new Error("Phase 331 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 331 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 331 refuses the protected catalog or hard-link alias.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 331 owned copy must be migrated through 032.");
  }
  ...
  if (
    ...
    throw new Error("Phase 331 client is not bound to the authorized owned copy.");
  }
}
...
async function identity(
  ...
  if (
    ...
    throw new Error(`Phase 331 Wancher brand identity mismatch: ${JSON.stringify(brand)}`);
  }
  const pack = packs.find((item) => item.entityId === PHASE331_WORLD_TREE_EBONY_ID);
  if (!pack) throw new Error("Phase 331 World Tree Ebony pack missing.");
  const existing = await rows(
    client,
    "SELECT id,type,slug,name FROM entities WHERE id=? OR slug=?",
    [PHASE331_WORLD_TREE_EBONY_ID, PHASE331_WORLD_TREE_EBONY_SLUG],
  );
  if (
    existing.length > 1 ||
    (existing.length === 1 &&
      (existing[0]?.id !== PHASE331_WORLD_TREE_EBONY_ID ||
        existing[0]?.type !== pack.expectedType ||
        existing[0]?.slug !== pack.expectedSlug ||
        existing[0]?.name !== pack.canonicalName))
  ) {
    throw new Error(`Phase 331 World Tree Ebony identity collision: ${JSON.stringify(existing)}`);
  }
}
  ...
  try {
    await tx.execute({
      ...
      args: [
        PHASE331_WORLD_TREE_EBONY_ID,
        PHASE331_WORLD_TREE_EBONY_SLUG,
        "Wancher World Tree – Ebony",
      ],
    });
    await tx.execute({
      sql: "DELETE FROM entity_links WHERE source_id=? AND link_type='made_by' AND target_id<>?",
      args: [PHASE331_WORLD_TREE_EBONY_ID, PHASE331_WANCHER_BRAND_ID],
    });
    await tx.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
      args: [
        "phase331-made-by-wancher-world-tree-ebony",
        PHASE331_WORLD_TREE_EBONY_ID,
        PHASE331_WANCHER_BRAND_ID,
        "Phase 331 verified Wancher World Tree Ebony maker relation",
      ],
    });
    await tx.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
      args: [
        "phase331-reverse-wancher-world-tree-ebony",
        PHASE331_WANCHER_BRAND_ID,
        PHASE331_WORLD_TREE_EBONY_ID,
        "Phase 331 Wancher brand navigation to World Tree Ebony",
      ],
    });
    const maker = await tx.execute({
      sql: "SELECT target_id FROM entity_links WHERE source_id=? AND link_type='made_by'",
      args: [PHASE331_WORLD_TREE_EBONY_ID],
    });
    if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE331_WANCHER_BRAND_ID
    ) {
      throw new Error("Phase 331 World Tree Ebony maker topology is ambiguous.");
    }
    await tx.commit();
  } catch (error) {
...
async function ensurePublicBrandNavigation(client: Client): Promise<void> {
  const pens = await client.execute({
    ...
    args: [PHASE331_WANCHER_BRAND_ID],
  });
  const tx = await client.transaction("write");
  try {
    for (const row of pens.rows) {
      ...
      await tx.execute({
        sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
        args: [
          `phase331-reverse-wancher-${penId}`,
          PHASE331_WANCHER_BRAND_ID,
          penId,
          "Phase 331 public Wancher model navigation repair",
        ],
      });
    }
    await tx.commit();
  ...
  const missing = await client.execute({
    ...
    args: [PHASE331_WANCHER_BRAND_ID, PHASE331_WANCHER_BRAND_ID],
  });
  if (missing.rows.length) {
    throw new Error(
      `Phase 331 Wancher public navigation incomplete: ${missing.rows
...
export async function applyPhase331WancherWorldTreeEbonyContent(
  client: Client,
  options: ApplyPhase331Options,
): Promise<ApplyPhase331Result> {
  await authority(client, options);
  const packs = phase331WancherWorldTreeEbonyPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase331-wancher-world-tree-ebony-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase331WancherWorldTreeEbonyContent(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase331-wancher-world-tree-ebony",
