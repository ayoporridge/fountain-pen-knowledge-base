...
} from "./data/phase492-sailor-special-and-1911-depth";

export type ApplyPhase492Options = ApplyPhase22Options;
export type ApplyPhase492Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase492Options): void {
  for (const key of [
    "TURSO_DATABASE_URL",
    "TURSO_AUTH_TOKEN",
    "FPKG_DATABASE_URL",
    ...
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 492 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function authority(client: Client, options: ApplyPhase492Options): Promise<void> {
  ...
  if (
    ...
    throw new Error("Phase 492 requires an owned, non-symlink catalog copy.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 492 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 492 owned copy must be migrated through 032.");
  }
}
...
async function identity(
  ...
  const ids = Object.values(PHASE492_IDS);
  if (
    packs.length !== ids.length ||
    new Set(packs.map((pack) => pack.entityId)).size !== ids.length
  ) {
    throw new Error("Phase 492 must load three Sailor pen packs exactly once.");
  }
  for (const [label, id] of Object.entries(PHASE492_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack?.expectedSlug) throw new Error(`Phase 492 ${label} pack identity mismatch.`);
    ...
    if (
      ...
      throw new Error(`Phase 492 ${label} identity mismatch: ${JSON.stringify(existing)}`);
    }
  }
}
...
async function repairModelNavigation(
  ...
  try {
    for (const pack of packs) {
      ...
      if (!brandId) throw new Error(`Phase 492 ${pack.expectedSlug} has no maker identity.`);
      const maker = await tx.execute({
        sql: "SELECT count(*) AS n FROM entity_links WHERE source_id=? AND target_id=? AND link_type='made_by'",
        args: [pack.entityId, brandId],
      });
      if (Number(maker.rows[0]?.n) !== 1) {
        throw new Error(`Phase 492 ${pack.expectedSlug} must have exactly one made_by relation.`);
      }
      const reverse = await tx.execute({
        sql: "SELECT count(*) AS n FROM entity_links WHERE source_id=? AND target_id=? AND link_type='reverse'",
        args: [brandId, pack.entityId],
      });
      if (Number(reverse.rows[0]?.n) === 0) {
        await tx.execute({
          sql: "INSERT INTO entity_links (id,source_id,target_id,link_type,reason) VALUES (?,?,?,'reverse',?)",
          args: [
            `phase492-reverse-${pack.entityId}`,
            brandId,
            pack.entityId,
            `Phase 492 model navigation for ${pack.expectedSlug}`,
          ],
        });
      } else if (Number(reverse.rows[0]?.n) !== 1) {
        throw new Error(`Phase 492 ${pack.expectedSlug} reverse navigation is ambiguous.`);
      }
    }
    await tx.commit();
  } catch (error) {
...
async function applyPacks(
  client: Client,
  options: ApplyPhase492Options,
  packs: LoadedCuratedEntityPack[],
): Promise<ApplyPhase492Result> {
  ...
  const entities: ApplyPhase492Result["entities"] = [];
  for (const pack of packs) {
    for (const reviewKind of ["fact", "language", "media"] as const) {
      await recordEntityContentReview(client, {
        ...
        status: "approved",
        notes: `${pack.sourceMarker}; Phase 492 fact/language/media review.`,
      });
    }
...
export async function applyPhase492SailorSpecialAnd1911Depth(
  client: Client,
  options: ApplyPhase492Options,
): Promise<ApplyPhase492Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 492 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase492SailorSpecialAnd1911DepthPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase492-sailor-special-and-1911-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  const client = createClient({ url: `file:${path.resolve(database)}` });
  try {
    const result = await applyPhase492SailorSpecialAnd1911Depth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase492-sailor-special-and-1911-depth",
