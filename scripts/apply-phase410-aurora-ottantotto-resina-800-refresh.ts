...
import {
  ...
  PHASE410_800_SLUG,
  PHASE410_AURORA_ID,
  phase410AuroraOttantottoResina800RefreshPacks,
} from "./data/phase410-aurora-ottantotto-resina-800-refresh";
import {
...
export type ApplyPhase410Options = ApplyPhase22Options;
export type ApplyPhase410Result = ApplyPhase22Result;
...
function rejectRemote(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (env[key]?.trim()) {
      throw new Error(`Phase 410 refuses inherited remote database selection: ${key}.`);
    }
  }
}

async function assertAuthority(client: Client, options: ApplyPhase410Options): Promise<void> {
  rejectRemote(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 410 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 410 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 410 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 410 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 410 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  const result = await client.execute({
    ...
    args: [PHASE410_AURORA_ID, PHASE410_800_ID],
  });
  const brand = result.rows.find((row) => String(row.id) === PHASE410_AURORA_ID);
  const model = result.rows.find((row) => String(row.id) === PHASE410_800_ID);
  if (
    !brand ||
    String(brand.type) !== "brand" ||
    String(brand.slug) !== "aurora" ||
    String(brand.name) !== "奥罗拉 Aurora" ||
    !model ||
    String(model.type) !== "pen" ||
    String(model.slug) !== pack.expectedSlug ||
    String(model.name) !== PHASE410_800_NAME
  ) {
    throw new Error("Phase 410 Aurora Ottantotto Resina 800 identity mismatch.");
  }
}
...
async function ensureTopology(client: Client): Promise<void> {
  ...
  try {
    await tx.execute({
      ...
      args: [PHASE410_800_ID, PHASE410_AURORA_ID],
    });
    const maker = await tx.execute({
      sql: "SELECT id,target_id FROM entity_links WHERE source_id=? AND link_type='made_by'",
      args: [PHASE410_800_ID],
    });
    if (maker.rows.length === 0) {
      await tx.execute({
        sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
        args: [
          "phase410-made-by-aurora-ottantotto-resina-800",
          PHASE410_800_ID,
          PHASE410_AURORA_ID,
          "Phase 410 verified Aurora Ottantotto Resina 800 maker relation",
        ],
      });
    } else if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE410_AURORA_ID
    ) {
      throw new Error("Phase 410 Aurora 800 maker relation is ambiguous.");
    }
    ...
    await tx.execute({
      ...
      args: [
        "phase410-reverse-aurora-ottantotto-resina-800",
        PHASE410_AURORA_ID,
        PHASE410_800_ID,
        "Phase 410 Aurora brand navigation to Ottantotto Resina 800",
      ],
    });
    ...
    if (
      ...
      String(makerAfter.rows[0]?.target_id) !== PHASE410_AURORA_ID ||
      reverseAfter.rows.length !== 1
    ) {
      throw new Error("Phase 410 Aurora 800 topology is ambiguous.");
    }
    await tx.commit();
  } catch (error) {
...
async function exactTerminalState(
  ...
  const reverse = await client.execute({
    sql: "SELECT source_id FROM entity_links WHERE source_id=? AND target_id=? AND link_type='reverse'",
    args: [PHASE410_AURORA_ID, pack.entityId],
  });
...
async function republishAuroraBrand(client: Client, reviewer: string): Promise<void> {
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE410_AURORA_ID,
      reviewKind,
      reviewer,
      status: "approved",
      notes: "Phase 410 re-approves the unchanged Aurora brand after normalizing the Ottantotto Resina 800 navigation link.",
    });
  }
  await publishEntity(client, { entityId: PHASE410_AURORA_ID, reviewer });
}

export async function applyPhase410AuroraOttantottoResina800Refresh(
  client: Client,
  options: ApplyPhase410Options,
): Promise<ApplyPhase410Result> {
  await assertAuthority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const pack = loadCuratedEntityPack(
    workspaceRoot,
    phase410AuroraOttantottoResina800RefreshPacks[0]!,
  );
  validatePack(workspaceRoot, pack);
  if (Array.from(pack.bodyMd).length < 8_000) {
    throw new Error("Phase 410 reviewed body must contain at least 8,000 Unicode characters.");
  }
  if (
    pack.sources.length < 10 ||
    pack.variants?.length !== 5 ||
    pack.variants.filter((variant) => variant.variantKind === "edition_group").length !== 1 ||
    pack.variants.filter((variant) => variant.variantKind === "market_sku").length !== 4
  ) {
    throw new Error("Phase 410 pack lacks the expected Aurora 800 source or nib-option coverage.");
  }
  ...
  await republishAuroraBrand(client, options.reviewer);
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase410-aurora-ottantotto-resina-800-refresh.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase410AuroraOttantottoResina800Refresh(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase410-aurora-ottantotto-resina-800-refresh",
