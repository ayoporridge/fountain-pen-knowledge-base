...
import {
  ...
  PHASE411_MODEL20_ID,
  PHASE411_MODEL20_NAME,
  PHASE411_MODEL20_SLUG,
  phase411FranklinChristophModel20RefreshPacks,
} from "./data/phase411-franklin-christoph-model20-refresh";
...
export type ApplyPhase411Options = ApplyPhase22Options;
export type ApplyPhase411Result = ApplyPhase22Result;
...
function rejectRemote(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (env[key]?.trim()) {
      throw new Error(`Phase 411 refuses inherited remote database selection: ${key}.`);
    }
  }
}

async function assertAuthority(client: Client, options: ApplyPhase411Options): Promise<void> {
  rejectRemote(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 411 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 411 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 411 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 411 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 411 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  const result = await client.execute({
    ...
    args: [PHASE411_FC_BRAND_ID, PHASE411_MODEL20_ID],
  });
  const brand = result.rows.find((row) => String(row.id) === PHASE411_FC_BRAND_ID);
  const model = result.rows.find((row) => String(row.id) === PHASE411_MODEL20_ID);
  if (
    !brand ||
    String(brand.type) !== "brand" ||
    String(brand.slug) !== "franklin-christoph" ||
    String(brand.name) !== "Franklin-Christoph" ||
    !model ||
    String(model.type) !== "pen" ||
    String(model.slug) !== pack.expectedSlug ||
    String(model.name) !== PHASE411_MODEL20_NAME
  ) {
    throw new Error("Phase 411 Franklin-Christoph Model 20 identity mismatch.");
  }
}
...
async function ensureTopology(client: Client): Promise<void> {
  ...
  try {
    await tx.execute({
      ...
      args: [PHASE411_MODEL20_ID, PHASE411_FC_BRAND_ID],
    });
    const maker = await tx.execute({
      sql: "SELECT target_id FROM entity_links WHERE source_id=? AND link_type='made_by'",
      args: [PHASE411_MODEL20_ID],
    });
    if (maker.rows.length === 0) {
      await tx.execute({
        sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
        args: [
          "phase411-made-by-franklin-christoph-model20",
          PHASE411_MODEL20_ID,
          PHASE411_FC_BRAND_ID,
          "Phase 411 verified Franklin-Christoph Model 20 Marietta maker relation",
        ],
      });
    } else if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE411_FC_BRAND_ID
    ) {
      throw new Error("Phase 411 Model 20 maker relation is ambiguous.");
    }
    await tx.execute({
    ...
    await tx.execute({
      ...
      args: [
        "phase411-reverse-franklin-christoph-model20",
        PHASE411_FC_BRAND_ID,
        PHASE411_MODEL20_ID,
        "Phase 411 Franklin-Christoph brand navigation to Model 20 Marietta",
      ],
    });
    await tx.commit();
  } catch (error) {
...
async function exactTerminalState(
  ...
  const reverse = await client.execute({
    ...
    args: [PHASE411_FC_BRAND_ID, pack.entityId],
  });
...
async function republishBrand(client: Client, reviewer: string): Promise<void> {
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE411_FC_BRAND_ID,
      reviewKind,
      reviewer,
      status: "approved",
      notes: "Phase 411 re-approves the unchanged Franklin-Christoph brand after normalizing the Model 20 Marietta navigation link.",
    });
  }
  await publishEntity(client, { entityId: PHASE411_FC_BRAND_ID, reviewer });
}

export async function applyPhase411FranklinChristophModel20Refresh(
  client: Client,
  options: ApplyPhase411Options,
): Promise<ApplyPhase411Result> {
  await assertAuthority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const definition = phase411FranklinChristophModel20RefreshPacks[0];
  if (!definition) throw new Error("Phase 411 Model 20 pack definition is missing.");
  ...
  if (Array.from(pack.bodyMd).length < 8_000) {
    throw new Error("Phase 411 reviewed body must contain at least 8,000 Unicode characters.");
  }
  if (
    pack.sources.length < 10 ||
    new Set(pack.sources.map((source) => source.independenceGroup)).size < 10 ||
    pack.variants?.length !== 6 ||
    pack.media.filter((media) => media.usageStatus === "primary").length !== 1
  ) {
    throw new Error("Phase 411 pack lacks expected Model 20 source, variant, or media coverage.");
  }
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase411-franklin-christoph-model20-refresh.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase411FranklinChristophModel20Refresh(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase411-franklin-christoph-model20-refresh",
