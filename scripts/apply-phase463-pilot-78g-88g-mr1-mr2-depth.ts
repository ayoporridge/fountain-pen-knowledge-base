...
import {
  PHASE463_IDS,
  phase463Pilot78g88gMr1Mr2DepthPacks,
} from "./data/phase463-pilot-78g-88g-mr1-mr2-depth";

export type ApplyPhase463Options = ApplyPhase22Options;
export type ApplyPhase463Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase463Options): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 463 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function authority(client: Client, options: ApplyPhase463Options): Promise<void> {
  ...
  if (
    ...
    throw new Error("Phase 463 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 463 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 463 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 463 owned copy must be migrated through 032.");
  }
}

async function identity(client: Client, packs: LoadedCuratedEntityPack[]): Promise<void> {
  const ids = Object.values(PHASE463_IDS);
  if (
    packs.length !== ids.length ||
    new Set(packs.map((pack) => pack.entityId)).size !== ids.length
  ) {
    throw new Error("Phase 463 must load Pilot 78G, 88G MR1 and 88G MR2 exactly once.");
  }
  for (const [label, id] of Object.entries(PHASE463_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack || !pack.expectedSlug) {
      throw new Error(`Phase 463 ${label} pack identity mismatch.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 463 ${label} identity mismatch: ${JSON.stringify(existing)}`);
    }
  }
}
...
async function repairModelNavigation(
  ...
  try {
    for (const pack of packs.filter((candidate) => candidate.expectedType === "pen")) {
      ...
      if (!brandId) throw new Error(`Phase 463 ${pack.expectedSlug} has no maker identity in spec.`);
      const maker = await tx.execute({
        sql: "SELECT count(*) AS n FROM entity_links WHERE source_id=? AND target_id=? AND link_type='made_by'",
        args: [pack.entityId, brandId],
      });
      if (Number(maker.rows[0]?.n) !== 1) {
        throw new Error(`Phase 463 ${pack.expectedSlug} must have exactly one made_by relation.`);
      }
      await tx.execute({
        sql: "INSERT OR IGNORE INTO entity_links (id,source_id,target_id,link_type,reason) VALUES (?,?,?,'reverse',?)",
        args: [
          `phase463-reverse-${pack.entityId}`,
          brandId,
          pack.entityId,
          `Phase 463 model navigation for ${pack.expectedSlug}`,
        ],
      });
      ...
      if (Number(reverse.rows[0]?.n) !== 1) {
        throw new Error(`Phase 463 ${pack.expectedSlug} reverse navigation is ambiguous.`);
      }
    }
    await tx.commit();
  } catch (error) {
...
async function applyPacks(
  client: Client,
  options: ApplyPhase463Options,
  packs: LoadedCuratedEntityPack[],
): Promise<ApplyPhase463Result> {
  ...
  const entities: ApplyPhase463Result["entities"] = [];
  for (const pack of packs) {
    for (const reviewKind of ["fact", "language", "media"] as const) {
      await recordEntityContentReview(client, {
        ...
        status: "approved",
        notes: `${pack.sourceMarker}; ${reviewKind} review of checked-in Pilot 78G/88G MR1/88G MR2 copy.`,
      });
    }
...
export async function applyPhase463Pilot78g88gMr1Mr2Depth(
  client: Client,
  options: ApplyPhase463Options,
): Promise<ApplyPhase463Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 463 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase463Pilot78g88gMr1Mr2DepthPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase463-pilot-78g-88g-mr1-mr2-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase463Pilot78g88gMr1Mr2Depth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase463-pilot-78g-88g-mr1-mr2",
