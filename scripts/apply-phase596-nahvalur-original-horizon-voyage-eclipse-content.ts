...
} from "./data/phase596-nahvalur-original-horizon-voyage-eclipse";
import {
...
export type ApplyPhase596Options = ApplyPhase22Options;
export type ApplyPhase596Result = ApplyPhase22Result;
...
const ACCEPTED_INITIAL_BRAND_SOURCE_PREFIX =
  "curated-content:phase446-nahvalur-brand-depth-v1:";
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of [
    ...
    if (env[key]?.trim()) {
      throw new Error(
        `Phase 596 refuses inherited remote database selection: ${key}.`,
      );
    }
  }
}
...
async function assertOwnedCatalog(
  client: Client,
  options: ApplyPhase596Options,
): Promise<string> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) {
    throw new Error("Phase 596 reviewer must not be empty.");
  }
  ...
  if (
    ...
    throw new Error(
      "Phase 596 requires a non-symlink catalog file inside the caller-owned root.",
    );
  }
  ...
  if (owned.nlink !== BigInt(1)) {
    throw new Error("Phase 596 refuses catalog files with more than one hard link.");
  }
  if (
    ...
    throw new Error(
      "Phase 596 refuses the protected catalog or a hard-link alias.",
    );
  }
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) {
    throw new Error("Phase 596 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 596 owned copy must be migrated through 032.");
  }
  return workspaceRoot;
}

function loadAndValidatePackSet(
  workspaceRoot: string,
): LoadedCuratedEntityPack[] {
  const packs = phase596NahvalurPacks.map((pack) =>
  ...
  if (
    ...
    throw new Error(
      "Phase 596 requires the exact Nahvalur brand plus Original, Horizon, Voyage and Eclipse packs.",
    );
  }
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 596 pack identity mismatch: ${target.id}.`);
    }
  }
  ...
  for (const pack of packs.slice(1)) {
    for (const value of [
      ...
      if (existing && existing !== pack.entityId) {
        throw new Error(`Phase 596 pack-set name or alias collision: ${value}.`);
      }
...
async function assertIdentityPreflight(
  ...
  if (
    !brandPack ||
    brand.length !== 1 ||
    brand[0]?.id !== PHASE596_NAHVALUR_BRAND_ID ||
    brand[0]?.type !== "brand" ||
    ...
    throw new Error(
      `Phase 596 Nahvalur brand identity mismatch: ${JSON.stringify(brand)}.`,
    );
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 596 entity, slug, or name collision: ${target.slug}.`);
    }
    ...
    if (canonicalAliasCollisions.length > 0) {
      throw new Error(
        `Phase 596 canonical name collides with an alias: ${target.name}.`,
      );
    }

    const pack = packs.find((candidate) => candidate.entityId === target.id);
    if (!pack) throw new Error(`Phase 596 missing loaded pack: ${target.id}.`);
    for (const alias of pack.aliases) {
      ...
      if (collisions.length > 0) {
        throw new Error(
          `Phase 596 alias collision for ${alias.alias}: ${JSON.stringify(collisions)}.`,
        );
      }
    }
  }
  ...
  try {
    for (const target of TARGETS) {
      ...
      if (existing.length === 0) {
        ...
        throw new Error(`Phase 596 refuses a conflicting identity: ${target.slug}.`);
      }
      ...
      if (maker.length === 0) {
        await transaction.execute({
          ...
          args: [
            stableId("phase596-made-by", target.id),
            target.id,
            PHASE596_NAHVALUR_BRAND_ID,
            "Phase 596 verified exact Nahvalur maker relation",
          ],
        });
      } else if (
        maker.length !== 1 ||
        maker[0]?.target_id !== PHASE596_NAHVALUR_BRAND_ID
      ) {
        throw new Error(`Phase 596 ${target.slug} maker topology is ambiguous.`);
      }
      ...
      if (reverse.length === 0) {
        await transaction.execute({
          ...
          args: [
            stableId("phase596-reverse", target.id),
            PHASE596_NAHVALUR_BRAND_ID,
            target.id,
            "Phase 596 Nahvalur brand public model navigation",
          ],
        });
      } else if (
        reverse.length !== 1 ||
        reverse[0]?.source_id !== PHASE596_NAHVALUR_BRAND_ID
      ) {
        throw new Error(`Phase 596 ${target.slug} reverse topology is ambiguous.`);
      }
      ...
      if (
        ...
        throw new Error(`Phase 596 publication row is missing: ${target.id}.`);
      }
    }
...
async function protectedNahvalurModelDigests(
  client: Client,
...
async function assertTerminalState(
  ...
  for (const pack of packs) {
    ...
    if (
      ...
      throw new Error(`Phase 596 terminal publication mismatch: ${pack.entityId}.`);
    }
    ...
    if (
      ...
      reviews.some(
        (review, index) =>
          review.review_kind !== expectedReviews[index] ||
          review.status !== "approved",
      )
    ) {
      throw new Error(
        `Phase 596 current-hash approved review set is incomplete: ${pack.entityId}.`,
      );
    }
  }
  ...
  for (const target of TARGETS) {
    ...
    if (
      ...
      throw new Error(`Phase 596 terminal topology mismatch: ${target.id}.`);
    }
  }
}

export async function applyPhase596NahvalurOriginalHorizonVoyageEclipseContent(
  client: Client,
  options: ApplyPhase596Options,
): Promise<ApplyPhase596Result> {
  ...
  const protectedBefore = await protectedNahvalurModelDigests(client);
  ...
  for (const [id, before] of protectedBefore) {
    if ((await entityDigest(client, id)) !== before) {
      throw new Error(`Phase 596 changed protected existing Nahvalur model: ${id}.`);
    }
  }
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return result;
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase596-nahvalur-original-horizon-voyage-eclipse-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result =
      await applyPhase596NahvalurOriginalHorizonVoyageEclipseContent(client, {
        workspaceRoot: process.cwd(),
        reviewer:
          value("--reviewer") ??
          "phase596-nahvalur-original-horizon-voyage-eclipse",
        databasePath,
