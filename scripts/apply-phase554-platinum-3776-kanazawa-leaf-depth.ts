...
} from "./data/phase554-platinum-3776-kanazawa-leaf-depth";

export type ApplyPhase554Options = ApplyPhase22Options;
export type ApplyPhase554Result = ApplyPhase22Result;
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of [
    ...
    if (env[key]?.trim()) {
      throw new Error(`Phase 554 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function assertOwnedCatalog(
  client: Client,
  options: ApplyPhase554Options,
): Promise<void> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 554 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 554 requires a non-symlink catalog inside the owned root.");
  }
  ...
  if (
    ...
    throw new Error("Phase 554 refuses the protected catalog or a hard-link alias.");
  }
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) {
    throw new Error("Phase 554 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 554 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentityPreflight(client: Client): Promise<void> {
  const rows = await client.execute({
    ...
    args: [PHASE554_PNB35000H_ID, PHASE554_PNB35000H_SLUG],
  });
  if (
    rows.rows.length !== 1 ||
    String(rows.rows[0]?.id) !== PHASE554_PNB35000H_ID ||
    String(rows.rows[0]?.type) !== "pen" ||
    String(rows.rows[0]?.slug) !== PHASE554_PNB35000H_SLUG
  ) {
    throw new Error("Phase 554 PNB-35000H identity preflight failed.");
  }
}

export async function applyPhase554PlatinumPnb35000hDepth(
  client: Client,
  options: ApplyPhase554Options,
): Promise<ApplyPhase554Result> {
  await assertOwnedCatalog(client, options);
  await assertIdentityPreflight(client);
  const result = await applyCuratedContentPacks(
    client,
    options,
    structuredClone(phase554PlatinumPnb35000hDepthPacks),
  );
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return result;
}
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase554-platinum-3776-kanazawa-leaf-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase554PlatinumPnb35000hDepth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase554-platinum-pnb35000h-depth",
