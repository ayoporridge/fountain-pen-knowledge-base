...
import {
  ...
  PHASE404_SE_ID,
  PHASE404_SE_SLUG,
  phase404PilotCaplessSeRefreshPacks,
} from "./data/phase404-pilot-capless-se-refresh";
import {
...
export type ApplyPhase404Options = ApplyPhase22Options;
export type ApplyPhase404Result = ApplyPhase22Result;
...
function rejectRemote(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (env[key]?.trim()) {
      throw new Error(`Phase 404 refuses inherited remote database selection: ${key}.`);
    }
  }
}

async function assertAuthority(client: Client, options: ApplyPhase404Options): Promise<void> {
  rejectRemote(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 404 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 404 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 404 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 404 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 404 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  const result = await client.execute({
    ...
    args: [PHASE404_PILOT_ID, PHASE404_SE_ID],
  });
  const brand = result.rows.find((row) => String(row.id) === PHASE404_PILOT_ID);
  const model = result.rows.find((row) => String(row.id) === PHASE404_SE_ID);
  if (
    !brand ||
    ...
    throw new Error("Phase 404 Pilot Capless SE identity mismatch.");
  }
}
...
async function ensureTopology(client: Client): Promise<void> {
  ...
  try {
    await tx.execute({
      ...
      args: [PHASE404_SE_ID, PHASE404_PILOT_ID],
    });
    ...
    if (maker.rows.length === 0) {
      await tx.execute({
        ...
        args: [
          "phase404-made-by-pilot-capless-se",
          PHASE404_SE_ID,
          PHASE404_PILOT_ID,
          "Phase 404 verified Pilot Capless SE maker relation",
        ],
      });
    } else if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE404_PILOT_ID
    ) {
      throw new Error("Phase 404 Pilot Capless SE maker relation is ambiguous.");
    }
    ...
    await tx.execute({
      ...
      args: [
        "phase404-reverse-pilot-capless-se",
        PHASE404_PILOT_ID,
        PHASE404_SE_ID,
        "Phase 404 Pilot brand navigation to Capless SE",
      ],
    });
    ...
    if (
      ...
      String(makerAfter.rows[0]?.target_id) !== PHASE404_PILOT_ID ||
      reverseAfter.rows.length !== 1
    ) {
      throw new Error("Phase 404 Pilot Capless SE topology is ambiguous.");
    }
    await tx.commit();
  } catch (error) {
...
async function exactTerminalState(
  ...
  const reverse = await client.execute({
    sql: "SELECT source_id FROM entity_links WHERE source_id=? AND target_id=? AND link_type='reverse'",
    args: [PHASE404_PILOT_ID, pack.entityId],
  });
...
async function republishPilotBrand(client: Client, reviewer: string): Promise<void> {
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE404_PILOT_ID,
      reviewKind,
      reviewer,
      status: "approved",
      notes: "Phase 404 re-approves the unchanged Pilot brand after normalizing the Capless SE navigation link.",
    });
  }
  await publishEntity(client, { entityId: PHASE404_PILOT_ID, reviewer });
}

export async function applyPhase404PilotCaplessSeRefresh(
  client: Client,
  options: ApplyPhase404Options,
): Promise<ApplyPhase404Result> {
  await assertAuthority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const pack = loadCuratedEntityPack(workspaceRoot, phase404PilotCaplessSeRefreshPacks[0]!);
  validatePack(workspaceRoot, pack);
  if (Array.from(pack.bodyMd).length < 8_000) {
    throw new Error("Phase 404 reviewed body must contain at least 8,000 Unicode characters.");
  }
  if (
    pack.sources.length < 10 ||
    pack.variants?.length !== 11 ||
    pack.variants.filter((variant) => variant.variantKind === "edition_group").length !== 1 ||
    pack.variants.filter((variant) => variant.variantKind === "market_sku").length !== 10
  ) {
    throw new Error("Phase 404 pack lacks the expected source or SKU coverage.");
  }
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase404-pilot-capless-se-refresh.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase404PilotCaplessSeRefresh(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase404-pilot-capless-se-refresh",
