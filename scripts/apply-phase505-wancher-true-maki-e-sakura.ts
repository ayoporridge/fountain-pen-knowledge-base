...
} from "./data/phase505-wancher-true-maki-e-sakura";

export type ApplyPhase505Options = ApplyPhase22Options;
export type ApplyPhase505Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase505Options): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 505 refuses inherited remote database selection: ${key}.`);
    }
  }
}

async function assertAuthority(client: Client, options: ApplyPhase505Options): Promise<void> {
  ...
  if (
    ...
    throw new Error("Phase 505 owned catalog authority check failed.");
  }
  ...
  if (
    ...
    throw new Error("Phase 505 refuses the protected catalog and hard-link aliases.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 505 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 505 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  const brand = await client.execute({
    ...
    args: [PHASE505_WANCHER_BRAND_ID],
  });
  if (
    brand.rows.length !== 1 ||
    ...
    throw new Error("Phase 505 Wancher brand identity is not the approved canonical row.");
  }
  ...
  if (
    ...
    throw new Error(`Phase 505 Wancher True Maki-e Sakura identity collision: ${pack.entityId}.`);
  }
}
...
async function prepareTopology(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  ...
  try {
    ...
    if (existing.rows.length === 0) {
      ...
      throw new Error("Phase 505 refuses a conflicting Wancher True Maki-e Sakura identity.");
    }
    ...
    await transaction.execute({
      ...
      args: [
        "phase505-made-by-wancher-true-maki-e-sakura",
        PHASE505_TRUE_MAKI_E_SAKURA_ID,
        PHASE505_WANCHER_BRAND_ID,
        "Phase 505 verified Wancher True Maki-e Sakura maker relation",
      ],
    });
    await transaction.execute({
      sql: `INSERT OR IGNORE INTO entity_links
      ...
      args: [
        "phase505-reverse-wancher-true-maki-e-sakura",
        PHASE505_WANCHER_BRAND_ID,
        PHASE505_TRUE_MAKI_E_SAKURA_ID,
        "Phase 505 Wancher brand navigation to True Maki-e Sakura",
      ],
    });
    ...
    if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE505_WANCHER_BRAND_ID
    ) {
      throw new Error("Phase 505 Wancher True Maki-e Sakura maker topology is ambiguous.");
    }
    ...
    if (
      ...
      throw new Error("Phase 505 expects a draft or already-published publication row.");
    }
...
async function refreshWancherBrandPublication(
  client: Client,
  reviewer: string,
): Promise<void> {
  ...
  const contentHash = await computePublicationContentHash(client, PHASE505_WANCHER_BRAND_ID);
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE505_WANCHER_BRAND_ID,
      reviewKind,
      reviewer,
      status: "approved",
      notes: `Phase 505 ${reviewKind} review after adding the Wancher True Maki-e Sakura reverse navigation.`,
      contentHash,
    });
  }
...
async function applySinglePack(
  client: Client,
  options: ApplyPhase505Options,
  pack: LoadedCuratedEntityPack,
): Promise<ApplyPhase505Result> {
  ...
  if ((pack.publicationIntent ?? "publish") !== "publish") {
    throw new Error("Phase 505 only supports a publish-intent pack.");
  }
  const transaction = await client.transaction("write");
  try {
  ...
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      ...
      status: "approved",
      notes: `${pack.sourceMarker}; ${reviewKind} review of the checked-in Wancher True Maki-e Sakura copy.`,
    });
  }
...
export async function applyPhase505WancherTrueMakiESakura(
  client: Client,
  options: ApplyPhase505Options,
): Promise<ApplyPhase505Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 505 reviewer must not be empty.");
  await assertAuthority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const rawPack = phase505WancherTrueMakiESakuraPacks[0];
  if (!rawPack) throw new Error("Phase 505 Wancher True Maki-e Sakura pack is missing.");
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase505-wancher-true-maki-e-sakura.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase505WancherTrueMakiESakura(client, {
      workspaceRoot: process.cwd(),
      reviewer: cliValue("--reviewer") ?? "phase505-wancher-true-maki-e-sakura",
