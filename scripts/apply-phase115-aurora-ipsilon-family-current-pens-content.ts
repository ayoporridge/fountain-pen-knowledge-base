...
import {
  PHASE115_AURORA_BRAND_ID, PHASE115_OPTIMA_ID, PHASE115_AURORA_88_FAMILY_ID,
  PHASE115_RESINA_800_ID, PHASE115_FAMILY_ID, PHASE115_FAMILY_SLUG,
  PHASE115_FAMILY_NAME, PHASE115_FAMILY_SVG, PHASE115_DEMO_ID,
  PHASE115_DEMO_SLUG, PHASE115_DEMO_NAME, PHASE115_RESIN_ID,
  PHASE115_RESIN_SLUG, PHASE115_RESIN_NAME, PHASE115_RESIN_OFFICIAL_URL,
  PHASE115_DEMO_CATEGORY_URL, PHASE115_DEMO_PDF_URL,
  loadPhase115AuroraIpsilonDemoColorsPack, loadPhase115AuroraIpsilonResinB11NPack,
  phase115AuroraIpsilonFamilyArticle,
} from "./data/phase115-aurora-ipsilon-family-current-pens";
import { curatedId, packId, type CuratedSource, type LoadedCuratedEntityPack } from "./lib/curated-content-pack";

export type ApplyPhase115Options = ApplyPhase22Options;
export type ApplyPhase115Result = ApplyPhase22Result;
export { PHASE115_FAMILY_ID, PHASE115_FAMILY_SLUG, PHASE115_DEMO_ID, PHASE115_DEMO_SLUG, PHASE115_RESIN_ID, PHASE115_RESIN_SLUG };
...
const FAMILY_MARKDOWN = ".planning/content-research/aurora-ipsilon-family-phase115.md";
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) if (env[key]?.trim()) throw new Error(`Phase 115 refuses inherited remote database selection: ${key}.`);
}
function assertVerifiedRepoPair(input: string): string {
  if (!ALLOWED_REPO_INPUTS.has(input)) throw new Error("Phase 115 requires the verified CodeBuddy/Documents repo pair.");
  ...
  catch { throw new Error("Phase 115 could not resolve the verified CodeBuddy/Documents repo pair."); }
  if (real !== CANONICAL_REPO || gitRoot !== CANONICAL_REPO) throw new Error("Phase 115 verified CodeBuddy/Documents repo pair must resolve to one canonical git root.");
  return real;
}
async function assertAuthority(client: Client, options: ApplyPhase115Options): Promise<string> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 115 reviewer must not be empty.");
  const workspaceRoot = assertVerifiedRepoPair(options.workspaceRoot);
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  if (fs.lstatSync(options.databasePath).isSymbolicLink()) throw new Error("Phase 115 caller-owned database must not be a symlink.");
  const ownedRoot = fs.realpathSync.native(options.ownedRoot);
  const databasePath = fs.realpathSync.native(options.databasePath);
  const protectedPath = fs.realpathSync.native(options.protectedCatalogPath);
  if (!fs.statSync(ownedRoot).isDirectory() || !fs.statSync(databasePath).isFile() || !isInside(databasePath, ownedRoot)) throw new Error("Phase 115 requires a regular database inside the caller-owned root.");
  const protectedNames = new Set([protectedPath, `${protectedPath}-wal`, `${protectedPath}-shm`]);
  if (protectedNames.has(databasePath)) throw new Error("Phase 115 refuses the protected catalog or sidecar path.");
  const owned = fs.statSync(databasePath, { bigint: true }); const protectedCatalog = fs.statSync(protectedPath, { bigint: true });
  if (owned.dev === protectedCatalog.dev && owned.ino === protectedCatalog.ino) throw new Error("Phase 115 refuses a hard-link alias of the protected catalog.");
  const listed = await client.execute("PRAGMA database_list"); const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) throw new Error("Phase 115 client/path mismatch for caller-owned database.");
  ...
  if (migration.rows.length !== 1) throw new Error("Phase 115 owned copy must be migrated through 032.");
...
function loadFamilyCopy(workspaceRoot: string) {
  ...
  if (!summary || !bodyMd) throw new Error("Phase 115 family reviewed copy is incomplete.");
  const summaryLength = Array.from(summary).length;
  if (summaryLength < 60 || summaryLength > 160 || Array.from(bodyMd).length < 2_000) {
    throw new Error("Phase 115 family reviewed copy violates summary/body length contract.");
  }
  if (
    !bodyMd.includes(`/pen/${PHASE115_DEMO_SLUG}`) ||
    !bodyMd.includes(`/pen/${PHASE115_RESIN_SLUG}`)
  ) {
    throw new Error("Phase 115 family reviewed copy is missing three-way navigation.");
  }
  const digest = createHash("sha256").update(`${summary}\0${bodyMd}`).digest("hex");
  return {
    summary,
    bodyMd,
    sourceMarker: `${phase115AuroraIpsilonFamilyArticle.sourceMarkerPrefix}${digest}`,
  };
}
...
async function upsertPhase115Sources(
  ...
  for (const source of pack.sources) {
    await transaction.execute({
      ...
      args: [
        ...
        source.author ?? null,
        source.homepageUrl,
        `Phase 115 curated source registry: ${source.registryKey}`,
        source.retrievedAt,
        source.tier,
...
async function installFamilyArticle(
  ...
  try {
    await upsertPhase115Sources(transaction, phase115AuroraIpsilonFamilyArticle);
    ...
    if (updated.rowsAffected !== 1) {
      throw new Error("Phase 115 family identity changed before article payload install.");
    }
    await transaction.execute({
      ...
      args: [
        stableId("phase115-family-story", PHASE115_FAMILY_ID),
        PHASE115_FAMILY_ID,
        phase115AuroraIpsilonFamilyArticle.storyTitle,
        copy.summary,
        copy.bodyMd,
        copy.sourceMarker,
      ],
    });
    for (const source of phase115AuroraIpsilonFamilyArticle.sources) {
      await transaction.execute({
        sql: `INSERT INTO entity_references(
                id,entity_id,source_item_id,relation_type,note,review_status
              ) VALUES(?,?,?,?,?,'approved')`,
        args: [
          stableId("phase115-family-reference", source.key),
          PHASE115_FAMILY_ID,
          sourceItemId(source.key),
    ...
    const image = phase115AuroraIpsilonFamilyArticle.primaryImage;
    await transaction.execute({
      ...
      args: [
        stableId("phase115-family-media", PHASE115_FAMILY_ID),
        PHASE115_FAMILY_ID,
        image.title,
        PHASE115_FAMILY_SVG,
        image.author ?? null,
        image.license ?? null,
        "本站原创 Aurora Ipsilon sibling family navigation；非产品照片、logo、比例、颜色或饰面证明。",
        PHASE115_FAMILY_SVG,
...
async function installPhase115Pack(
  client: Client,
  pack: LoadedCuratedEntityPack,
): Promise<void> {
  const transaction = await client.transaction("write");
  try {
    await upsertPhase115Sources(transaction, pack);
    const updated = await transaction.execute({
    ...
    if (updated.rowsAffected !== 1) throw new Error("Phase 115 target identity changed before payload install.");
    ...
    if (publication.rowsAffected !== 1) throw new Error("Phase 115 target publication row is missing.");
    ...
    for (const alias of pack.aliases) {
      ...
      if (!source) throw new Error(`Phase 115 alias source missing: ${alias.sourceKey}`);
      await transaction.execute({
    ...
    for (const claim of pack.claims) {
      ...
      for (const item of claim.evidence) {
        ...
        if (!scopeId) throw new Error(`Phase 115 claim scope missing: ${item.scopeKey}`);
    ...
    if (!pack.spec) throw new Error("Phase 115 target pack requires model specs.");
    ...
    for (const item of pack.spec.evidence) {
      const scopeId = scopeIds.get(item.scopeKey);
      if (!scopeId) throw new Error(`Phase 115 spec scope missing: ${item.scopeKey}`);
    ...
    for (const conflict of pack.conflicts ?? []) {
      ...
      for (const member of conflict.members) {
        ...
        if (!citationId) throw new Error(`Phase 115 conflict citation missing: ${member.citationKey}`);
        await transaction.execute({
...
async function assertBaseline(client: Client): Promise<void> {
  const result = await client.execute({
    sql: `SELECT entity.id,entity.type,entity.slug,entity.source,publication.status,
          ...
          WHERE entity.id IN (?,?,?,?)`,
    args: [PHASE115_AURORA_BRAND_ID, PHASE115_OPTIMA_ID, PHASE115_AURORA_88_FAMILY_ID, PHASE115_RESINA_800_ID],
  });
  const byId = new Map(result.rows.map((row) => [String(row.id), row]));
  const expected = [
    [PHASE115_AURORA_BRAND_ID, "brand", "aurora", BRAND_MARKER_PREFIX],
    [PHASE115_OPTIMA_ID, "pen", "aurora-optima", OPTIMA_MARKER_PREFIX],
    [PHASE115_AURORA_88_FAMILY_ID, "article", "aurora-88", PHASE114_FAMILY_MARKER_PREFIX],
    [PHASE115_RESINA_800_ID, "pen", "aurora-ottantotto-resina-800", PHASE114_RESINA_MARKER_PREFIX],
  ] as const;
  for (const [id, type, slug, marker] of expected) {
    const row = byId.get(id);
    if (!row || row.type !== type || row.slug !== slug || Number(row.is_public) !== 1 || !String(row.source).startsWith(marker) || ((type === "brand" || type === "pen") && row.status !== "published")) {
      throw new Error(`Phase 115 requires exact Phase 114 terminal baseline for ${id}.`);
    }
  }
}
async function inspectTargets(client: Client, markers: [string, string, string]): Promise<"absent" | "terminal"> {
  const generic = await client.execute({
    sql: `SELECT DISTINCT entity.id,entity.type,entity.slug,entity.name
          ...
          WHERE entity.id NOT IN (?,?,?) AND (
            lower(entity.name) IN (lower('Ipsilon'),lower('Aurora Ipsilon'),lower('Demo Colors'),lower('B11-N'))
            OR lower(COALESCE(alias.alias,'')) IN (lower('Ipsilon'),lower('Aurora Ipsilon'),lower('Demo Colors'),lower('B11-N'))
            OR lower(entity.slug) IN ('aurora-ipsilon','aurora-ipsilon-demo-colors','aurora-ipsilon-resin-b11-n')
          ) ORDER BY entity.id`,
    args: [PHASE115_FAMILY_ID, PHASE115_DEMO_ID, PHASE115_RESIN_ID],
  });
  if (generic.rows.length) throw new Error(`Phase 115 found generic or alternate Ipsilon owner: ${JSON.stringify(generic.rows)}`);
  const urls = [PHASE115_DEMO_CATEGORY_URL, PHASE115_DEMO_PDF_URL, PHASE115_RESIN_OFFICIAL_URL];
  const urlOwners = await client.execute({
    sql: `SELECT DISTINCT reference.entity_id,item.url FROM entity_references reference
          ...
          WHERE item.url IN (?,?,?) AND reference.entity_id NOT IN (?,?,?)`,
    args: [...urls, PHASE115_FAMILY_ID, PHASE115_DEMO_ID, PHASE115_RESIN_ID],
  });
  if (urlOwners.rows.length) throw new Error(`Phase 115 found alternate official source owner: ${JSON.stringify(urlOwners.rows)}`);
  const routes = await client.execute({ sql: "SELECT source_path,target_path FROM entity_redirects WHERE source_path IN ('/pen/aurora-ipsilon','/article/aurora-ipsilon','/pen/aurora-ipsilon-demo-colors','/pen/aurora-ipsilon-resin-b11-n')", args: [] });
  if (routes.rows.length) throw new Error(`Phase 115 route collision: ${JSON.stringify(routes.rows)}`);
  const rows = await client.execute({ sql: "SELECT id,type,slug,name,source FROM entities WHERE id IN (?,?,?) OR slug IN (?,?,?) ORDER BY id", args: [PHASE115_FAMILY_ID, PHASE115_DEMO_ID, PHASE115_RESIN_ID, PHASE115_FAMILY_SLUG, PHASE115_DEMO_SLUG, PHASE115_RESIN_SLUG] });
  if (rows.rows.length === 0) return "absent";
  if (rows.rows.length !== 3) throw new Error("Phase 115 partial terminal identity is invalid; automatic repair is forbidden.");
  const byId = new Map(rows.rows.map((row) => [String(row.id), row]));
  const expected = [[PHASE115_FAMILY_ID, "article", PHASE115_FAMILY_SLUG, PHASE115_FAMILY_NAME, markers[0]], [PHASE115_DEMO_ID, "pen", PHASE115_DEMO_SLUG, PHASE115_DEMO_NAME, markers[1]], [PHASE115_RESIN_ID, "pen", PHASE115_RESIN_SLUG, PHASE115_RESIN_NAME, markers[2]]] as const;
  for (const [id, type, slug, name, marker] of expected) {
    const row = byId.get(id);
    if (!row || row.type !== type || row.slug !== slug || row.name !== name || row.source !== marker) throw new Error("Phase 115 terminal identity/source marker is invalid; automatic repair is forbidden.");
  }
  return "terminal";
}
...
async function assertTerminal(client: Client, markers: [string, string, string]): Promise<string[]> {
  ...
  if (identities.rows.length !== 3) throw new Error("Phase 115 terminal identities missing.");
  const byId = new Map(identities.rows.map((row) => [String(row.id), row]));
  const family = byId.get(PHASE115_FAMILY_ID);
  if (!family || family.type !== "article" || family.slug !== PHASE115_FAMILY_SLUG || family.source !== markers[0] || Number(family.is_public) !== 1 || family.status != null || Array.from(String(family.summary)).length < 60 || Array.from(String(family.body_md)).length < 2000 || !String(family.body_md).includes(`/pen/${PHASE115_DEMO_SLUG}`) || !String(family.body_md).includes(`/pen/${PHASE115_RESIN_SLUG}`)) throw new Error("Phase 115 terminal family content is invalid; automatic repair is forbidden.");
  const familyCounts = (await client.execute({ sql: `SELECT
    ...
    (SELECT count(*) FROM media_assets WHERE entity_id=? AND usage_status='primary' AND review_status='approved') AS media`, args: [PHASE115_FAMILY_ID, PHASE115_FAMILY_ID, PHASE115_FAMILY_ID, PHASE115_FAMILY_ID, PHASE115_FAMILY_ID, PHASE115_FAMILY_ID] })).rows[0];
  if (Number(familyCounts?.publications) || Number(familyCounts?.topology) || Number(familyCounts?.specs) || Number(familyCounts?.variants) || Number(familyCounts?.media) !== 1) throw new Error("Phase 115 article retained pen-only payload or lost primary media.");
  const hashes: string[] = [];
  for (const [id, slug, marker, sibling] of [[PHASE115_DEMO_ID, PHASE115_DEMO_SLUG, markers[1], PHASE115_RESIN_SLUG], [PHASE115_RESIN_ID, PHASE115_RESIN_SLUG, markers[2], PHASE115_DEMO_SLUG]] as const) {
    const row = byId.get(id); const hash = await computePublicationContentHash(client, id); hashes.push(hash);
    if (!row || row.type !== "pen" || row.slug !== slug || row.source !== marker || row.status !== "published" || row.approved_content_hash !== hash || Number(row.is_public) !== 1 || Array.from(String(row.summary)).length < 60 || Array.from(String(row.body_md)).length < 2000 || !String(row.body_md).includes("/article/aurora-ipsilon") || !String(row.body_md).includes(`/pen/${sibling}`)) throw new Error("Phase 115 terminal pen content/publication is invalid; automatic repair is forbidden.");
    ...
    if (links.rows.length !== 2 || links.rows[0]?.source_id !== id || links.rows[0]?.target_id !== PHASE115_AURORA_BRAND_ID || links.rows[0]?.link_type !== "made_by" || links.rows[1]?.source_id !== PHASE115_AURORA_BRAND_ID || links.rows[1]?.target_id !== id || links.rows[1]?.link_type !== "reverse") throw new Error("Phase 115 terminal maker topology is invalid; automatic repair is forbidden.");
    ...
    const expectedScopes = id === PHASE115_DEMO_ID ? 2 : 3; const expectedClaims = id === PHASE115_DEMO_ID ? 2 : 3;
    if (Number(counts?.scopes) !== expectedScopes || Number(counts?.claims) !== expectedClaims || Number(counts?.rejected) < 2 || Number(counts?.media) !== 1 || Number(counts?.reviews) !== 4) throw new Error("Phase 115 terminal scope/evidence/media/reviews invalid; automatic repair is forbidden.");
  }
  const brandHash = await computePublicationContentHash(client, PHASE115_AURORA_BRAND_ID);
  const brand = (await client.execute({ sql: "SELECT status,approved_content_hash FROM entity_publications WHERE entity_id=?", args: [PHASE115_AURORA_BRAND_ID] })).rows[0];
  if (brand?.status !== "published" || brand.approved_content_hash !== brandHash) throw new Error("Phase 115 Aurora post-topology publication invalid; automatic repair is forbidden.");
  return hashes;
}
export async function applyPhase115AuroraIpsilonFamilyCurrentPensContent(client: Client, options: ApplyPhase115Options): Promise<ApplyPhase115Result> {
  const workspaceRoot = await assertAuthority(client, options);
  const familyCopy = loadFamilyCopy(workspaceRoot);
  const demo = loadPhase115AuroraIpsilonDemoColorsPack(workspaceRoot);
  const resin = loadPhase115AuroraIpsilonResinB11NPack(workspaceRoot);
  const markers: [string,string,string] = [familyCopy.sourceMarker, demo.sourceMarker, resin.sourceMarker];
  await assertBaseline(client);
  const state = await inspectTargets(client, markers);
  if (state === "terminal") {
    const hashes = await assertTerminal(client, markers);
    assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
    return { entities: [{ entityId: PHASE115_FAMILY_ID, outcome: "noop", contentHash: hashes[0] }, { entityId: PHASE115_DEMO_ID, outcome: "noop", contentHash: hashes[0] }, { entityId: PHASE115_RESIN_ID, outcome: "noop", contentHash: hashes[1] }] };
  }
  const protectedIds = [PHASE115_OPTIMA_ID, PHASE115_AURORA_88_FAMILY_ID, PHASE115_RESINA_800_ID];
  ...
  const brandPayloadBefore = await payloadDigest(client, PHASE115_AURORA_BRAND_ID, false);
  const brandHashBefore = await computePublicationContentHash(client, PHASE115_AURORA_BRAND_ID);
  const reverseBefore = await jsonRows(client, "SELECT source_id,target_id,link_type FROM entity_links WHERE source_id=? AND link_type='reverse' ORDER BY target_id", [PHASE115_AURORA_BRAND_ID]);
  const tx = await client.transaction("write");
  try {
    await tx.execute({ sql: "INSERT INTO entities(id,type,slug,name) VALUES(?,'article',?,?)", args: [PHASE115_FAMILY_ID, PHASE115_FAMILY_SLUG, PHASE115_FAMILY_NAME] });
    for (const [id,slug,name] of [[PHASE115_DEMO_ID,PHASE115_DEMO_SLUG,PHASE115_DEMO_NAME],[PHASE115_RESIN_ID,PHASE115_RESIN_SLUG,PHASE115_RESIN_NAME]] as const) {
      await tx.execute({ sql: "INSERT INTO entities(id,type,slug,name) VALUES(?,'pen',?,?)", args: [id,slug,name] });
      await tx.execute({ sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)", args: [stableId("phase115-made-by", `${id}:${PHASE115_AURORA_BRAND_ID}`), id, PHASE115_AURORA_BRAND_ID, "Phase 115 exact Ipsilon maker relation"] });
      await tx.execute({ sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)", args: [stableId("phase115-reverse", `${PHASE115_AURORA_BRAND_ID}:${id}`), PHASE115_AURORA_BRAND_ID, id, "Phase 115 Aurora-to-Ipsilon public navigation"] });
    }
    await tx.commit();
  } catch (error) { if (!tx.closed) await tx.rollback(); throw error; }
  if (await payloadDigest(client, PHASE115_AURORA_BRAND_ID, false) !== brandPayloadBefore) throw new Error("Phase 115 changed Aurora non-topology payload.");
  const reverseAfter = await jsonRows(client, "SELECT source_id,target_id,link_type FROM entity_links WHERE source_id=? AND link_type='reverse' ORDER BY target_id", [PHASE115_AURORA_BRAND_ID]);
  const additions = [{source_id:PHASE115_AURORA_BRAND_ID,target_id:PHASE115_DEMO_ID,link_type:"reverse"},{source_id:PHASE115_AURORA_BRAND_ID,target_id:PHASE115_RESIN_ID,link_type:"reverse"}];
  const expectedReverse = reverseBefore.concat(additions).sort((a,b)=>String(a.target_id).localeCompare(String(b.target_id)));
  if (JSON.stringify(reverseAfter) !== JSON.stringify(expectedReverse)) throw new Error("Phase 115 Aurora topology delta is not exactly two new reverse links.");
  const brandHashAfter = await computePublicationContentHash(client, PHASE115_AURORA_BRAND_ID);
  if (brandHashAfter === brandHashBefore) throw new Error("Phase 115 expected Aurora topology hash change.");
  await reviewAndPublish(client, PHASE115_AURORA_BRAND_ID, options.reviewer.trim(), "Phase 115 exact post-topology current hash; Phase 48/114 brand packs not replayed.");
  await installFamilyArticle(client, familyCopy);
  await installPhase115Pack(client, demo); await installPhase115Pack(client, resin);
  const demoHash = await reviewAndPublish(client, PHASE115_DEMO_ID, options.reviewer.trim(), `${demo.sourceMarker}; Phase 115 checked-in sourced copy.`);
  const resinHash = await reviewAndPublish(client, PHASE115_RESIN_ID, options.reviewer.trim(), `${resin.sourceMarker}; Phase 115 checked-in sourced copy.`);
  await assertTerminal(client, markers);
  for (const id of protectedIds) if (await payloadDigest(client,id,true) !== protectedBefore.get(id)) throw new Error(`Phase 115 changed protected entity ${id}.`);
  if (await payloadDigest(client,PHASE115_AURORA_BRAND_ID,false) !== brandPayloadBefore) throw new Error("Phase 115 replayed or changed Aurora non-topology payload.");
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return { entities: [{entityId:PHASE115_FAMILY_ID,outcome:"published",contentHash:demoHash},{entityId:PHASE115_DEMO_ID,outcome:"published",contentHash:demoHash},{entityId:PHASE115_RESIN_ID,outcome:"published",contentHash:resinHash}] };
}
...
async function main(): Promise<void> {
  ...
  const reviewer=value("--reviewer") ?? "phase115-aurora-ipsilon-family-current-pens";
  if (!databasePath || !ownedRoot || !protectedCatalogPath) throw new Error("Usage: tsx scripts/apply-phase115-aurora-ipsilon-family-current-pens-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]");
  const client=createClient({url:`file:${path.resolve(databasePath)}`});
  try { process.stdout.write(`${JSON.stringify(await applyPhase115AuroraIpsilonFamilyCurrentPensContent(client,{workspaceRoot:process.cwd(),reviewer,databasePath:path.resolve(databasePath),ownedRoot:path.resolve(ownedRoot),protectedCatalogPath:path.resolve(protectedCatalogPath),protectedCatalogSnapshot:snapshotCatalogFiles(path.resolve(protectedCatalogPath)),env:process.env}),null,2)}\n`); } finally { client.close(); }
}
