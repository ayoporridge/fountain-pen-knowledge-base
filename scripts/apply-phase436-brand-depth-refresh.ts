...
import {
  PHASE436_BRAND_IDS,
  phase436BrandDepthRefreshPacks,
} from "./data/phase436-brand-depth-refresh";

export type ApplyPhase436Options = ApplyPhase22Options;
export type ApplyPhase436Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase436Options): void {
  for (const key of [
    "TURSO_DATABASE_URL",
    "TURSO_AUTH_TOKEN",
    "FPKG_DATABASE_URL",
    ...
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 436 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function authority(
  ...
  if (
    ...
    throw new Error("Phase 436 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 436 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 436 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 436 owned copy must be migrated through 032.");
  }
}
...
async function identity(
  ...
  if (packs.length !== 5 || new Set(packs.map((pack) => pack.entityId)).size !== 5) {
    throw new Error("Phase 436 must load five unique brand packs.");
  }
  for (const [label, id] of Object.entries(PHASE436_BRAND_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack || pack.expectedType !== "brand" || pack.expectedSlug !== label) {
      throw new Error(`Phase 436 ${label} pack identity mismatch.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 436 ${label} brand identity mismatch: ${JSON.stringify(existing)}`);
    }
  }
}
...
async function repairBrandNavigation(client: Client): Promise<void> {
  ...
  try {
    for (const [label, brandId] of Object.entries(PHASE436_BRAND_IDS)) {
      ...
      for (const row of models.rows) {
        ...
        await tx.execute({
          ...
          args: [
            `phase436-reverse-${label}-${modelId}`,
            brandId,
            modelId,
            `Phase 436 ${label} brand navigation for published pen ${modelId}`,
          ],
        });
        ...
        if (Number(reverse.rows[0]?.n) !== 1) {
          throw new Error(`Phase 436 ${label} reverse navigation is ambiguous for ${modelId}.`);
        }
      }
    }
    await tx.commit();
...
async function applyBrandPacks(
  ...
  const entities: ApplyPhase436Result["entities"] = [];
...
export async function applyPhase436BrandDepthRefresh(
  client: Client,
  options: ApplyPhase436Options,
): Promise<ApplyPhase436Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 436 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase436BrandDepthRefreshPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase436-brand-depth-refresh.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase436BrandDepthRefresh(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase436-brand-depth-refresh",
