...
import {
  PHASE119_AKA_TAMENURI_ID,
  PHASE119_COLLECTION_URL,
  PHASE119_DREAM_ARTICLE_ID,
  PHASE119_KIMBERLY_URL,
  PHASE119_PUCHICO_ID,
  PHASE119_PUCHICO_SLUG,
  PHASE119_SARAH_URL,
  PHASE119_TITANIUM_BLACK_ID,
  PHASE119_TRUE_EBONITE_ID,
  PHASE119_VARIANT_NAMES,
  PHASE119_WHITE_SNOW_URL,
  PHASE119_WANCHER_ID,
  loadPhase119WancherPuchicoPack,
  phase119WancherPuchicoPack,
} from "./data/phase119-wancher-puchico";
import {
  curatedId,
  packId,
  type CuratedSource,
...
export type ApplyPhase119Options = ApplyPhase22Options;
export type ApplyPhase119Result = ApplyPhase22Result;
...
function rejectRemoteSelection(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (env[key]?.trim()) throw new Error(`Phase 119 refuses inherited remote database selection: ${key}.`);
  }
}

function assertVerifiedRepoPair(input: string): string {
  if (!ALLOWED_REPO_INPUTS.has(input)) {
    throw new Error("Phase 119 requires the verified CodeBuddy/Documents repo pair.");
  }
  ...
  try {
    ...
    throw new Error("Phase 119 could not resolve the verified CodeBuddy/Documents repo pair.");
  }
  if (real !== CANONICAL_REPO || gitRoot !== CANONICAL_REPO) {
    throw new Error("Phase 119 verified CodeBuddy/Documents repo pair must resolve to one canonical git root.");
  }
  return real;
}

async function assertAuthority(client: Client, options: ApplyPhase119Options): Promise<string> {
  rejectRemoteSelection(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 119 reviewer must not be empty.");
  ...
  if (fs.lstatSync(options.databasePath).isSymbolicLink()) {
    throw new Error("Phase 119 caller-owned database must not be a symlink.");
  }
  ...
  if (!fs.statSync(ownedRoot).isDirectory() || !fs.statSync(databasePath).isFile() || !isInside(databasePath, ownedRoot)) {
    throw new Error("Phase 119 requires a regular database inside the caller-owned root.");
  }
  const protectedNames = new Set([protectedPath, `${protectedPath}-wal`, `${protectedPath}-shm`]);
  if (protectedNames.has(databasePath)) throw new Error("Phase 119 refuses the protected catalog or sidecar path.");
  ...
  if (owned.dev === protectedCatalog.dev && owned.ino === protectedCatalog.ino) {
    throw new Error("Phase 119 refuses a hard-link alias of the protected catalog.");
  }
  ...
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== databasePath) {
    throw new Error("Phase 119 client/path mismatch for caller-owned database.");
  }
  const migration = await client.execute({
    sql: "SELECT 1 FROM migrations WHERE name=? AND checksum IS NOT NULL",
    args: ["032_taxonomy_identity.sql"],
  });
  if (migration.rows.length !== 1) throw new Error("Phase 119 owned copy must be migrated through 032.");
...
async function assertBaseline(client: Client): Promise<void> {
  ...
  const brand = byId.get(PHASE119_WANCHER_ID);
  const article = byId.get(PHASE119_DREAM_ARTICLE_ID);
  const trueEbonite = byId.get(PHASE119_TRUE_EBONITE_ID);
  const titaniumBlack = byId.get(PHASE119_TITANIUM_BLACK_ID);
  const akaTamenuri = byId.get(PHASE119_AKA_TAMENURI_ID);
  if (!brand || brand.type !== "brand" || brand.slug !== "wancher" || brand.status !== "published" || Number(brand.is_public) !== 1 || !String(brand.source).startsWith(BRAND_MARKER_PREFIX)) {
    throw new Error("Phase 119 requires the exact published Phase 107 Wancher brand baseline.");
  }
  if (!article || article.type !== "article" || article.slug !== ARTICLE_SLUG || Number(article.is_public) !== 1) {
    throw new Error("Phase 119 requires the exact published Phase 104 Dream Pen article baseline.");
  }
  if (!trueEbonite || trueEbonite.type !== "pen" || trueEbonite.slug !== "wancher-dream-pen-true-ebonite-matte-black" || trueEbonite.status !== "published" || Number(trueEbonite.is_public) !== 1 || !String(trueEbonite.source).startsWith(TRUE_EBONITE_MARKER_PREFIX)) {
    throw new Error("Phase 119 requires the exact published Phase 107 True Ebonite baseline.");
  }
  if (
    !titaniumBlack ||
    ...
    throw new Error("Phase 119 requires the exact published Phase 112 Titanium Black baseline.");
  }
  if (
    !akaTamenuri ||
    ...
    throw new Error("Phase 119 requires the exact published Phase 113 Aka Tamenuri baseline.");
  }
  const action = await client.execute({
    ...
    args: [PHASE119_DREAM_ARTICLE_ID, PHASE119_DREAM_ARTICLE_ID, "wancher万佳-dream-pen"],
  });
  if (action.rows.length !== 1 || action.rows[0]?.action_kind !== "rename" || action.rows[0]?.status !== "applied") {
    throw new Error("Phase 119 requires the applied Phase 104 route reclassification.");
  }
}
...
async function inspectTarget(client: Client, sourceMarker: string): Promise<"absent" | "terminal"> {
  const packFiles = fs
    ...
    .filter((name) => /puchico/i.test(name) && name !== "phase119-wancher-puchico.ts");
  if (packFiles.length > 0) {
    throw new Error(`Phase 119 found an existing PuChiCo content pack: ${packFiles.join(", ")}`);
  }
  const collisions = await client.execute({
    sql: `SELECT DISTINCT entity.id,entity.type,entity.slug,entity.name,entity.source
          ...
          WHERE entity.id=? OR entity.slug=?
             OR lower(entity.name) IN (lower('Wancher PuChiCo'),lower('PuChiCo'),lower('Wancher PuChico'))
             OR lower(COALESCE(alias.alias,'')) IN (lower('Wancher PuChiCo'),lower('PuChiCo'),lower('Wancher PuChico'))
             OR entity.source_url IN (?,?,?)
             OR (item.url IN (?,?,?,?) AND entity.type='pen')
             OR (entity.source LIKE 'curated-content:phase119-wancher-puchico-v1:%')
          ORDER BY entity.id`,
    args: [
      PHASE119_PUCHICO_ID,
      PHASE119_PUCHICO_SLUG,
  ...
  const expectedSourceItems = [
    ["phase119-wancher-puchico-official-collection", PHASE119_COLLECTION_URL],
    ["phase119-pen-addict-sarah-read-2024", PHASE119_SARAH_URL],
    ["phase119-pen-addict-kimberly-lau-2025", PHASE119_KIMBERLY_URL],
    [
      "phase119-wancher-puchico-boundary-svg",
  ...
  if (collisions.rows.length === 0) {
    if (sourceItems.rows.length > 0) {
      throw new Error(
        "Phase 119 source marker collision exists without the locked target; repair is forbidden.",
      );
    }
    return "absent";
  }
  if (collisions.rows.length !== 1) throw new Error("Phase 119 found an alternate PuChiCo identity or source owner before first write.");
  const row = collisions.rows[0];
  if (row?.id !== PHASE119_PUCHICO_ID || row.type !== "pen" || row.slug !== PHASE119_PUCHICO_SLUG || row.name !== "Wancher PuChiCo") {
    throw new Error("Phase 119 found an alternate PuChiCo identity or source owner before first write.");
  }
  if (row.source !== sourceMarker) throw new Error("Phase 119 locked target exists without its exact source marker; repair is forbidden.");
  ...
  if (JSON.stringify(actualSourcePairs) !== JSON.stringify(expectedSourcePairs)) {
    throw new Error(
      "Phase 119 terminal source item ownership is incomplete or tampered; repair is forbidden.",
    );
  }
  return "terminal";
}
...
async function upsertPhase119Sources(
  ...
  for (const source of pack.sources) {
    await transaction.execute({
      ...
      args: [
        ...
        source.author ?? null,
        source.homepageUrl,
        `Phase 119 curated source registry: ${source.registryKey}`,
        source.retrievedAt,
        source.tier,
...
async function installPhase119Pack(
  client: Client,
  pack: LoadedCuratedEntityPack,
): Promise<void> {
  const transaction = await client.transaction("write");
  try {
    await upsertPhase119Sources(transaction, pack);
    ...
    if (updated.rowsAffected !== 1) throw new Error("Phase 119 target identity changed before payload install.");
    ...
    if (publication.rowsAffected !== 1) throw new Error("Phase 119 target publication row is missing.");
    ...
    for (const alias of pack.aliases) {
      ...
      if (!source) throw new Error(`Phase 119 alias source missing: ${alias.sourceKey}`);
      await transaction.execute({
    ...
    for (const claim of pack.claims) {
      ...
      for (const item of claim.evidence) {
        ...
        if (!scopeId) throw new Error(`Phase 119 claim scope missing: ${item.scopeKey}`);
    ...
    if (!pack.spec) throw new Error("Phase 119 target pack requires model specs.");
    ...
    for (const item of pack.spec.evidence) {
      const scopeId = scopeIds.get(item.scopeKey);
      if (!scopeId) throw new Error(`Phase 119 spec scope missing: ${item.scopeKey}`);
    ...
    for (const conflict of pack.conflicts ?? []) {
      ...
      for (const member of conflict.members) {
        ...
        if (!citationId) throw new Error(`Phase 119 conflict citation missing: ${member.citationKey}`);
        await transaction.execute({
...
async function reviewAndPublishBrand(client: Client, reviewer: string, expectedHash: string): Promise<void> {
  const currentHash = await computePublicationContentHash(client, PHASE119_WANCHER_ID);
  if (currentHash !== expectedHash) throw new Error("Phase 119 Wancher post-topology current hash changed unexpectedly.");
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE119_WANCHER_ID,
      reviewKind,
      reviewer,
      status: "approved",
      notes: `Phase 119 ${reviewKind} review for exact post-topology current hash; Phase 107 brand pack not replayed.`,
    });
  }
  await publishEntity(client, { entityId: PHASE119_WANCHER_ID, reviewer });
}
...
async function assertTerminal(client: Client, sourceMarker: string): Promise<string> {
  ...
  const currentHash = await computePublicationContentHash(client, PHASE119_PUCHICO_ID);
  if (!row || row.type !== "pen" || row.slug !== PHASE119_PUCHICO_SLUG || row.name !== "Wancher PuChiCo" || row.source !== sourceMarker || row.status !== "published" || row.approved_content_hash !== currentHash || Number(row.blocker_count) !== 0 || Number(row.publishable) !== 1 || Number(row.is_public) !== 1) {
    throw new Error("Phase 119 terminal identity/source/publication state is invalid; automatic repair is forbidden.");
  }
  const topology = await client.execute({
    ...
    args: [PHASE119_PUCHICO_ID, PHASE119_PUCHICO_ID],
  });
  if (topology.rows.length !== 2 || topology.rows[0]?.source_id !== PHASE119_PUCHICO_ID || topology.rows[0]?.target_id !== PHASE119_WANCHER_ID || topology.rows[0]?.link_type !== "made_by" || topology.rows[1]?.source_id !== PHASE119_WANCHER_ID || topology.rows[1]?.target_id !== PHASE119_PUCHICO_ID || topology.rows[1]?.link_type !== "reverse") {
    throw new Error("Phase 119 terminal made_by/reverse topology is invalid; automatic repair is forbidden.");
  }
  const evidence = await client.execute({
    sql: `SELECT
            ...
            (SELECT count(*) FROM model_variants WHERE model_entity_id=? AND variant_kind='color' AND notes LIKE '%availability%mutable%') AS variants,
            (SELECT count(*) FROM fact_scopes WHERE entity_id=? AND scope_key LIKE '%sarah-read%' AND edition_scope LIKE '%JetPens%') AS sarah_scopes,
            (SELECT count(*) FROM fact_scopes WHERE entity_id=? AND scope_key LIKE '%kimberly-lau%' AND edition_scope LIKE '%full price%' AND nib_scope LIKE '%Black Chocolate Orange/Fine%') AS kimberly_scopes,
    ...
    args: [PHASE119_PUCHICO_ID, PHASE119_PUCHICO_ID, PHASE119_PUCHICO_ID, PHASE119_PUCHICO_ID, PHASE119_PUCHICO_ID, PHASE119_PUCHICO_ID, PHASE119_PUCHICO_ID, PHASE119_PUCHICO_ID, currentHash],
  });
  const counts = evidence.rows[0];
  if (
    Number(counts?.scopes) !== 3 ||
    Number(counts?.claims) !== 4 ||
    Number(counts?.variants) !== PHASE119_VARIANT_NAMES.length ||
    Number(counts?.sarah_scopes) !== 1 ||
    Number(counts?.kimberly_scopes) !== 1 ||
    Number(counts?.rejected_spec_evidence) < 10 ||
    Number(counts?.primary_media) !== 1 ||
    Number(counts?.reviews) !== 4
  ) {
    throw new Error("Phase 119 terminal scope/evidence/media/review state is invalid; automatic repair is forbidden.");
  }
  ...
  const expectedVariants = [...PHASE119_VARIANT_NAMES].sort();
  if (JSON.stringify(actualVariants) !== JSON.stringify(expectedVariants)) {
    throw new Error("Phase 119 terminal variant set is invalid; automatic repair is forbidden.");
  }
  const brandHash = await computePublicationContentHash(client, PHASE119_WANCHER_ID);
  const brandPublication = await client.execute({
    ...
    args: [PHASE119_WANCHER_ID],
  });
  if (brandPublication.rows[0]?.status !== "published" || brandPublication.rows[0]?.approved_content_hash !== brandHash) {
    throw new Error("Phase 119 terminal Wancher post-topology publication is invalid; automatic repair is forbidden.");
  }
  return currentHash;
}

export async function applyPhase119WancherPuchicoContent(
  client: Client,
  options: ApplyPhase119Options,
): Promise<ApplyPhase119Result> {
  const workspaceRoot = await assertAuthority(client, options);
  const pack = loadPhase119WancherPuchicoPack(workspaceRoot);
  ...
  if (targetState === "terminal") {
    ...
    assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
    return { entities: [{ entityId: PHASE119_PUCHICO_ID, outcome: "noop", contentHash }] };
  }

  const articleBefore = await payloadDigest(client, PHASE119_DREAM_ARTICLE_ID, true);
  const trueEboniteBefore = await payloadDigest(client, PHASE119_TRUE_EBONITE_ID, true);
  const titaniumBefore = await payloadDigest(client, PHASE119_TITANIUM_BLACK_ID, true);
  const akaBefore = await payloadDigest(client, PHASE119_AKA_TAMENURI_ID, true);
  const brandPayloadBefore = await payloadDigest(client, PHASE119_WANCHER_ID, false);
  const brandHashBefore = await computePublicationContentHash(client, PHASE119_WANCHER_ID);
  const transaction = await client.transaction("write");
  try {
    await transaction.execute({
      sql: "INSERT INTO entities(id,type,slug,name) VALUES(?,'pen',?,?)",
      args: [PHASE119_PUCHICO_ID, PHASE119_PUCHICO_SLUG, "Wancher PuChiCo"],
    });
    await transaction.execute({
      sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
      args: [stableId("phase119-made-by", `${PHASE119_PUCHICO_ID}:${PHASE119_WANCHER_ID}`), PHASE119_PUCHICO_ID, PHASE119_WANCHER_ID, "Phase 119 exact Wancher maker relation"],
    });
    await transaction.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
      args: [stableId("phase119-reverse", `${PHASE119_WANCHER_ID}:${PHASE119_PUCHICO_ID}`), PHASE119_WANCHER_ID, PHASE119_PUCHICO_ID, "Phase 119 Wancher-to-PuChiCo public navigation"],
    });
    await transaction.commit();
  } catch (error) {
    if (!transaction.closed) await transaction.rollback();
  ...
  if (await payloadDigest(client, PHASE119_DREAM_ARTICLE_ID, true) !== articleBefore) throw new Error("Phase 119 changed the protected Dream Pen article payload/topology.");
  if (await payloadDigest(client, PHASE119_TRUE_EBONITE_ID, true) !== trueEboniteBefore) throw new Error("Phase 119 changed the protected True Ebonite payload/topology.");
  if (await payloadDigest(client, PHASE119_TITANIUM_BLACK_ID, true) !== titaniumBefore) throw new Error("Phase 119 changed the protected Titanium Black payload/topology/publication.");
  if (await payloadDigest(client, PHASE119_AKA_TAMENURI_ID, true) !== akaBefore) throw new Error("Phase 119 changed the protected Aka Tamenuri payload/topology/publication.");
  if (await payloadDigest(client, PHASE119_WANCHER_ID, false) !== brandPayloadBefore) throw new Error("Phase 119 changed Wancher non-topology payload or Phase 107 source marker.");
  const brandHashAfter = await computePublicationContentHash(client, PHASE119_WANCHER_ID);
  if (brandHashAfter === brandHashBefore) throw new Error("Phase 119 expected one-link-pair Wancher topology hash change.");
  await reviewAndPublishBrand(client, options.reviewer.trim(), brandHashAfter);

  await installPhase119Pack(client, pack);
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      ...
      status: "approved",
      notes: `${pack.sourceMarker}; ${reviewKind} review of Phase 119 checked-in sourced copy.`,
    });
  }
  ...
  const result: ApplyPhase119Result = {
    entities: [
      {
        entityId: PHASE119_PUCHICO_ID,
        outcome: "published",
  ...
  if (await payloadDigest(client, PHASE119_DREAM_ARTICLE_ID, true) !== articleBefore) throw new Error("Phase 119 changed the protected Dream Pen article after publish.");
  if (await payloadDigest(client, PHASE119_TRUE_EBONITE_ID, true) !== trueEboniteBefore) throw new Error("Phase 119 changed the protected True Ebonite entity after publish.");
  if (await payloadDigest(client, PHASE119_TITANIUM_BLACK_ID, true) !== titaniumBefore) throw new Error("Phase 119 changed the protected Titanium Black entity after publish.");
  if (await payloadDigest(client, PHASE119_AKA_TAMENURI_ID, true) !== akaBefore) throw new Error("Phase 119 changed the protected Aka Tamenuri entity after publish.");
  if (await payloadDigest(client, PHASE119_WANCHER_ID, false) !== brandPayloadBefore) throw new Error("Phase 119 replayed or changed the Phase 107 Wancher brand pack.");
  assertCatalogSnapshotUnchanged(options.protectedCatalogSnapshot);
  return result;
}
...
async function main(): Promise<void> {
  ...
  const reviewer = value("--reviewer") ?? "phase119-wancher-puchico";
  if (!databasePath || !ownedRoot || !protectedCatalogPath) {
    throw new Error("Usage: tsx scripts/apply-phase119-wancher-puchico-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]");
  }
  ...
  try {
    const result = await applyPhase119WancherPuchicoContent(client, {
