...
import {
  ...
  phase400PilotCustomUrushiRefreshPacks,
} from "./data/phase400-pilot-custom-urushi-refresh";
import {
...
export type ApplyPhase400Options = ApplyPhase22Options;
export type ApplyPhase400Result = ApplyPhase22Result;
...
function rejectRemote(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (env[key]?.trim()) {
      throw new Error(`Phase 400 refuses inherited remote database selection: ${key}.`);
    }
  }
}

async function assertAuthority(client: Client, options: ApplyPhase400Options): Promise<void> {
  rejectRemote(options.env ?? process.env);
  if (!options.reviewer.trim()) throw new Error("Phase 400 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 400 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 400 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 400 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 400 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  const rows = await client.execute({
    ...
    args: [PHASE400_PILOT_ID, PHASE400_URUSHI_ID],
  });
  const brand = rows.rows.find((row) => String(row.id) === PHASE400_PILOT_ID);
  const model = rows.rows.find((row) => String(row.id) === PHASE400_URUSHI_ID);
  if (
    !brand ||
    ...
    throw new Error("Phase 400 Pilot Custom URUSHI identity mismatch.");
  }
}
...
async function ensureTopology(client: Client): Promise<void> {
  ...
  try {
    await tx.execute({
      ...
      args: [PHASE400_URUSHI_ID, PHASE400_PILOT_ID],
    });
    const maker = await tx.execute({
      sql: "SELECT id,target_id FROM entity_links WHERE source_id=? AND link_type='made_by'",
      args: [PHASE400_URUSHI_ID],
    });
    if (maker.rows.length === 0) {
      await tx.execute({
        sql: "INSERT INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
        args: [
          "phase400-made-by-pilot-custom-urushi",
          PHASE400_URUSHI_ID,
          PHASE400_PILOT_ID,
          "Phase 400 verified Pilot Custom URUSHI maker relation",
        ],
      });
    } else if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE400_PILOT_ID
    ) {
      throw new Error("Phase 400 Pilot Custom URUSHI maker relation is ambiguous.");
    }
    ...
    await tx.execute({
      ...
      args: [
        "phase400-reverse-pilot-custom-urushi",
        PHASE400_PILOT_ID,
        PHASE400_URUSHI_ID,
        "Phase 400 Pilot brand navigation to Custom URUSHI",
      ],
    });
    ...
    if (
      ...
      String(makerAfter.rows[0]?.target_id) !== PHASE400_PILOT_ID ||
      reverseAfter.rows.length !== 1
    ) {
      throw new Error("Phase 400 Pilot Custom URUSHI topology is ambiguous.");
    }
    await tx.commit();
  } catch (error) {
...
async function exactTerminalState(
  ...
  const reverse = await client.execute({
    sql: "SELECT source_id FROM entity_links WHERE source_id=? AND target_id=? AND link_type='reverse'",
    args: [PHASE400_PILOT_ID, pack.entityId],
  });
...
async function republishPilotBrand(client: Client, reviewer: string): Promise<void> {
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE400_PILOT_ID,
      reviewKind,
      reviewer,
      status: "approved",
      notes: "Phase 400 re-approves the unchanged Pilot brand after normalizing the Custom URUSHI navigation link.",
    });
  }
  await publishEntity(client, { entityId: PHASE400_PILOT_ID, reviewer });
}

export async function applyPhase400PilotCustomUrushiRefresh(
  client: Client,
  options: ApplyPhase400Options,
): Promise<ApplyPhase400Result> {
  await assertAuthority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const pack = loadCuratedEntityPack(workspaceRoot, phase400PilotCustomUrushiRefreshPacks[0]!);
  validatePack(workspaceRoot, pack);
  if (Array.from(pack.bodyMd).length < 8_000) {
    throw new Error("Phase 400 reviewed body must contain at least 8,000 Unicode characters.");
  }
  if (pack.sources.length < 10 || pack.variants?.length !== 12) {
    throw new Error("Phase 400 pack lacks the expected source or SKU coverage.");
  }
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase400-pilot-custom-urushi-refresh.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase400PilotCustomUrushiRefresh(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase400-pilot-custom-urushi-refresh",
