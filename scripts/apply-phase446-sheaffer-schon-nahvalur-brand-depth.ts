...
} from "./data/phase446-sheaffer-schon-nahvalur-brand-depth";

export type ApplyPhase446Options = ApplyPhase22Options;
export type ApplyPhase446Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase446Options): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 446 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function authority(client: Client, options: ApplyPhase446Options): Promise<void> {
  ...
  if (
    ...
    throw new Error("Phase 446 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 446 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 446 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 446 owned copy must be migrated through 032.");
  }
}
...
async function identity(
  ...
  if (packs.length !== 3 || new Set(packs.map((pack) => pack.entityId)).size !== 3) {
    throw new Error("Phase 446 must load three unique brand packs.");
  }
  for (const [label, id] of Object.entries(PHASE446_BRAND_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack || pack.expectedType !== "brand" || pack.expectedSlug !== label) {
      throw new Error(`Phase 446 ${label} pack identity mismatch.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 446 ${label} brand identity mismatch: ${JSON.stringify(existing)}`);
    }
  }
}
    ...
    for (const [label, brandId] of Object.entries(PHASE446_BRAND_IDS)) {
      ...
      for (const row of models.rows) {
        ...
        await tx.execute({
          ...
          args: [
            `phase446-reverse-${label}-${modelId}`,
            brandId,
            modelId,
            `Phase 446 ${label} brand navigation for published pen ${modelId}`,
          ],
        });
        ...
        if (Number(reverse.rows[0]?.n) !== 1) {
          throw new Error(`Phase 446 ${label} reverse navigation is ambiguous for ${modelId}.`);
        }
      }
    }
    await tx.commit();
...
async function applyBrandPacks(
  client: Client,
  options: ApplyPhase446Options,
  packs: LoadedCuratedEntityPack[],
): Promise<ApplyPhase446Result> {
  ...
  const entities: ApplyPhase446Result["entities"] = [];
...
export async function applyPhase446BrandDepth(
  client: Client,
  options: ApplyPhase446Options,
): Promise<ApplyPhase446Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 446 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase446SheafferSchonNahvalurBrandPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase446-sheaffer-schon-nahvalur-brand-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase446BrandDepth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase446-brand-depth",
