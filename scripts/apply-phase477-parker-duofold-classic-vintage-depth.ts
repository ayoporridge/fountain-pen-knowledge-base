...
import {
  PHASE477_IDS,
  phase477ParkerDuofoldClassicVintageDepthPacks,
} from "./data/phase477-parker-duofold-classic-vintage-depth";

export type ApplyPhase477Options = ApplyPhase22Options;
export type ApplyPhase477Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase477Options): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 477 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function authority(client: Client, options: ApplyPhase477Options): Promise<void> {
  ...
  if (
    ...
    throw new Error("Phase 477 requires an owned, non-symlink catalog copy.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 477 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 477 owned copy must be migrated through 032.");
  }
}

async function identity(client: Client, packs: LoadedCuratedEntityPack[]): Promise<void> {
  const ids = Object.values(PHASE477_IDS);
  if (
    packs.length !== ids.length ||
    new Set(packs.map((pack) => pack.entityId)).size !== ids.length
  ) {
    throw new Error("Phase 477 must load Parker Duofold vintage and Centennial exactly once.");
  }
  for (const [label, id] of Object.entries(PHASE477_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack?.expectedSlug) throw new Error(`Phase 477 ${label} pack identity mismatch.`);
    ...
    if (
      ...
      throw new Error(`Phase 477 ${label} identity mismatch: ${JSON.stringify(existing)}`);
    }
  }
}
...
async function repairModelNavigation(
  ...
  try {
    for (const pack of packs) {
      ...
      if (!brandId) throw new Error(`Phase 477 ${pack.expectedSlug} has no maker identity.`);
      const maker = await tx.execute({
        sql: "SELECT count(*) AS n FROM entity_links WHERE source_id=? AND target_id=? AND link_type='made_by'",
        args: [pack.entityId, brandId],
      });
      if (Number(maker.rows[0]?.n) !== 1) {
        throw new Error(`Phase 477 ${pack.expectedSlug} must have exactly one made_by relation.`);
      }
      await tx.execute({
        sql: "INSERT OR IGNORE INTO entity_links (id,source_id,target_id,link_type,reason) VALUES (?,?,?,'reverse',?)",
        args: [
          `phase477-reverse-${pack.entityId}`,
          brandId,
          pack.entityId,
          `Phase 477 Parker Duofold navigation for ${pack.expectedSlug}`,
        ],
      });
      ...
      if (Number(reverse.rows[0]?.n) !== 1) {
        throw new Error(`Phase 477 ${pack.expectedSlug} reverse navigation is ambiguous.`);
      }
    }
    await tx.commit();
  } catch (error) {
...
async function applyPacks(
  client: Client,
  options: ApplyPhase477Options,
  packs: LoadedCuratedEntityPack[],
): Promise<ApplyPhase477Result> {
  ...
  const entities: ApplyPhase477Result["entities"] = [];
  for (const pack of packs) {
    for (const reviewKind of ["fact", "language", "media"] as const) {
      await recordEntityContentReview(client, {
        ...
        status: "approved",
        notes: `${pack.sourceMarker}; ${reviewKind} review of Parker Duofold family copy.`,
      });
    }
...
export async function applyPhase477ParkerDuofoldClassicVintageDepth(
  client: Client,
  options: ApplyPhase477Options,
): Promise<ApplyPhase477Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 477 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase477ParkerDuofoldClassicVintageDepthPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase477-parker-duofold-classic-vintage-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase477ParkerDuofoldClassicVintageDepth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase477-parker-duofold",
