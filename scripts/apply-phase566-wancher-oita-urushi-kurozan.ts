...
} from "./data/phase521-wancher-japanese-lacquer-siblings";

export const PHASE566_TARGET_ID = "phase521-wancher-oita-urushi-kurozan";
export const PHASE566_DUPLICATE_ID = "phase524-wancher-oita-urushi-kurozan";
export const PHASE566_REVIEWER = "phase566-wancher-oita-urushi-kurozan";

export type ApplyPhase566Options = {
...
export type ApplyPhase566Result = {
  entityId: string;
...
function rejectRemote(env: NodeJS.ProcessEnv): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (env[key]?.trim()) {
      throw new Error(`Phase 566 refuses inherited remote database selection: ${key}.`);
    }
  }
}

async function assertAuthority(client: Client, options: ApplyPhase566Options): Promise<void> {
  if (!options.reviewer.trim()) throw new Error("Phase 566 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 566 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 566 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 566 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 566 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  const brand = await client.execute({
    ...
    args: [PHASE521_WANCHER_BRAND_ID],
  });
  if (
    brand.rows.length !== 1 ||
    ...
    throw new Error("Phase 566 Wancher brand identity is not the approved canonical row.");
  }
  ...
  if (
    ...
    throw new Error(`Phase 566 refuses an identity collision: ${pack.entityId}.`);
  }
  const duplicate = await client.execute({
    sql: "SELECT type,slug FROM entities WHERE id=?",
    args: [PHASE566_DUPLICATE_ID],
  });
  if (
    duplicate.rows.length !== 1 ||
    String(duplicate.rows[0]?.type) !== "pen" ||
    String(duplicate.rows[0]?.slug) !== "wancher-oita-urushi-kurozan"
  ) {
    throw new Error("Phase 566 duplicate identity donor is missing or changed.");
  }
  ...
  if (
    ...
    String(publication.rows[0]?.status) !== "retired" ||
    String(publication.rows[0]?.blockers_json) !== '["taxonomy_merged"]'
  ) {
    throw new Error("Phase 566 refuses to reuse the retired duplicate identity.");
  }
}
...
async function prepareTopology(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  ...
  try {
    await transaction.execute({
      ...
      args: [pack.entityId, PHASE521_WANCHER_BRAND_ID],
    });
    await transaction.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
      args: [
        `phase566-made-by-${pack.entityId}`,
        pack.entityId,
        PHASE521_WANCHER_BRAND_ID,
        `Phase 566 verified Wancher maker relation for ${pack.canonicalName}`,
      ],
    });
    await transaction.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
      args: [
        `phase566-reverse-${pack.entityId}`,
        PHASE521_WANCHER_BRAND_ID,
        pack.entityId,
        `Phase 566 Wancher brand navigation to ${pack.canonicalName}`,
      ],
    });
    ...
    const reverse = await transaction.execute({
      ...
      args: [PHASE521_WANCHER_BRAND_ID, pack.entityId],
    });
    if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE521_WANCHER_BRAND_ID
    ) {
      throw new Error(`Phase 566 maker topology is ambiguous: ${pack.entityId}.`);
    }
    if (reverse.rows.length !== 1) {
      throw new Error(`Phase 566 reverse navigation is ambiguous: ${pack.entityId}.`);
    }
...
async function applyPack(
  client: Client,
  options: ApplyPhase566Options,
  pack: LoadedCuratedEntityPack,
): Promise<ApplyPhase566Result> {
  ...
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      ...
      status: "approved",
      notes: `${pack.sourceMarker}; ${reviewKind} review of the deepened Kurozan copy.`,
    });
  }
...
export async function applyPhase566WancherOitaUrushiKurozan(
  client: Client,
  options: ApplyPhase566Options,
): Promise<ApplyPhase566Result> {
  await assertAuthority(client, options);
  const rawPack = phase521WancherJapaneseLacquerPacks.find(
    (pack) => pack.entityId === PHASE566_TARGET_ID,
  );
  if (!rawPack) throw new Error("Phase 566 Kurozan pack is missing.");
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase566-wancher-oita-urushi-kurozan.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase566WancherOitaUrushiKurozan(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? PHASE566_REVIEWER,
