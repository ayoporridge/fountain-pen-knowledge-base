...
import {
  PHASE453_MODEL_IDS,
  phase453ParkerPremierVpDepthPacks,
} from "./data/phase453-parker-premier-vp-depth";

export type ApplyPhase453Options = ApplyPhase22Options;
export type ApplyPhase453Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase453Options): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 453 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function authority(client: Client, options: ApplyPhase453Options): Promise<void> {
  ...
  if (
    ...
    throw new Error("Phase 453 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 453 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 453 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 453 owned copy must be migrated through 032.");
  }
}

async function identity(client: Client, packs: LoadedCuratedEntityPack[]): Promise<void> {
  const ids = Object.values(PHASE453_MODEL_IDS);
  if (
    packs.length !== ids.length ||
    new Set(packs.map((pack) => pack.entityId)).size !== ids.length
  ) {
    throw new Error("Phase 453 must load three unique Parker models.");
  }
  for (const [label, id] of Object.entries(PHASE453_MODEL_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack || pack.expectedType !== "pen" || !pack.expectedSlug) {
      throw new Error(`Phase 453 ${label} pack identity mismatch.`);
    }
    ...
    if (
      ...
      throw new Error(
        `Phase 453 ${label} model identity mismatch: ${JSON.stringify(existing)}`,
      );
    }
  }
}
...
async function repairModelNavigation(
  ...
  try {
    for (const pack of packs) {
      ...
      if (!brandId) throw new Error(`Phase 453 ${pack.expectedSlug} has no brand identity in spec.`);
      const maker = await tx.execute({
        sql: "SELECT count(*) AS n FROM entity_links WHERE source_id=? AND target_id=? AND link_type='made_by'",
        args: [pack.entityId, brandId],
      });
      if (Number(maker.rows[0]?.n) !== 1) {
        throw new Error(`Phase 453 ${pack.expectedSlug} must have exactly one made_by relation.`);
      }
      await tx.execute({
        sql: "INSERT OR IGNORE INTO entity_links (id,source_id,target_id,link_type,reason) VALUES (?,?,?,'reverse',?)",
        args: [
          `phase453-reverse-${pack.entityId}`,
          brandId,
          pack.entityId,
          `Phase 453 model navigation for ${pack.expectedSlug}`,
        ],
      });
      ...
      if (Number(reverse.rows[0]?.n) !== 1) {
        throw new Error(`Phase 453 ${pack.expectedSlug} reverse navigation is ambiguous.`);
      }
    }
    await tx.commit();
  } catch (error) {
...
async function applyPacks(
  client: Client,
  options: ApplyPhase453Options,
  packs: LoadedCuratedEntityPack[],
): Promise<ApplyPhase453Result> {
  ...
  const entities: ApplyPhase453Result["entities"] = [];
  for (const pack of packs) {
    for (const reviewKind of ["fact", "language", "media"] as const) {
      await recordEntityContentReview(client, {
        ...
        status: "approved",
        notes: `${pack.sourceMarker}; ${reviewKind} review of checked-in Parker Premier/VP copy.`,
      });
    }
...
export async function applyPhase453ParkerPremierVpDepth(
  client: Client,
  options: ApplyPhase453Options,
): Promise<ApplyPhase453Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 453 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase453ParkerPremierVpDepthPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase453-parker-premier-vp-depth.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase453ParkerPremierVpDepth(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase453-parker-premier-vp-depth",
