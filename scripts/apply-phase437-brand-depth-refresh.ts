...
} from "./data/phase437-brand-depth-refresh";

export type ApplyPhase437Options = ApplyPhase22Options;
export type ApplyPhase437Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase437Options): void {
  for (const key of [
    "TURSO_DATABASE_URL",
    "TURSO_AUTH_TOKEN",
    "FPKG_DATABASE_URL",
    ...
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 437 refuses inherited remote database selection: ${key}.`);
    }
  }
}
...
async function authority(
  ...
  if (
    ...
    throw new Error("Phase 437 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 437 refuses the protected catalog or hard-link alias.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 437 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 437 owned copy must be migrated through 032.");
  }
}
...
async function identity(
  ...
  if (packs.length !== 5 || new Set(packs.map((pack) => pack.entityId)).size !== 5) {
    throw new Error("Phase 437 must load five unique brand packs.");
  }
  for (const [label, id] of Object.entries(PHASE437_BRAND_IDS)) {
    const pack = packs.find((candidate) => candidate.entityId === id);
    if (!pack || pack.expectedType !== "brand" || pack.expectedSlug !== label) {
      throw new Error(`Phase 437 ${label} pack identity mismatch.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 437 ${label} brand identity mismatch: ${JSON.stringify(existing)}`);
    }
  }
}
...
async function repairBrandNavigation(client: Client): Promise<void> {
  ...
  try {
    for (const [label, brandId] of Object.entries(PHASE437_BRAND_IDS)) {
      ...
      for (const row of models.rows) {
        ...
        await tx.execute({
          ...
          args: [
            `phase437-reverse-${label}-${modelId}`,
            brandId,
            modelId,
            `Phase 437 ${label} brand navigation for published pen ${modelId}`,
          ],
        });
        ...
        if (Number(reverse.rows[0]?.n) !== 1) {
          throw new Error(`Phase 437 ${label} reverse navigation is ambiguous for ${modelId}.`);
        }
      }
    }
    await tx.commit();
...
async function applyBrandPacks(
  ...
  const entities: ApplyPhase437Result["entities"] = [];
...
export async function applyPhase437BrandDepthRefresh(
  client: Client,
  options: ApplyPhase437Options,
): Promise<ApplyPhase437Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 437 reviewer must not be empty.");
  await authority(client, options);
  const workspaceRoot = fs.realpathSync.native(options.workspaceRoot);
  const packs = phase437BrandDepthRefreshPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase437-brand-depth-refresh.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase437BrandDepthRefresh(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase437-brand-depth-refresh",
