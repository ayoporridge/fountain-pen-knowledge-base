...
import {
  PHASE523_TARGETS,
  PHASE523_WANCHER_BRAND_ID,
  phase523WancherHirotaYakumoPacks,
} from "./data/phase523-wancher-hirota-yakumo-siblings";

export type ApplyPhase523Options = ApplyPhase22Options;
export type ApplyPhase523Result = ApplyPhase22Result;
...
function assertNoRemote(options: ApplyPhase523Options): void {
  for (const key of ["TURSO_DATABASE_URL", "TURSO_AUTH_TOKEN", "FPKG_DATABASE_URL"] as const) {
    if (options.env?.[key]?.trim()) {
      throw new Error(`Phase 523 refuses inherited remote database selection: ${key}.`);
    }
  }
}

async function assertAuthority(client: Client, options: ApplyPhase523Options): Promise<void> {
  ...
  if (
    ...
    throw new Error("Phase 523 owned catalog authority check failed.");
  }
  if (
    database === protectedPath ||
    (owned.dev === protectedStat.dev && owned.ino === protectedStat.ino)
  ) {
    throw new Error("Phase 523 refuses the protected catalog and hard-link aliases.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 523 client is not bound to the authorized owned copy.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 523 owned copy must be migrated through 032.");
  }
}
...
async function assertIdentity(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  const brand = await client.execute({
    ...
    args: [PHASE523_WANCHER_BRAND_ID],
  });
  if (
    brand.rows.length !== 1 ||
    ...
    throw new Error("Phase 523 Wancher brand identity is not the approved canonical row.");
  }
  ...
  if (
    ...
    throw new Error(`Phase 523 Wancher identity collision: ${pack.entityId}.`);
  }
}
...
async function prepareTopology(client: Client, pack: LoadedCuratedEntityPack): Promise<void> {
  ...
  try {
    ...
    if (existing.rows.length === 0) {
      ...
      throw new Error(`Phase 523 refuses a conflicting Wancher identity: ${pack.entityId}.`);
    }
    await transaction.execute({
      sql: "DELETE FROM entity_links WHERE source_id = ? AND link_type = 'made_by' AND target_id <> ?",
      args: [pack.entityId, PHASE523_WANCHER_BRAND_ID],
    });
    await transaction.execute({
      sql: "INSERT OR IGNORE INTO entity_links (id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
      args: [
        `phase523-made-by-${pack.entityId}`,
        pack.entityId,
        PHASE523_WANCHER_BRAND_ID,
        `Phase 523 verified Wancher maker relation for ${pack.canonicalName}`,
      ],
    });
    await transaction.execute({
      sql: "INSERT OR IGNORE INTO entity_links (id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
      args: [
        `phase523-reverse-${pack.entityId}`,
        PHASE523_WANCHER_BRAND_ID,
        pack.entityId,
        `Phase 523 Wancher brand navigation to ${pack.canonicalName}`,
      ],
    });
    ...
    if (maker.rows.length !== 1 || String(maker.rows[0]?.target_id) !== PHASE523_WANCHER_BRAND_ID) {
      throw new Error(`Phase 523 maker topology is ambiguous: ${pack.entityId}.`);
    }
    ...
    if (
      ...
      throw new Error(`Phase 523 expects a publication row: ${pack.entityId}.`);
    }
...
async function refreshWancherBrandPublication(client: Client, reviewer: string): Promise<void> {
  ...
  const contentHash = await computePublicationContentHash(client, PHASE523_WANCHER_BRAND_ID);
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      entityId: PHASE523_WANCHER_BRAND_ID,
      reviewKind,
      reviewer,
      status: "approved",
      notes: `Phase 523 ${reviewKind} review after adding Wancher Hirota and Yakumo siblings.`,
      contentHash,
    });
  }
  await publishEntity(client, { entityId: PHASE523_WANCHER_BRAND_ID, reviewer });
}

async function applySinglePack(
  client: Client,
  options: ApplyPhase523Options,
  pack: LoadedCuratedEntityPack,
): Promise<ApplyPhase523Result> {
  ...
  if ((pack.publicationIntent ?? "publish") !== "publish") {
    throw new Error(`Phase 523 only supports publish-intent packs: ${pack.entityId}.`);
  }
  const transaction = await client.transaction("write");
  try {
  ...
  for (const reviewKind of ["fact", "language", "media"] as const) {
    await recordEntityContentReview(client, {
      ...
      status: "approved",
      notes: `${pack.sourceMarker}; ${reviewKind} review of the checked-in Wancher Hirota/Yakumo copy.`,
    });
  }
...
export async function applyPhase523WancherHirotaYakumoSiblings(
  client: Client,
  options: ApplyPhase523Options,
): Promise<ApplyPhase523Result> {
  if (!options.reviewer.trim()) throw new Error("Phase 523 reviewer must not be empty.");
  await assertAuthority(client, options);
  if (PHASE523_TARGETS.length !== 4 || phase523WancherHirotaYakumoPacks.length !== 4) {
    throw new Error("Phase 523 expects four Wancher Hirota/Yakumo packs.");
  }
  const packs = phase523WancherHirotaYakumoPacks.map((rawPack) =>
    loadCuratedEntityPack(fs.realpathSync.native(options.workspaceRoot), rawPack),
  );
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error("Usage: tsx scripts/apply-phase523-wancher-hirota-yakumo-siblings.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]");
  }
  const resolvedDatabase = path.resolve(database);
  const resolvedProtected = path.resolve(protectedCatalog);
  const client = createClient({ url: `file:${resolvedDatabase}` });
  try {
    const result = await applyPhase523WancherHirotaYakumoSiblings(client, {
      workspaceRoot: process.cwd(),
      reviewer: cliValue("--reviewer") ?? "phase523-wancher-hirota-yakumo-siblings",
