...
} from "./data/phase332-wancher-sekai-ai";

export type ApplyPhase332Options = ApplyPhase22Options;
export type ApplyPhase332Result = ApplyPhase22Result;
export {
  PHASE332_SEKAI_AI_ID,
  PHASE332_SEKAI_AI_SLUG,
  PHASE332_WANCHER_BRAND_ID,
...
function assertNoRemote(options: ApplyPhase332Options): void {
  for (const key of [
    "TURSO_DATABASE_URL",
    "TURSO_AUTH_TOKEN",
    "FPKG_DATABASE_URL",
    ...
    if (options.env?.[key]?.trim()) {
      throw new Error(
        `Phase 332 refuses inherited remote database selection: ${key}.`,
      );
    }
  }
}
...
async function authority(
  client: Client,
  options: ApplyPhase332Options,
): Promise<void> {
  assertNoRemote(options);
  if (!options.reviewer.trim()) throw new Error("Phase 332 reviewer must not be empty.");
  ...
  if (
    ...
    throw new Error("Phase 332 requires an owned, non-symlink catalog copy.");
  }
  ...
  if (
    ...
    throw new Error("Phase 332 refuses the protected catalog or hard-link alias.");
  }
  ...
  if (migration.rows.length !== 1) {
    throw new Error("Phase 332 owned copy must be migrated through 032.");
  }
  const listed = await client.execute("PRAGMA database_list");
  const main = listed.rows.find((row) => String(row.name) === "main");
  if (!main?.file || fs.realpathSync.native(String(main.file)) !== database) {
    throw new Error("Phase 332 client is not bound to the authorized owned copy.");
  }
}
...
async function identity(
  ...
  if (
    ...
    throw new Error(`Phase 332 Wancher brand identity mismatch: ${JSON.stringify(brand)}`);
  }
  const pack = packs.find((item) => item.entityId === PHASE332_SEKAI_AI_ID);
  if (!pack) throw new Error("Phase 332 Sekai Ai pack missing.");
  const existing = await rows(
    client,
    "SELECT id,type,slug,name FROM entities WHERE id=? OR slug=?",
    [PHASE332_SEKAI_AI_ID, PHASE332_SEKAI_AI_SLUG],
  );
  if (
    existing.length > 1 ||
    (existing.length === 1 &&
      (existing[0]?.id !== PHASE332_SEKAI_AI_ID ||
        existing[0]?.type !== pack.expectedType ||
        existing[0]?.slug !== pack.expectedSlug ||
        existing[0]?.name !== pack.canonicalName))
  ) {
    throw new Error(`Phase 332 Sekai Ai identity collision: ${JSON.stringify(existing)}`);
  }
}
...
async function topology(client: Client): Promise<void> {
  ...
  try {
    await tx.execute({
      ...
      args: [PHASE332_SEKAI_AI_ID, PHASE332_SEKAI_AI_SLUG, "Wancher Sekai Ai"],
    });
    await tx.execute({
      sql: "DELETE FROM entity_links WHERE source_id=? AND link_type='made_by' AND target_id<>?",
      args: [PHASE332_SEKAI_AI_ID, PHASE332_WANCHER_BRAND_ID],
    });
    await tx.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'made_by',?)",
      args: [
        "phase332-made-by-wancher-sekai-ai",
        PHASE332_SEKAI_AI_ID,
        PHASE332_WANCHER_BRAND_ID,
        "Phase 332 verified Wancher Sekai Ai maker relation",
      ],
    });
    await tx.execute({
      sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
      args: [
        "phase332-reverse-wancher-sekai-ai",
        PHASE332_WANCHER_BRAND_ID,
        PHASE332_SEKAI_AI_ID,
        "Phase 332 Wancher brand navigation to Sekai Ai",
      ],
    });
    ...
    if (
      maker.rows.length !== 1 ||
      String(maker.rows[0]?.target_id) !== PHASE332_WANCHER_BRAND_ID
    ) {
      throw new Error("Phase 332 Sekai Ai maker topology is ambiguous.");
    }
    await tx.commit();
  } catch (error) {
...
async function ensurePublicBrandNavigation(client: Client): Promise<void> {
  const pens = await client.execute({
    ...
    args: [PHASE332_WANCHER_BRAND_ID],
  });
  const tx = await client.transaction("write");
  try {
    for (const row of pens.rows) {
      ...
      await tx.execute({
        sql: "INSERT OR IGNORE INTO entity_links(id,source_id,target_id,link_type,reason) VALUES(?,?,?,'reverse',?)",
        args: [
          `phase332-reverse-wancher-${penId}`,
          PHASE332_WANCHER_BRAND_ID,
          penId,
          "Phase 332 public Wancher model navigation repair",
        ],
      });
    }
    await tx.commit();
  ...
  const missing = await client.execute({
    ...
    args: [PHASE332_WANCHER_BRAND_ID, PHASE332_WANCHER_BRAND_ID],
  });
  if (missing.rows.length) {
    throw new Error(
      `Phase 332 Wancher public navigation incomplete: ${missing.rows
...
export async function applyPhase332WancherSekaiAiContent(
  client: Client,
  options: ApplyPhase332Options,
): Promise<ApplyPhase332Result> {
  await authority(client, options);
  const packs = phase332WancherSekaiAiPacks.map((pack) =>
...
async function main(): Promise<void> {
  ...
  if (!database || !ownedRoot || !protectedCatalog) {
    throw new Error(
      "Usage: tsx scripts/apply-phase332-wancher-sekai-ai-content.ts --database <owned-copy> --owned-root <root> --protected-catalog <real-db> [--reviewer <name>]",
    );
  }
  ...
  try {
    const result = await applyPhase332WancherSekaiAiContent(client, {
      workspaceRoot: process.cwd(),
      reviewer: value("--reviewer") ?? "phase332-wancher-sekai-ai",
