...
import { applyPhase453ParkerPremierVpDepth } from "../../scripts/apply-phase453-parker-premier-vp-depth";
import {
  PHASE453_MODEL_IDS,
  phase453ParkerPremierVpDepthPacks,
} from "../../scripts/data/phase453-parker-premier-vp-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-78i-objective-deepen-existing-aurora-ottanto/checkpoint.db",
);

test("Phase 453 deepens Parker Premier and VP on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase453-parker-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase453-parker-premier-vp-test",
  ...
  try {
    ...
    assert.equal(phase453ParkerPremierVpDepthPacks.length, 3);
    ...
    await assert.rejects(
      applyPhase453ParkerPremierVpDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase453ParkerPremierVpDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE453_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE453_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase453ParkerPremierVpDepthPacks.find(
    ...
    const replay = await applyPhase453ParkerPremierVpDepth(client, options);
