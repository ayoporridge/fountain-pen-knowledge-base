...
import { applyPhase459MajohnBrandP140P141Depth } from "../../scripts/apply-phase459-majohn-brand-p140-p141-depth";
import {
  PHASE459_MAJOHN_IDS,
  phase459MajohnBrandP140P141DepthPacks,
} from "../../scripts/data/phase459-majohn-brand-p140-p141-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-947-objective-deepen-majohn-brand-p140-p141/checkpoint.db",
);

test("Phase 459 deepens the Majohn brand, P140 and P141 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase459-majohn-brand-p140-p141-"),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase459-majohn-brand-p140-p141-test",
  ...
  try {
    ...
    assert.equal(phase459MajohnBrandP140P141DepthPacks.length, 3);
    for (const pack of phase459MajohnBrandP140P141DepthPacks) {
      assert.ok(
    ...
    await assert.rejects(
      applyPhase459MajohnBrandP140P141Depth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase459MajohnBrandP140P141Depth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE459_MAJOHN_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE459_MAJOHN_IDS)) {
      const pack = phase459MajohnBrandP140P141DepthPacks.find(
    ...
    const replay = await applyPhase459MajohnBrandP140P141Depth(client, options);
