...
import { applyPhase491EversharpDepth } from "../../scripts/apply-phase491-eversharp-depth";
import {
  PHASE491_IDS,
  phase491EversharpDepthPacks,
} from "../../scripts/data/phase491-eversharp-depth";
import {
...
test("Phase 491 deepens Eversharp Envoy, Coronet and Bantam on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase491-eversharp-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase491-eversharp-depth-test",
  ...
  try {
    ...
    assert.equal(phase491EversharpDepthPacks.length, 3);
    for (const pack of phase491EversharpDepthPacks) {
    ...
    await assert.rejects(
      applyPhase491EversharpDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase491EversharpDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE491_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE491_IDS)) {
      const pack = phase491EversharpDepthPacks.find(
    ...
    const replay = await applyPhase491EversharpDepth(client, options);
