...
import {
  type ApplyPhase519Options,
  applyPhase519WancherDreamPenNewProducts,
} from "../../scripts/apply-phase519-wancher-dream-pen-new-products";
import {
  PHASE519_TARGETS,
...
test("Phase 519 publishes four sourced Wancher Dream Pen products on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase519-wancher-")),
  );
  ...
  const options: ApplyPhase519Options = {
    workspaceRoot: ROOT,
    reviewer: "phase519-wancher-dream-pen-new-products-test",
  ...
  try {
    ...
    assert.equal(PHASE519_TARGETS.length, 4);
    assert.equal(phase519WancherDreamPenPacks.length, 4);
    for (const pack of phase519WancherDreamPenPacks) {
      ...
      assert.match(svg, /Zogan|Asagao|Tsuikin|Swan/);
    }
    await assert.rejects(
      applyPhase519WancherDreamPenNewProducts(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase519WancherDreamPenNewProducts(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase519WancherDreamPenPacks.map((pack) => [pack.entityId, "published"]),
    );
    for (const [index, pack] of phase519WancherDreamPenPacks.entries()) {
      const target = PHASE519_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Wancher|Dream Pen/);
      assert.match(String(publicRow?.body_md ?? ""), /converter|Converter/);
      assert.match(
        String(publicRow?.body_md ?? ""),
        /JoWo|Urushi|Zogan|Tsuikin|Asagao/,
      );
      assert.doesNotMatch(
        String(publicRow?.body_md ?? ""),
    ...
    const replay = await applyPhase519WancherDreamPenNewProducts(
      client,
      options,
    );
    assert.deepEqual(
