...
import { applyPhase474PilotSilvernJustusGranceDepth } from "../../scripts/apply-phase474-pilot-silvern-justus-grance-depth";
import {
  PHASE474_IDS,
  phase474PilotSilvernJustusGranceDepthPacks,
} from "../../scripts/data/phase474-pilot-silvern-justus-grance-depth";
import {
...
test("Phase 474 deepens Pilot Silvern, Justus 95 and Grance on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase474-pilot-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase474-pilot-silvern-justus-grance-test",
  ...
  try {
    ...
    assert.equal(phase474PilotSilvernJustusGranceDepthPacks.length, 3);
    const baselineCounts = new Map<
      string,
      { aliases: number; refs: number; variants: number }
    >();
    for (const pack of phase474PilotSilvernJustusGranceDepthPacks) {
      const markdown = fs.readFileSync(
        path.join(ROOT, pack.markdownFile),
        "utf8",
      );
      assert.ok(
        markdown.length >= 3_800,
    ...
    await assert.rejects(
      applyPhase474PilotSilvernJustusGranceDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase474PilotSilvernJustusGranceDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE474_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE474_IDS)) {
      const pack = phase474PilotSilvernJustusGranceDepthPacks.find(
      ...
      assert.ok(
        String(entity?.body_md ?? "").length >= 2_900,
    ...
    const replay = await applyPhase474PilotSilvernJustusGranceDepth(
      client,
      options,
    );
