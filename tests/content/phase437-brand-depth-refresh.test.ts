...
import { applyPhase437BrandDepthRefresh } from "../../scripts/apply-phase437-brand-depth-refresh";
import {
  PHASE437_BRAND_IDS,
  phase437BrandDepthRefreshPacks,
} from "../../scripts/data/phase437-brand-depth-refresh";
import {
...
test("Phase 437 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase437-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase437-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase437BrandDepthRefreshPacks.length, 5);
    for (const pack of phase437BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase437BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase437BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE437_BRAND_IDS),
    );
    ...
    const replay = await applyPhase437BrandDepthRefresh(client, options);
