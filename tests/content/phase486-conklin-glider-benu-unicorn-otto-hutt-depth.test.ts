...
import { applyPhase486ConklinGliderBenuUnicornOttoHuttDepth } from "../../scripts/apply-phase486-conklin-glider-benu-unicorn-otto-hutt-depth";
import {
  PHASE486_IDS,
  phase486ConklinGliderBenuUnicornOttoHuttDepthPacks,
} from "../../scripts/data/phase486-conklin-glider-benu-unicorn-otto-hutt-depth";
import {
...
test("Phase 486 deepens Conklin Glider, BENU Talisman True Unicorn and Otto Hutt design07 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase486-conklin-benu-otto-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase486-conklin-benu-otto-test",
  ...
  try {
    ...
    assert.equal(phase486ConklinGliderBenuUnicornOttoHuttDepthPacks.length, 3);
    for (const pack of phase486ConklinGliderBenuUnicornOttoHuttDepthPacks) {
    ...
    await assert.rejects(
      applyPhase486ConklinGliderBenuUnicornOttoHuttDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase486ConklinGliderBenuUnicornOttoHuttDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE486_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE486_IDS)) {
      const pack = phase486ConklinGliderBenuUnicornOttoHuttDepthPacks.find(
      ...
      assert.ok(Number(refs?.n) >= 5, `${label} references are too few`);
      const media = (
        await client.execute({
    ...
    const replay = await applyPhase486ConklinGliderBenuUnicornOttoHuttDepth(
      client,
      options,
    );
