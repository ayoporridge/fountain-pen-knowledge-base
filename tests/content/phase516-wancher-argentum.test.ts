...
import {
  type ApplyPhase516Options,
  applyPhase516WancherArgentum,
} from "../../scripts/apply-phase516-wancher-argentum";
import {
  PHASE516_ARGENTUM_JSON_URL,
  PHASE516_WANCHER_BRAND_ID,
  phase516WancherArgentumPacks,
} from "../../scripts/data/phase516-wancher-argentum";
import {
...
test("Phase 516 publishes sourced Wancher Argentum on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase516-wancher-")),
  );
  ...
  const options: ApplyPhase516Options = {
    workspaceRoot: ROOT,
    reviewer: "phase516-wancher-argentum-test",
  ...
  const [pack] = phase516WancherArgentumPacks;
  assert.ok(pack);
  try {
    ...
    assert.match(svg, /Argentum/);
    assert.match(svg, /925 Sterling Silver/);
    assert.match(svg, /acid etching/i);
    await assert.rejects(
      applyPhase516WancherArgentum(client, {
        ...options,
        env: {
          ...options.env,
    ...
    const first = await applyPhase516WancherArgentum(client, options);
    assert.deepEqual(
    ...
    assert.match(String(publicRow?.body_md ?? ""), /Argentum|925/);
    assert.match(String(publicRow?.body_md ?? ""), /酸蚀|Acid Etching/);
    assert.match(String(publicRow?.body_md ?? ""), /Milonga/);
    assert.match(String(publicRow?.body_md ?? ""), /Sailor Standard/);
    assert.match(String(publicRow?.body_md ?? ""), /1-of-1|1 of 1/);
    assert.doesNotMatch(
    ...
    assert.equal(
      (
        await client.execute({
          ...
          args: [pack.entityId, PHASE516_WANCHER_BRAND_ID],
        })
      ).rows[0]?.n,
      1,
    );
    assert.equal(
      (
        await client.execute({
          ...
          args: [PHASE516_WANCHER_BRAND_ID, pack.entityId],
        })
      ).rows[0]?.n,
      1,
    );
    ...
    assert.equal(
      (
        await client.execute({
          ...
          args: [PHASE516_ARGENTUM_JSON_URL],
        })
      ).rows[0]?.n,
      1,
    );
    ...
    const replay = await applyPhase516WancherArgentum(client, options);
    assert.deepEqual(
