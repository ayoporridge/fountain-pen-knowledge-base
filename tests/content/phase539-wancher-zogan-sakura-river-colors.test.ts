...
import {
  type ApplyPhase539Options,
  applyPhase539WancherZoganSakuraRiverColors,
} from "../../scripts/apply-phase539-wancher-zogan-sakura-river-colors";
import {
  PHASE539_TARGETS,
  PHASE539_WANCHER_BRAND_ID,
  phase539WancherZoganSakuraRiverColorPacks,
} from "../../scripts/data/phase539-wancher-zogan-sakura-river-colors";
import {
...
test("Phase 539 publishes three sourced Wancher Zogan Sakura River Urushi colors on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase539-wancher-")),
  );
  ...
  const options: ApplyPhase539Options = {
    workspaceRoot: ROOT,
    reviewer: "phase539-wancher-zogan-sakura-river-colors-test",
  ...
  try {
    ...
    assert.equal(PHASE539_TARGETS.length, 3);
    assert.equal(phase539WancherZoganSakuraRiverColorPacks.length, 3);
    const mediaPaths = new Set<string>();
    for (const pack of phase539WancherZoganSakuraRiverColorPacks) {
    ...
    await assert.rejects(
      applyPhase539WancherZoganSakuraRiverColors(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase539WancherZoganSakuraRiverColors(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase539WancherZoganSakuraRiverColorPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase539WancherZoganSakuraRiverColorPacks.entries()) {
      const target = PHASE539_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE539_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE539_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase539WancherZoganSakuraRiverColors(
      client,
      options,
    );
    assert.deepEqual(
