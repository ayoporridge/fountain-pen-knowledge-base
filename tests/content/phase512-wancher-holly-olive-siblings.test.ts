...
import {
  type ApplyPhase512Options,
  applyPhase512WancherHollyOliveSiblings,
} from "../../scripts/apply-phase512-wancher-holly-olive-siblings";
import {
  PHASE512_TARGETS,
  PHASE512_WANCHER_BRAND_ID,
  phase512WancherHollyOliveSiblingPacks,
} from "../../scripts/data/phase512-wancher-holly-olive-siblings";
import {
...
test("Phase 512 publishes six sourced Wancher Kiei Holly Olive sibling SKUs on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase512-wancher-")),
  );
  ...
  const options: ApplyPhase512Options = {
    workspaceRoot: ROOT,
    reviewer: "phase512-wancher-holly-olive-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE512_TARGETS.length, 6);
    assert.equal(phase512WancherHollyOliveSiblingPacks.length, 6);
    for (const pack of phase512WancherHollyOliveSiblingPacks) {
      ...
      assert.match(svg, /Holly Olive/);
      assert.match(svg, /Urushi/);
    }
    await assert.rejects(
      applyPhase512WancherHollyOliveSiblings(client, {
        ...options,
        env: {
          ...options.env,
    ...
    const first = await applyPhase512WancherHollyOliveSiblings(client, options);
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase512WancherHollyOliveSiblingPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase512WancherHollyOliveSiblingPacks.entries()) {
      const target = PHASE512_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Holly Olive|冬青/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE512_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE512_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [
              `https://www.wancherpen.com/products/kiei-urushi-holly-olive-${target.urlSlug}`,
            ],
          })
        ).rows[0]?.n,
        1,
    ...
    const replay = await applyPhase512WancherHollyOliveSiblings(
      client,
      options,
    );
    assert.deepEqual(
