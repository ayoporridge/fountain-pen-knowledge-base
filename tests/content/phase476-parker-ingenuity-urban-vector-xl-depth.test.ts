...
import { applyPhase476ParkerIngenuityUrbanVectorXlDepth } from "../../scripts/apply-phase476-parker-ingenuity-urban-vector-xl-depth";
import {
  PHASE476_IDS,
  phase476ParkerIngenuityUrbanVectorXlDepthPacks,
} from "../../scripts/data/phase476-parker-ingenuity-urban-vector-xl-depth";
import {
...
test("Phase 476 deepens Parker Ingenuity, Urban and Vector XL on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase476-parker-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase476-parker-current-test",
  ...
  try {
    ...
    assert.equal(phase476ParkerIngenuityUrbanVectorXlDepthPacks.length, 3);
    const baselineCounts = new Map<
      string,
      { aliases: number; refs: number; variants: number }
    >();
    for (const pack of phase476ParkerIngenuityUrbanVectorXlDepthPacks) {
    ...
    await assert.rejects(
      applyPhase476ParkerIngenuityUrbanVectorXlDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase476ParkerIngenuityUrbanVectorXlDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE476_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE476_IDS)) {
      const pack = phase476ParkerIngenuityUrbanVectorXlDepthPacks.find(
    ...
    const replay = await applyPhase476ParkerIngenuityUrbanVectorXlDepth(
      client,
      options,
    );
