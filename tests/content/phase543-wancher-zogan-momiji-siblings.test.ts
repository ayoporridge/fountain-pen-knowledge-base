...
import {
  type ApplyPhase543Options,
  applyPhase543WancherZoganMomijiSiblings,
} from "../../scripts/apply-phase543-wancher-zogan-momiji-siblings";
import {
  PHASE543_TARGETS,
  PHASE543_WANCHER_BRAND_ID,
  phase543WancherZoganMomijiSiblingPacks,
} from "../../scripts/data/phase543-wancher-zogan-momiji-siblings";
import {
...
test("Phase 543 publishes three sourced Wancher Zogan Momiji siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase543-wancher-")),
  );
  ...
  const options: ApplyPhase543Options = {
    workspaceRoot: ROOT,
    reviewer: "phase543-wancher-zogan-momiji-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE543_TARGETS.length, 3);
    assert.equal(phase543WancherZoganMomijiSiblingPacks.length, 3);
    const mediaPaths = new Set<string>();
    for (const pack of phase543WancherZoganMomijiSiblingPacks) {
    ...
    await assert.rejects(
      applyPhase543WancherZoganMomijiSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase543WancherZoganMomijiSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase543WancherZoganMomijiSiblingPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase543WancherZoganMomijiSiblingPacks.entries()) {
      const target = PHASE543_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE543_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE543_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase543WancherZoganMomijiSiblings(
      client,
      options,
    );
    assert.deepEqual(
