...
import {
  type ApplyPhase393Options,
  applyPhase393WancherShellGingerContent,
} from "../../scripts/apply-phase393-wancher-tsuikin-shell-ginger-content";
import {
  PHASE393_SHELL_GINGER_ID,
  PHASE393_WANCHER_BRAND_ID,
  phase393WancherShellGingerPacks,
} from "../../scripts/data/phase393-wancher-tsuikin-shell-ginger";
import {
...
test("Phase 393 publishes Wancher Ryukyu Tsuikin Shell Ginger on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase393-wancher-shell-ginger-"),
    ),
  );
  ...
  const options: ApplyPhase393Options = {
    workspaceRoot: ROOT,
    reviewer: "phase393-wancher-shell-ginger-test",
  ...
  try {
    ...
    assert.equal(phase393WancherShellGingerPacks.length, 2);
    const pack = phase393WancherShellGingerPacks.find(
      (candidate) => candidate.entityId === PHASE393_SHELL_GINGER_ID,
    );
    assert.ok(pack);
    ...
    await assert.rejects(
      applyPhase393WancherShellGingerContent(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase393WancherShellGingerContent(client, options);
    assert.deepEqual(
      first.entities.map((entity) => entity.outcome),
      ["published", "published"],
    );
    for (const candidate of phase393WancherShellGingerPacks) {
      const state = (
        await rows(
          client,
    ...
    assert.match(String(spec?.nib), /JoWo.*Wancher 18K.*Keiryu/i);
    assert.match(String(spec?.fill_system), /cartridge.*converter/i);
    assert.match(String(spec?.material), /ABS.*Tamesukashi.*Tsuikin/i);
    assert.match(String(spec?.dimensions), /未公布|Size & Shape/);
    assert.match(String(spec?.weight), /未公布/);
    assert.match(String(spec?.price_range), /900/);
    assert.match(String(spec?.status), /Shell Ginger.*demonstrator/i);
    ...
    assert.deepEqual(
      ...
      [
        `${PHASE393_SHELL_GINGER_ID}:${PHASE393_WANCHER_BRAND_ID}:made_by`,
        `${PHASE393_WANCHER_BRAND_ID}:${PHASE393_SHELL_GINGER_ID}:reverse`,
      ],
    );

    const replay = await applyPhase393WancherShellGingerContent(
      client,
      options,
    );
    assert.deepEqual(
