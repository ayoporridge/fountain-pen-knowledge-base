...
import { applyPhase466KawecoDia2MaioraImpronteOversizeNakayaWriterDepth } from "../../scripts/apply-phase466-kaweco-dia2-maiora-impronte-oversize-nakaya-writer-depth";
import {
  PHASE466_IDS,
  phase466KawecoDia2MaioraImpronteOversizeNakayaWriterDepthPacks,
} from "../../scripts/data/phase466-kaweco-dia2-maiora-impronte-oversize-nakaya-writer-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-arf-moore-fingertip-gravitas-ultemate-vac-wa/checkpoint.db",
);

test("Phase 466 deepens Kaweco DIA2, Maiora Impronte Oversize and Nakaya Writer Portable Kuro-tamenuri on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(
        os.tmpdir(),
        "fpkg-phase466-kaweco-dia2-maiora-impronte-oversize-nakaya-writer-",
      ),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer:
      "phase466-kaweco-dia2-maiora-impronte-oversize-nakaya-writer-test",
  ...
  try {
    ...
    assert.equal(
      phase466KawecoDia2MaioraImpronteOversizeNakayaWriterDepthPacks.length,
      3,
    );
    for (const pack of phase466KawecoDia2MaioraImpronteOversizeNakayaWriterDepthPacks) {
      assert.ok(
    ...
    await assert.rejects(
      applyPhase466KawecoDia2MaioraImpronteOversizeNakayaWriterDepth(client, {
        ...options,
    ...
    const first =
      await applyPhase466KawecoDia2MaioraImpronteOversizeNakayaWriterDepth(
        client,
        options,
      );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE466_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE466_IDS)) {
      const pack =
        phase466KawecoDia2MaioraImpronteOversizeNakayaWriterDepthPacks.find(
