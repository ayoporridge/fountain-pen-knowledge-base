...
import {
  type ApplyPhase522Options,
  applyPhase522WancherKyotoTsugaruSiblings,
} from "../../scripts/apply-phase522-wancher-kyoto-tsugaru-siblings";
import {
  PHASE522_TARGETS,
  PHASE522_WANCHER_BRAND_ID,
  phase522WancherKyotoTsugaruPacks,
} from "../../scripts/data/phase522-wancher-kyoto-tsugaru-siblings";
import {
...
test("Phase 522 publishes four sourced Wancher Kyoto/Tsugaru siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase522-wancher-")),
  );
  ...
  const options: ApplyPhase522Options = {
    workspaceRoot: ROOT,
    reviewer: "phase522-wancher-kyoto-tsugaru-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE522_TARGETS.length, 4);
    assert.equal(phase522WancherKyotoTsugaruPacks.length, 4);
    for (const pack of phase522WancherKyotoTsugaruPacks) {
      ...
      assert.match(svg, /Hanasusuki|Yukinoshita|Anzu|Kobai-iro/);
    }
    await assert.rejects(
      applyPhase522WancherKyotoTsugaruSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase522WancherKyotoTsugaruSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase522WancherKyotoTsugaruPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase522WancherKyotoTsugaruPacks.entries()) {
      const target = PHASE522_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Kyoto|Tsugaru|Urushi/);
      assert.match(String(publicRow?.body_md ?? ""), /JoWo/);
      assert.match(String(publicRow?.body_md ?? ""), /converter|Converter/);
      assert.doesNotMatch(
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE522_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE522_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase522WancherKyotoTsugaruSiblings(
      client,
      options,
    );
    assert.deepEqual(
