...
import { applyPhase428BrandDepthRefresh } from "../../scripts/apply-phase428-brand-depth-refresh";
import {
  PHASE428_BRAND_IDS,
  phase428BrandDepthRefreshPacks,
} from "../../scripts/data/phase428-brand-depth-refresh";
import {
...
test("Phase 428 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase428-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase428-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase428BrandDepthRefreshPacks.length, 5);
    for (const pack of phase428BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase428BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase428BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE428_BRAND_IDS),
    );
    ...
    const replay = await applyPhase428BrandDepthRefresh(client, options);
