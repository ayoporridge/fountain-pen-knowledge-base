...
import {
  type ApplyPhase495Options,
  applyPhase495ChineseBrandMedia,
  PHASE495_MEDIA_TARGETS,
  PHASE495_MODEL_MEDIA_TARGETS,
} from "../../scripts/apply-phase495-chinese-brand-media";
import {
...
test("Phase 495 separates five Chinese brand primary media rows", {
  timeout: 420_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL_CATALOG);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase495-media-")),
  );
  ...
  const options: ApplyPhase495Options = {
    workspaceRoot: ROOT,
    reviewer: "phase495-chinese-brand-media-test",
  ...
  try {
    ...
    const modelBefore = await client.execute({
      sql: `SELECT ma.entity_id,ma.id,ma.local_path,ma.source_item_id,ma.title
            FROM media_assets ma WHERE ma.id IN (${PHASE495_MODEL_MEDIA_TARGETS.map(() => "?").join(",")}) ORDER BY ma.id`,
      args: PHASE495_MODEL_MEDIA_TARGETS.map((target) => target[2]),
    });
    for (const oldPath of [
      "/images/library/site-original/phase231/yiren/yiren-878-structure.svg",
      "/images/library/site-original/phase234/yongxu/yongxu-286.svg",
      "/images/library/site-original/phase235/dongwu/dongwu-948.svg",
      "/images/library/site-original/phase235/shule/shule-2398.svg",
      "/images/library/site-original/phase235/zhangjiang/zhangjiang-988.svg",
    ]) {
      assert.equal(
        await scalar(
          client,
    ...
    await assert.rejects(
      applyPhase495ChineseBrandMedia(client, {
        ...options,
        env: {
          NODE_ENV: "test",
    ...
    const first = await applyPhase495ChineseBrandMedia(client, options);
    assert.equal(first.changed, true);
    assert.equal(first.entities.length, PHASE495_MEDIA_TARGETS.length);
    assert.deepEqual(
      first.entities.map((entity) => entity.outcome),
      new Array(PHASE495_MEDIA_TARGETS.length).fill("published"),
    );
    ...
    const second = await applyPhase495ChineseBrandMedia(client, options);
    assert.equal(second.changed, false);
    assert.deepEqual(
      second.entities.map((entity) => entity.outcome),
      new Array(PHASE495_MEDIA_TARGETS.length).fill("noop"),
    );
