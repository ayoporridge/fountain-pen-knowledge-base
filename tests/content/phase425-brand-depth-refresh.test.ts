...
import { applyPhase425BrandDepthRefresh } from "../../scripts/apply-phase425-brand-depth-refresh";
import {
  PHASE425_BRAND_IDS,
  phase425BrandDepthRefreshPacks,
} from "../../scripts/data/phase425-brand-depth-refresh";
import {
...
test("Phase 425 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase425-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase425-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase425BrandDepthRefreshPacks.length, 5);
    for (const pack of phase425BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase425BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase425BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE425_BRAND_IDS),
    );
    ...
    const replay = await applyPhase425BrandDepthRefresh(client, options);
