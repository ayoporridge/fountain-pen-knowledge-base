...
import { applyPhase469WatermanHundredYearInkVueXPenDepth } from "../../scripts/apply-phase469-waterman-hundred-year-ink-vue-x-pen-depth";
import {
  PHASE469_IDS,
  phase469WatermanHundredYearInkVueXPenDepthPacks,
} from "../../scripts/data/phase469-waterman-hundred-year-ink-vue-x-pen-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-daz-phase-468-deepen-existing-waterman-comma/checkpoint.db",
);

test("Phase 469 deepens Waterman Hundred Year, Ink-Vue and X-Pen on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase469-waterman-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase469-waterman-hundred-year-ink-vue-x-pen-test",
  ...
  try {
    ...
    assert.equal(phase469WatermanHundredYearInkVueXPenDepthPacks.length, 3);
    for (const pack of phase469WatermanHundredYearInkVueXPenDepthPacks) {
    ...
    await assert.rejects(
      applyPhase469WatermanHundredYearInkVueXPenDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase469WatermanHundredYearInkVueXPenDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE469_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE469_IDS)) {
      const pack = phase469WatermanHundredYearInkVueXPenDepthPacks.find(
    ...
    const replay = await applyPhase469WatermanHundredYearInkVueXPenDepth(
      client,
      options,
    );
