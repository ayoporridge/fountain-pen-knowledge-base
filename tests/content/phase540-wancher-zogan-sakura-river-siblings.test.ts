...
import {
  type ApplyPhase540Options,
  applyPhase540WancherZoganSakuraRiverSiblings,
} from "../../scripts/apply-phase540-wancher-zogan-sakura-river-siblings";
import {
  PHASE540_TARGETS,
  PHASE540_WANCHER_BRAND_ID,
  phase540WancherZoganSakuraRiverSiblingPacks,
} from "../../scripts/data/phase540-wancher-zogan-sakura-river-siblings";
import {
...
test("Phase 540 publishes three sourced Wancher Zogan Sakura River siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase540-wancher-")),
  );
  ...
  const options: ApplyPhase540Options = {
    workspaceRoot: ROOT,
    reviewer: "phase540-wancher-zogan-sakura-river-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE540_TARGETS.length, 3);
    assert.equal(phase540WancherZoganSakuraRiverSiblingPacks.length, 3);
    const mediaPaths = new Set<string>();
    for (const pack of phase540WancherZoganSakuraRiverSiblingPacks) {
      ...
      assert.match(svg, /Zogan|Urushi/);
    }
    await assert.rejects(
      applyPhase540WancherZoganSakuraRiverSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase540WancherZoganSakuraRiverSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase540WancherZoganSakuraRiverSiblingPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase540WancherZoganSakuraRiverSiblingPacks.entries()) {
      const target = PHASE540_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Zogan|Urushi/);
      assert.match(String(publicRow?.body_md ?? ""), /Ebonite|ABS/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE540_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE540_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase540WancherZoganSakuraRiverSiblings(
      client,
      options,
    );
    assert.deepEqual(
