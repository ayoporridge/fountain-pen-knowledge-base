...
import { applyPhase440BrandDepthRefresh } from "../../scripts/apply-phase440-brand-depth-refresh";
import {
  PHASE440_BRAND_IDS,
  phase440BrandDepthRefreshPacks,
} from "../../scripts/data/phase440-brand-depth-refresh";
import {
...
test("Phase 440 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase440-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase440-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase440BrandDepthRefreshPacks.length, 5);
    for (const pack of phase440BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase440BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase440BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE440_BRAND_IDS),
    );
    ...
    const replay = await applyPhase440BrandDepthRefresh(client, options);
