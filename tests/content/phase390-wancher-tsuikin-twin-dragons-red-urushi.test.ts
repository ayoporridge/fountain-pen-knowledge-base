...
import {
  type ApplyPhase390Options,
  applyPhase390WancherTwinDragonsRedContent,
} from "../../scripts/apply-phase390-wancher-tsuikin-twin-dragons-red-urushi-content";
import {
  PHASE390_TWIN_DRAGONS_RED_ID,
  PHASE390_WANCHER_BRAND_ID,
  phase390WancherTwinDragonsRedPacks,
} from "../../scripts/data/phase390-wancher-tsuikin-twin-dragons-red-urushi";
import {
...
test("Phase 390 publishes Wancher Twin Dragons Red Urushi on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase390-wancher-twin-dragons-"),
    ),
  );
  ...
  const options: ApplyPhase390Options = {
    workspaceRoot: ROOT,
    reviewer: "phase390-wancher-twin-dragons-red-test",
  ...
  try {
    ...
    assert.equal(phase390WancherTwinDragonsRedPacks.length, 2);
    const pack = phase390WancherTwinDragonsRedPacks.find(
      (candidate) => candidate.entityId === PHASE390_TWIN_DRAGONS_RED_ID,
    );
    assert.ok(pack);
    ...
    await assert.rejects(
      applyPhase390WancherTwinDragonsRedContent(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase390WancherTwinDragonsRedContent(
      client,
      options,
    );
    assert.deepEqual(
    ...
    for (const candidate of phase390WancherTwinDragonsRedPacks) {
      const state = (
        await rows(
          client,
    ...
    assert.match(String(spec?.material), /Ebonite.*Red Urushi.*Tsuikin/i);
    ...
    assert.match(String(spec?.status), /可购买|available/i);
    ...
    assert.deepEqual(
      ...
      [
        `${PHASE390_TWIN_DRAGONS_RED_ID}:${PHASE390_WANCHER_BRAND_ID}:made_by`,
        `${PHASE390_WANCHER_BRAND_ID}:${PHASE390_TWIN_DRAGONS_RED_ID}:reverse`,
      ],
    );

    const replay = await applyPhase390WancherTwinDragonsRedContent(
      client,
      options,
    );
    assert.deepEqual(
