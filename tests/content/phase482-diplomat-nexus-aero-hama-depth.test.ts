...
import { applyPhase482DiplomatNexusAeroHamaDepth } from "../../scripts/apply-phase482-diplomat-nexus-aero-hama-depth";
import {
  PHASE482_IDS,
  phase482DiplomatNexusAeroHamaDepthPacks,
} from "../../scripts/data/phase482-diplomat-nexus-aero-hama-depth";
import {
...
test("Phase 482 deepens Diplomat Nexus, Diplomat Aero and Platinum Hama no Matsu on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase482-diplomat-aero-hama-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase482-diplomat-aero-hama-test",
  ...
  try {
    ...
    assert.equal(phase482DiplomatNexusAeroHamaDepthPacks.length, 3);
    for (const pack of phase482DiplomatNexusAeroHamaDepthPacks) {
    ...
    await assert.rejects(
      applyPhase482DiplomatNexusAeroHamaDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase482DiplomatNexusAeroHamaDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE482_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE482_IDS)) {
      const pack = phase482DiplomatNexusAeroHamaDepthPacks.find(
    ...
    const replay = await applyPhase482DiplomatNexusAeroHamaDepth(
      client,
      options,
    );
