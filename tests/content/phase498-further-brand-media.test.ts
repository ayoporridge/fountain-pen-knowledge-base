...
import {
  type ApplyPhase498Options,
  applyPhase498FurtherBrandMedia,
  PHASE498_MEDIA_TARGETS,
  PHASE498_MODEL_MEDIA_TARGETS,
} from "../../scripts/apply-phase498-further-brand-media";
import {
...
test("Phase 498 separates five further brand primary media rows", {
  timeout: 420_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL_CATALOG);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase498-media-")),
  );
  ...
  const options: ApplyPhase498Options = {
    workspaceRoot: ROOT,
    reviewer: "phase498-further-brand-media-test",
  ...
  const oldPaths: string[] = PHASE498_MODEL_MEDIA_TARGETS.map(
    (target) => target[3],
  );
  ...
  try {
    ...
    const modelBefore = await client.execute({
      sql: `SELECT ma.entity_id,ma.id,ma.local_path,ma.source_item_id,ma.title FROM media_assets ma WHERE ma.id IN (${PHASE498_MODEL_MEDIA_TARGETS.map(() => "?").join(",")}) ORDER BY ma.id`,
      args: PHASE498_MODEL_MEDIA_TARGETS.map((target) => target[2]),
    });
    ...
    await assert.rejects(
      applyPhase498FurtherBrandMedia(client, {
        ...options,
        env: {
          NODE_ENV: "test",
    ...
    const first = await applyPhase498FurtherBrandMedia(client, options);
    assert.equal(first.changed, true);
    assert.deepEqual(
      first.entities.map((entity) => entity.outcome),
      new Array(PHASE498_MEDIA_TARGETS.length).fill("published"),
    );
    ...
    const second = await applyPhase498FurtherBrandMedia(client, options);
    assert.equal(second.changed, false);
    assert.deepEqual(
      second.entities.map((entity) => entity.outcome),
      new Array(PHASE498_MEDIA_TARGETS.length).fill("noop"),
    );
