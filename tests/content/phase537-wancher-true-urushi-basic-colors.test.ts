...
import {
  type ApplyPhase537Options,
  applyPhase537WancherTrueUrushiBasicColors,
} from "../../scripts/apply-phase537-wancher-true-urushi-basic-colors";
import {
  PHASE537_TARGETS,
  PHASE537_WANCHER_BRAND_ID,
  phase537WancherTrueUrushiBasicColorPacks,
} from "../../scripts/data/phase537-wancher-true-urushi-basic-colors";
import {
...
test("Phase 537 publishes three sourced Wancher True Urushi basic colors on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase537-wancher-")),
  );
  ...
  const options: ApplyPhase537Options = {
    workspaceRoot: ROOT,
    reviewer: "phase537-wancher-true-urushi-basic-colors-test",
  ...
  try {
    ...
    assert.equal(PHASE537_TARGETS.length, 3);
    assert.equal(phase537WancherTrueUrushiBasicColorPacks.length, 3);
    const mediaPaths = new Set<string>();
    for (const pack of phase537WancherTrueUrushiBasicColorPacks) {
    ...
    await assert.rejects(
      applyPhase537WancherTrueUrushiBasicColors(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase537WancherTrueUrushiBasicColors(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase537WancherTrueUrushiBasicColorPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase537WancherTrueUrushiBasicColorPacks.entries()) {
      const target = PHASE537_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE537_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE537_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase537WancherTrueUrushiBasicColors(
      client,
      options,
    );
    assert.deepEqual(
