...
import {
  type ApplyPhase538Options,
  applyPhase538WancherTrueUrushiSeasonalColors,
} from "../../scripts/apply-phase538-wancher-true-urushi-seasonal-colors";
import {
  PHASE538_TARGETS,
  PHASE538_WANCHER_BRAND_ID,
  phase538WancherTrueUrushiSeasonalColorPacks,
} from "../../scripts/data/phase538-wancher-true-urushi-seasonal-colors";
import {
...
test("Phase 538 publishes three sourced Wancher True Urushi seasonal colors on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase538-wancher-")),
  );
  ...
  const options: ApplyPhase538Options = {
    workspaceRoot: ROOT,
    reviewer: "phase538-wancher-true-urushi-seasonal-colors-test",
  ...
  try {
    ...
    assert.equal(PHASE538_TARGETS.length, 3);
    assert.equal(phase538WancherTrueUrushiSeasonalColorPacks.length, 3);
    const mediaPaths = new Set<string>();
    for (const pack of phase538WancherTrueUrushiSeasonalColorPacks) {
      ...
      assert.match(svg, /Urushi|True Urushi/);
    }
    await assert.rejects(
      applyPhase538WancherTrueUrushiSeasonalColors(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase538WancherTrueUrushiSeasonalColors(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase538WancherTrueUrushiSeasonalColorPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase538WancherTrueUrushiSeasonalColorPacks.entries()) {
      const target = PHASE538_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Urushi|True Urushi/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE538_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE538_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase538WancherTrueUrushiSeasonalColors(
      client,
      options,
    );
    assert.deepEqual(
