...
import { applyPhase431BrandDepthRefresh } from "../../scripts/apply-phase431-brand-depth-refresh";
import {
  PHASE431_BRAND_IDS,
  phase431BrandDepthRefreshPacks,
} from "../../scripts/data/phase431-brand-depth-refresh";
import {
...
test("Phase 431 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase431-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase431-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase431BrandDepthRefreshPacks.length, 5);
    for (const pack of phase431BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase431BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase431BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE431_BRAND_IDS),
    );
    ...
    const replay = await applyPhase431BrandDepthRefresh(client, options);
