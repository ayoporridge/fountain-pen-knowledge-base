...
import { applyPhase457HeroFrontierDoricDepth } from "../../scripts/apply-phase457-hero-frontier-doric-depth";
import {
  PHASE457_MODEL_IDS,
  phase457HeroFrontierDoricDepthPacks,
} from "../../scripts/data/phase457-hero-frontier-doric-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-89z-objective-visconti-lava-color-lamy-cp1-l/checkpoint.db",
);

test("Phase 457 deepens Hero 100, Parker Frontier and Eversharp Doric on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase457-hero-frontier-doric-"),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase457-hero-frontier-doric-test",
  ...
  try {
    ...
    assert.equal(phase457HeroFrontierDoricDepthPacks.length, 3);
    for (const pack of phase457HeroFrontierDoricDepthPacks) {
    ...
    await assert.rejects(
      applyPhase457HeroFrontierDoricDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase457HeroFrontierDoricDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE457_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE457_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase457HeroFrontierDoricDepthPacks.find(
    ...
    const replay = await applyPhase457HeroFrontierDoricDepth(client, options);
