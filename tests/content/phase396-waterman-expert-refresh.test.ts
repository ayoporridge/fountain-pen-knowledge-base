...
import {
  type ApplyPhase396Options,
  applyPhase396WatermanExpertRefresh,
} from "../../scripts/apply-phase396-waterman-expert-refresh";
import {
  PHASE396_EXPERT_ID,
  PHASE396_WATERMAN_BRAND_ID,
  phase396WatermanExpertRefreshPacks,
} from "../../scripts/data/phase396-waterman-expert-refresh";
import {
...
test("Phase 396 refreshes Waterman Expert on an owned copy", {
  timeout: 900_000,
}, async () => {
  const keepAlive = setInterval(() => undefined, 1_000);
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase396-expert-")),
  );
  ...
  const options: ApplyPhase396Options = {
    workspaceRoot: ROOT,
    reviewer: "phase396-waterman-expert-refresh-test",
  ...
  try {
    ...
    assert.equal(phase396WatermanExpertRefreshPacks.length, 2);
    const pack = phase396WatermanExpertRefreshPacks.find(
      (candidate) => candidate.entityId === PHASE396_EXPERT_ID,
    );
    assert.ok(pack);
    assert.ok(
    ...
    assert.equal(pack.sources.length, 11);
    assert.ok(
    ...
    await assert.rejects(
      applyPhase396WatermanExpertRefresh(client, {
        ...options,
        env: {
        ...options.env,
        NODE_ENV: "test",
    ...
    const first = await applyPhase396WatermanExpertRefresh(client, options);
    assert.deepEqual(
    ...
    assert.equal(state?.slug, "waterman-expert");
    assert.equal(state?.name, "威迪文 Waterman Expert");
    ...
    const reviews = await rows(
      ...
      [PHASE396_EXPERT_ID, contentHash],
    );
    assert.deepEqual(
    ...
    assert.equal(spec?.brand_entity_id, PHASE396_WATERMAN_BRAND_ID);
    ...
    assert.match(String(spec?.dimensions), /141.*151|12\.6.*27/);
    ...
    assert.equal(
      await scalar(
        ...
        [PHASE396_EXPERT_ID, PHASE396_WATERMAN_BRAND_ID],
      ),
      1,
    );
    assert.equal(
      await scalar(
        ...
        [PHASE396_WATERMAN_BRAND_ID, PHASE396_EXPERT_ID],
      ),
      1,
    );
    ...
    const replay = await applyPhase396WatermanExpertRefresh(client, options);
