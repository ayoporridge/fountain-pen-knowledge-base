...
import { applyPhase487TwsbiVac700rNoodlersCreaperJinhao9019Depth } from "../../scripts/apply-phase487-twsbi-vac700r-noodlers-creaper-jinhao9019-depth";
import {
  PHASE487_IDS,
  phase487TwsbiVac700rNoodlersCreaperJinhao9019DepthPacks,
} from "../../scripts/data/phase487-twsbi-vac700r-noodlers-creaper-jinhao9019-depth";
import {
...
test("Phase 487 deepens TWSBI VAC700R, Noodler's Nib Creaper and Jinhao 9019 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase487-twsbi-noodlers-jinhao-"),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase487-twsbi-noodlers-jinhao-test",
  ...
  try {
    ...
    assert.equal(
      phase487TwsbiVac700rNoodlersCreaperJinhao9019DepthPacks.length,
      3,
    );
    for (const pack of phase487TwsbiVac700rNoodlersCreaperJinhao9019DepthPacks) {
    ...
    await assert.rejects(
      applyPhase487TwsbiVac700rNoodlersCreaperJinhao9019Depth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase487TwsbiVac700rNoodlersCreaperJinhao9019Depth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE487_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE487_IDS)) {
      const pack = phase487TwsbiVac700rNoodlersCreaperJinhao9019DepthPacks.find(
    ...
    const replay =
      await applyPhase487TwsbiVac700rNoodlersCreaperJinhao9019Depth(
        client,
        options,
      );
