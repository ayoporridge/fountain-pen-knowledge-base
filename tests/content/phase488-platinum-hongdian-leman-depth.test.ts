...
import { applyPhase488PlatinumHongdianLemanDepth } from "../../scripts/apply-phase488-platinum-hongdian-leman-depth";
import {
  PHASE488_IDS,
  phase488PlatinumHongdianLemanDepthPacks,
} from "../../scripts/data/phase488-platinum-hongdian-leman-depth";
import {
...
test("Phase 488 deepens Platinum Izumo PIZ-150000PW, HongDian 1843 and Caran d’Ache Léman on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase488-platinum-hongdian-leman-"),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase488-platinum-hongdian-leman-test",
  ...
  try {
    ...
    assert.equal(phase488PlatinumHongdianLemanDepthPacks.length, 3);
    for (const pack of phase488PlatinumHongdianLemanDepthPacks) {
    ...
    await assert.rejects(
      applyPhase488PlatinumHongdianLemanDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase488PlatinumHongdianLemanDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE488_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE488_IDS)) {
      const pack = phase488PlatinumHongdianLemanDepthPacks.find(
    ...
    const replay = await applyPhase488PlatinumHongdianLemanDepth(
      client,
      options,
    );
