...
import { applyPhase464MontegrappaElmo02AsvineV200SailorKDepth } from "../../scripts/apply-phase464-montegrappa-elmo-02-asvine-v200-sailor-k-depth";
import {
  PHASE464_IDS,
  phase464MontegrappaElmo02AsvineV200SailorKDepthPacks,
} from "../../scripts/data/phase464-montegrappa-elmo-02-asvine-v200-sailor-k-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-a7z-deepen-existing-pilot-78g-88g-mr1-and-88/checkpoint.db",
);

test("Phase 464 deepens Montegrappa Elmo 02, Asvine V200 and Sailor King Profit ST on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(
        os.tmpdir(),
        "fpkg-phase464-montegrappa-elmo-02-asvine-v200-sailor-k-",
      ),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase464-montegrappa-elmo-02-asvine-v200-sailor-k-test",
  ...
  try {
    ...
    assert.equal(
      phase464MontegrappaElmo02AsvineV200SailorKDepthPacks.length,
      3,
    );
    for (const pack of phase464MontegrappaElmo02AsvineV200SailorKDepthPacks) {
      assert.ok(
    ...
    await assert.rejects(
      applyPhase464MontegrappaElmo02AsvineV200SailorKDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase464MontegrappaElmo02AsvineV200SailorKDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE464_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE464_IDS)) {
      const pack = phase464MontegrappaElmo02AsvineV200SailorKDepthPacks.find(
    ...
    const replay = await applyPhase464MontegrappaElmo02AsvineV200SailorKDepth(
      client,
      options,
    );
