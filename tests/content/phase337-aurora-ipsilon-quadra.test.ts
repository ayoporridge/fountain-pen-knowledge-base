...
import {
  applyPhase337AuroraIpsilonQuadraContent,
  PHASE337_AURORA_BRAND_ID,
  PHASE337_TARGET_ID,
  PHASE337_TARGET_SLUG,
} from "../../scripts/apply-phase337-aurora-ipsilon-quadra-content";
import { phase337AuroraIpsilonQuadraPacks } from "../../scripts/data/phase337-aurora-ipsilon-quadra";
import {
...
test("Phase 337 publishes Aurora Ipsilon Quadra on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase337-aurora-quadra-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase337-aurora-ipsilon-quadra-test",
  ...
  try {
    ...
    const pack = phase337AuroraIpsilonQuadraPacks.find(
      (item) => item.entityId === PHASE337_TARGET_ID,
    );
    assert.ok(pack);
    assert.ok(
    ...
    await assert.rejects(
      applyPhase337AuroraIpsilonQuadraContent(client, {
        ...options,
        env: { ...options.env, FPKG_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase337AuroraIpsilonQuadraContent(
      client,
      options,
    );
    ...
    assert.match(String(entity?.body_md ?? ""), /墨囊|转换器|quadra/);
    ...
    assert.equal(String(spec?.brand_entity_id), PHASE337_AURORA_BRAND_ID);
    assert.match(String(spec?.nib), /14K|金/);
    assert.match(String(spec?.fill_system), /墨囊|converter|cartridge/i);
    assert.match(String(spec?.material), /树脂|quadra|镀金/);
    ...
    assert.match(
      ...
      /phase337\/aurora\/ipsilon-quadra\.svg/,
    );
    assert.equal(
      await scalar(
        client,
        "SELECT count(*) AS value FROM entity_links WHERE source_id=? AND target_id=? AND link_type='made_by'",
        [PHASE337_TARGET_ID, PHASE337_AURORA_BRAND_ID],
      ),
      1,
    );
    assert.equal(
      await scalar(
        ...
        [PHASE337_AURORA_BRAND_ID, PHASE337_TARGET_ID],
      ),
      1,
    );
    const replay = await applyPhase337AuroraIpsilonQuadraContent(
      client,
      options,
    );
    assert.equal(
      replay.entities.find((item) => item.entityId === PHASE337_TARGET_ID)
        ?.outcome,
      "noop",
    );
  } finally {
