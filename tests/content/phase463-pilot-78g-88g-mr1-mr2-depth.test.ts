...
import { applyPhase463Pilot78g88gMr1Mr2Depth } from "../../scripts/apply-phase463-pilot-78g-88g-mr1-mr2-depth";
import {
  PHASE463_IDS,
  phase463Pilot78g88gMr1Mr2DepthPacks,
} from "../../scripts/data/phase463-pilot-78g-88g-mr1-mr2-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-9wu-deepen-existing-wancher-puchico-wancher-/checkpoint.db",
);

test("Phase 463 deepens Pilot 78G, 88G MR1 and 88G MR2 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase463-pilot-78g-88g-mr1-mr2-"),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase463-pilot-78g-88g-mr1-mr2-test",
  ...
  try {
    ...
    assert.equal(phase463Pilot78g88gMr1Mr2DepthPacks.length, 3);
    for (const pack of phase463Pilot78g88gMr1Mr2DepthPacks) {
      assert.ok(
    ...
    await assert.rejects(
      applyPhase463Pilot78g88gMr1Mr2Depth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase463Pilot78g88gMr1Mr2Depth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE463_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE463_IDS)) {
      const pack = phase463Pilot78g88gMr1Mr2DepthPacks.find(
    ...
    const replay = await applyPhase463Pilot78g88gMr1Mr2Depth(client, options);
