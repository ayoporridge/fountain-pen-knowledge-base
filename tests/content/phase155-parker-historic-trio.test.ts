...
import {
  type ApplyPhase155Options,
  applyPhase155ParkerHistoricTrioContent,
} from "../../scripts/apply-phase155-parker-historic-trio-content";
import {
  PHASE155_IDS,
  phase155ParkerHistoricTrioPacks,
} from "../../scripts/data/phase155-parker-historic-trio";
import {
...
test("Phase 155 publishes canonical Parker 180, 21 and Vacumatic identities on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase155-")),
  );
  ...
  const options: ApplyPhase155Options = {
    workspaceRoot: ROOT_ALIAS,
    reviewer: "phase155-test",
  ...
  try {
    ...
    for (const pack of phase155ParkerHistoricTrioPacks) {
    ...
    await assert.rejects(
      () =>
        applyPhase155ParkerHistoricTrioContent(client, {
          ...options,
          reviewer: " ",
        }),
    ...
    const first = await applyPhase155ParkerHistoricTrioContent(client, options);
    assert.deepEqual(
    ...
    for (const penId of [
      PHASE155_IDS.parker180,
      PHASE155_IDS.parker21,
      PHASE155_IDS.vacumatic,
    ]) {
      assert.equal(
        Number(
          (
    ...
    const replay = await applyPhase155ParkerHistoricTrioContent(
      client,
      options,
    );
    assert.deepEqual(
