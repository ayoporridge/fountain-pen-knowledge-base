...
import { applyPhase547KawecoLiliputDepthContent } from "../../scripts/apply-phase547-kaweco-liliput-depth";
import {
  PHASE547_KAWECO_BRAND_ID,
  PHASE547_KAWECO_LILIPUT_ID,
  phase547KawecoLiliputDepthPacks,
} from "../../scripts/data/phase547-kaweco-liliput-depth";
import {
...
test("Phase 547 deepens the existing Kaweco Liliput entity on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase547-kaweco-liliput-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase547-kaweco-liliput-depth-test",
  ...
  try {
    ...
    assert.equal(phase547KawecoLiliputDepthPacks.length, 2);
    const pack = phase547KawecoLiliputDepthPacks.find(
      (candidate) => candidate.entityId === PHASE547_KAWECO_LILIPUT_ID,
    );
    assert.ok(pack);
    assert.equal(pack.expectedType, "pen");
    assert.equal(pack.expectedSlug, "kaweco-liliput");
    assert.ok(
    ...
    assert.ok(
      pack.sources.some(
        (source) => source.key === "phase547-kaweco-liliput-brass-b-10000866",
      ),
    );
    assert.ok(
      pack.sources.some(
        (source) => source.key === "phase547-kaweco-liliput-refill-guide",
      ),
    );
    assert.ok(
      pack.claims.some(
        (claim) => claim.key === "phase547-current-brass-b-specs",
      ),
    );

    await assert.rejects(
      applyPhase547KawecoLiliputDepthContent(client, {
        ...options,
    ...
    const first = await applyPhase547KawecoLiliputDepthContent(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      [PHASE547_KAWECO_BRAND_ID, PHASE547_KAWECO_LILIPUT_ID],
    );
    ...
    const entity = (
      await client.execute({
        ...
        args: [PHASE547_KAWECO_LILIPUT_ID],
      })
    ).rows[0];
    assert.equal(String(entity?.id), PHASE547_KAWECO_LILIPUT_ID);
    assert.equal(String(entity?.type), "pen");
    assert.equal(String(entity?.slug), "kaweco-liliput");
    ...
    const reverse = (
      await client.execute({
        ...
        args: [PHASE547_KAWECO_BRAND_ID],
      })
    ).rows;
    assert.ok(
      reverse.some(
        (row) => String(row.source_id) === PHASE547_KAWECO_LILIPUT_ID,
      ),
    );
    ...
    const reviews = (
      await client.execute({
        ...
        args: [PHASE547_KAWECO_LILIPUT_ID, hash],
      })
    ...
    const replay = await applyPhase547KawecoLiliputDepthContent(
      client,
      options,
    );
