...
import {
  type ApplyPhase536Options,
  applyPhase536WancherTrueUrushiCoreColors,
} from "../../scripts/apply-phase536-wancher-true-urushi-core-colors";
import {
  PHASE536_TARGETS,
  PHASE536_WANCHER_BRAND_ID,
  phase536WancherTrueUrushiCoreColorPacks,
} from "../../scripts/data/phase536-wancher-true-urushi-core-colors";
import {
...
test("Phase 536 publishes three sourced Wancher True Urushi core colors on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase536-wancher-")),
  );
  ...
  const options: ApplyPhase536Options = {
    workspaceRoot: ROOT,
    reviewer: "phase536-wancher-true-urushi-core-colors-test",
  ...
  try {
    ...
    assert.equal(PHASE536_TARGETS.length, 3);
    assert.equal(phase536WancherTrueUrushiCoreColorPacks.length, 3);
    const mediaPaths = new Set<string>();
    for (const pack of phase536WancherTrueUrushiCoreColorPacks) {
    ...
    await assert.rejects(
      applyPhase536WancherTrueUrushiCoreColors(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase536WancherTrueUrushiCoreColors(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase536WancherTrueUrushiCoreColorPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase536WancherTrueUrushiCoreColorPacks.entries()) {
      const target = PHASE536_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Urushi|Tamenuri/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE536_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE536_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase536WancherTrueUrushiCoreColors(
      client,
      options,
    );
    assert.deepEqual(
