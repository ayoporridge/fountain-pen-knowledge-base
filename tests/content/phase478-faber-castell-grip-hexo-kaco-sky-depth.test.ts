...
import { applyPhase478FaberCastellGripHexoKacoSkyDepth } from "../../scripts/apply-phase478-faber-castell-grip-hexo-kaco-sky-depth";
import {
  PHASE478_IDS,
  phase478FaberCastellGripHexoKacoSkyDepthPacks,
} from "../../scripts/data/phase478-faber-castell-grip-hexo-kaco-sky-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-fmt-phase-477-parker-duofold-classic-centenn/checkpoint.db",
);

test("Phase 478 deepens Faber-Castell Grip/HEXO and KACO SKY on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase478-faber-kaco-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase478-faber-kaco-test",
  ...
  try {
    ...
    assert.equal(phase478FaberCastellGripHexoKacoSkyDepthPacks.length, 3);
    const baselineCounts = new Map<
      string,
      { aliases: number; refs: number; variants: number }
    >();
    for (const pack of phase478FaberCastellGripHexoKacoSkyDepthPacks) {
    ...
    await assert.rejects(
      applyPhase478FaberCastellGripHexoKacoSkyDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase478FaberCastellGripHexoKacoSkyDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE478_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE478_IDS)) {
      const pack = phase478FaberCastellGripHexoKacoSkyDepthPacks.find(
    ...
    const replay = await applyPhase478FaberCastellGripHexoKacoSkyDepth(
      client,
      options,
    );
