...
import { applyPhase436BrandDepthRefresh } from "../../scripts/apply-phase436-brand-depth-refresh";
import {
  PHASE436_BRAND_IDS,
  phase436BrandDepthRefreshPacks,
} from "../../scripts/data/phase436-brand-depth-refresh";
import {
...
test("Phase 436 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase436-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase436-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase436BrandDepthRefreshPacks.length, 5);
    for (const pack of phase436BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase436BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase436BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE436_BRAND_IDS),
    );
    ...
    const replay = await applyPhase436BrandDepthRefresh(client, options);
