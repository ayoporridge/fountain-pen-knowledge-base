...
import { applyPhase472WatermanAllureExceptionDepth } from "../../scripts/apply-phase472-waterman-allure-exception-depth";
import {
  PHASE472_IDS,
  phase472WatermanAllureExceptionDepthPacks,
} from "../../scripts/data/phase472-waterman-allure-exception-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-dz4-phase-471-deepen-existing-waterman-c-f-a/checkpoint.db",
);

test("Phase 472 deepens Waterman Allure and Exception on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase472-waterman-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase472-waterman-allure-exception-test",
  ...
  try {
    ...
    assert.equal(phase472WatermanAllureExceptionDepthPacks.length, 2);
    for (const pack of phase472WatermanAllureExceptionDepthPacks) {
    ...
    await assert.rejects(
      applyPhase472WatermanAllureExceptionDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase472WatermanAllureExceptionDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE472_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE472_IDS)) {
      const pack = phase472WatermanAllureExceptionDepthPacks.find(
    ...
    const replay = await applyPhase472WatermanAllureExceptionDepth(
      client,
      options,
    );
