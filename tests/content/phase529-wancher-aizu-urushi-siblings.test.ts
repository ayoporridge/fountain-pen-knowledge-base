...
import {
  type ApplyPhase529Options,
  applyPhase529WancherAizuUrushiSiblings,
} from "../../scripts/apply-phase529-wancher-aizu-urushi-siblings";
import {
  PHASE529_TARGETS,
  PHASE529_WANCHER_BRAND_ID,
  phase529WancherAizuUrushiPacks,
} from "../../scripts/data/phase529-wancher-aizu-urushi-siblings";
import {
...
test("Phase 529 publishes five sourced Wancher Aizu Urushi siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase529-wancher-")),
  );
  ...
  const options: ApplyPhase529Options = {
    workspaceRoot: ROOT,
    reviewer: "phase529-wancher-aizu-urushi-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE529_TARGETS.length, 5);
    assert.equal(phase529WancherAizuUrushiPacks.length, 5);
    for (const pack of phase529WancherAizuUrushiPacks) {
      ...
      assert.match(
        svg,
        /Aka Tamenuri|Akebono-nuri|Tamamushi|Koma-nuri|Nashiji/,
      );
    }
    await assert.rejects(
      applyPhase529WancherAizuUrushiSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase529WancherAizuUrushiSiblings(client, options);
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase529WancherAizuUrushiPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase529WancherAizuUrushiPacks.entries()) {
      const target = PHASE529_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Aizu|Urushi|漆/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE529_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE529_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase529WancherAizuUrushiSiblings(
      client,
      options,
    );
    assert.deepEqual(
