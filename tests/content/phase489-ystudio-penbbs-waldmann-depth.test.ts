...
import { applyPhase489YstudioPenbbsWaldmannDepth } from "../../scripts/apply-phase489-ystudio-penbbs-waldmann-depth";
import {
  PHASE489_IDS,
  phase489YstudioPenbbsWaldmannDepthPacks,
} from "../../scripts/data/phase489-ystudio-penbbs-waldmann-depth";
import {
...
test("Phase 489 deepens YSTUDIO Classic Revolve, PenBBS 456 and Waldmann Tuscany on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase489-ystudio-penbbs-waldmann-"),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase489-ystudio-penbbs-waldmann-test",
  ...
  try {
    ...
    assert.equal(phase489YstudioPenbbsWaldmannDepthPacks.length, 3);
    for (const pack of phase489YstudioPenbbsWaldmannDepthPacks) {
    ...
    await assert.rejects(
      applyPhase489YstudioPenbbsWaldmannDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase489YstudioPenbbsWaldmannDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE489_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE489_IDS)) {
      const pack = phase489YstudioPenbbsWaldmannDepthPacks.find(
    ...
    const replay = await applyPhase489YstudioPenbbsWaldmannDepth(
      client,
      options,
    );
