...
import { applyPhase462WancherPuchicoShizukuPilotCavalierDepth } from "../../scripts/apply-phase462-wancher-puchico-shizuku-pilot-cavalier-depth";
import {
  PHASE462_IDS,
  phase462WancherPuchicoShizukuPilotCavalierDepthPacks,
} from "../../scripts/data/phase462-wancher-puchico-shizuku-pilot-cavalier-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-9n1-objective-deepen-sailor-1911-l-silver-es/checkpoint.db",
);

test("Phase 462 deepens Wancher PuChiCo, Shizuku and Pilot Cavalier on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(
        os.tmpdir(),
        "fpkg-phase462-wancher-puchico-shizuku-pilot-cavalier-",
      ),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase462-wancher-puchico-shizuku-pilot-cavalier-test",
  ...
  try {
    ...
    assert.equal(
      phase462WancherPuchicoShizukuPilotCavalierDepthPacks.length,
      3,
    );
    for (const pack of phase462WancherPuchicoShizukuPilotCavalierDepthPacks) {
      assert.ok(
    ...
    await assert.rejects(
      applyPhase462WancherPuchicoShizukuPilotCavalierDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase462WancherPuchicoShizukuPilotCavalierDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE462_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE462_IDS)) {
      const pack = phase462WancherPuchicoShizukuPilotCavalierDepthPacks.find(
    ...
    const replay = await applyPhase462WancherPuchicoShizukuPilotCavalierDepth(
      client,
      options,
    );
