...
import {
  type ApplyPhase514Options,
  applyPhase514WancherKieiYuukariHekitame,
} from "../../scripts/apply-phase514-wancher-kiei-yuukari-hekitame";
import {
  PHASE514_WANCHER_BRAND_ID,
  PHASE514_WANCHER_YUUKARI_PRODUCT_URL,
  phase514WancherKieiYuukariHekitamePacks,
} from "../../scripts/data/phase514-wancher-kiei-yuukari-hekitame";
import {
...
test("Phase 514 publishes sourced Wancher Yuukari Hekitame on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase514-wancher-")),
  );
  ...
  const options: ApplyPhase514Options = {
    workspaceRoot: ROOT,
    reviewer: "phase514-wancher-yuukari-hekitame-test",
  ...
  const [pack] = phase514WancherKieiYuukariHekitamePacks;
  assert.ok(pack);
  try {
    ...
    assert.match(svg, /Yuukari Hekitame/);
    assert.match(svg, /Kiei Urushi/);
    assert.match(svg, /Eucalyptus polyanthemos/);
    await assert.rejects(
      applyPhase514WancherKieiYuukariHekitame(client, {
        ...options,
        env: {
          ...options.env,
    ...
    const first = await applyPhase514WancherKieiYuukariHekitame(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      [[pack.entityId, "published"]],
    );
    const publicRow = (
    ...
    assert.match(String(publicRow?.body_md ?? ""), /Yuukari|桉叶/);
    assert.match(String(publicRow?.body_md ?? ""), /Eucalyptus polyanthemos/);
    assert.match(String(publicRow?.body_md ?? ""), /18K/);
    assert.match(String(publicRow?.body_md ?? ""), /Standard Plastic/);
    assert.doesNotMatch(
    ...
    assert.equal(
      (
        await client.execute({
          ...
          args: [pack.entityId, PHASE514_WANCHER_BRAND_ID],
        })
      ).rows[0]?.n,
      1,
    );
    assert.equal(
      (
        await client.execute({
          ...
          args: [PHASE514_WANCHER_BRAND_ID, pack.entityId],
        })
      ).rows[0]?.n,
      1,
    );
    ...
    assert.equal(
      (
        await client.execute({
          ...
          args: [PHASE514_WANCHER_YUUKARI_PRODUCT_URL],
        })
      ).rows[0]?.n,
      1,
    );
    ...
    const replay = await applyPhase514WancherKieiYuukariHekitame(
      client,
      options,
    );
    assert.deepEqual(
