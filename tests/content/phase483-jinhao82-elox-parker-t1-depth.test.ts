...
import { applyPhase483Jinhao82EloxParkerT1Depth } from "../../scripts/apply-phase483-jinhao82-elox-parker-t1-depth";
import {
  PHASE483_IDS,
  phase483Jinhao82EloxParkerT1DepthPacks,
} from "../../scripts/data/phase483-jinhao82-elox-parker-t1-depth";
import {
...
test("Phase 483 deepens Jinhao 82, Diplomat Elox and Parker T-1 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase483-jinhao-elox-parker-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase483-jinhao-elox-parker-test",
  ...
  try {
    ...
    assert.equal(phase483Jinhao82EloxParkerT1DepthPacks.length, 3);
    for (const pack of phase483Jinhao82EloxParkerT1DepthPacks) {
    ...
    await assert.rejects(
      applyPhase483Jinhao82EloxParkerT1Depth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase483Jinhao82EloxParkerT1Depth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE483_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE483_IDS)) {
      const pack = phase483Jinhao82EloxParkerT1DepthPacks.find(
    ...
    const replay = await applyPhase483Jinhao82EloxParkerT1Depth(
      client,
      options,
    );
