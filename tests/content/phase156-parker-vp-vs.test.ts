...
import {
  type ApplyPhase156Options,
  applyPhase156ParkerVpVsContent,
} from "../../scripts/apply-phase156-parker-vp-vs-content";
import {
  PHASE156_IDS,
  phase156ParkerVpVsPacks,
} from "../../scripts/data/phase156-parker-vp-vs";
import {
...
test("Phase 156 publishes canonical Parker VP and VS identities on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase156-")),
  );
  ...
  const options: ApplyPhase156Options = {
    workspaceRoot: ROOT_ALIAS,
    reviewer: "phase156-test",
  ...
  try {
    ...
    for (const pack of phase156ParkerVpVsPacks) {
    ...
    await assert.rejects(
      () =>
        applyPhase156ParkerVpVsContent(client, { ...options, reviewer: " " }),
      /reviewer must not be empty/,
    );
    await assert.rejects(
      () =>
        applyPhase156ParkerVpVsContent(client, {
          ...options,
          env: {
            NODE_ENV: "test",
            TURSO_DATABASE_URL: "",
    ...
    const first = await applyPhase156ParkerVpVsContent(client, options);
    assert.deepEqual(
    ...
    for (const penId of [PHASE156_IDS.vp, PHASE156_IDS.vs]) {
      assert.equal(
        Number(
          (
            await rows(
    ...
    const replay = await applyPhase156ParkerVpVsContent(client, options);
    assert.deepEqual(
