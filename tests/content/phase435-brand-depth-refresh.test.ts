...
import { applyPhase435BrandDepthRefresh } from "../../scripts/apply-phase435-brand-depth-refresh";
import {
  PHASE435_BRAND_IDS,
  phase435BrandDepthRefreshPacks,
} from "../../scripts/data/phase435-brand-depth-refresh";
import {
...
test("Phase 435 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase435-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase435-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase435BrandDepthRefreshPacks.length, 5);
    for (const pack of phase435BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase435BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase435BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE435_BRAND_IDS),
    );
    ...
    const replay = await applyPhase435BrandDepthRefresh(client, options);
