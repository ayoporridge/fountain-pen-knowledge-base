...
import { applyPhase426BrandDepthRefresh } from "../../scripts/apply-phase426-brand-depth-refresh";
import {
  PHASE426_BRAND_IDS,
  phase426BrandDepthRefreshPacks,
} from "../../scripts/data/phase426-brand-depth-refresh";
import {
...
test("Phase 426 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase426-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase426-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase426BrandDepthRefreshPacks.length, 5);
    for (const pack of phase426BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase426BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase426BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE426_BRAND_IDS),
    );
    ...
    const replay = await applyPhase426BrandDepthRefresh(client, options);
