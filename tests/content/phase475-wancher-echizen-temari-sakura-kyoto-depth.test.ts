...
import { applyPhase475WancherEchizenTemariSakuraKyotoDepth } from "../../scripts/apply-phase475-wancher-echizen-temari-sakura-kyoto-depth";
import {
  PHASE475_IDS,
  phase475WancherEchizenTemariSakuraKyotoDepthPacks,
} from "../../scripts/data/phase475-wancher-echizen-temari-sakura-kyoto-depth";
import {
...
test("Phase 475 deepens Wancher Temari, Sakura Zukiyo and Kyoto Ume on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase475-wancher-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase475-wancher-echizen-test",
  ...
  try {
    ...
    assert.equal(phase475WancherEchizenTemariSakuraKyotoDepthPacks.length, 3);
    const baselineCounts = new Map<
      string,
      { aliases: number; refs: number; variants: number }
    >();
    for (const pack of phase475WancherEchizenTemariSakuraKyotoDepthPacks) {
    ...
    await assert.rejects(
      applyPhase475WancherEchizenTemariSakuraKyotoDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase475WancherEchizenTemariSakuraKyotoDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE475_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE475_IDS)) {
      const pack = phase475WancherEchizenTemariSakuraKyotoDepthPacks.find(
    ...
    const replay = await applyPhase475WancherEchizenTemariSakuraKyotoDepth(
      client,
      options,
    );
