...
import { applyPhase458DiplomatMagnumOpusOmarTwsbiSwipeDepth } from "../../scripts/apply-phase458-diplomat-magnum-opus-omar-twsbi-swipe-depth";
import {
  PHASE458_MODEL_IDS,
  phase458DiplomatMagnumOpusOmarTwsbiSwipeDepthPacks,
} from "../../scripts/data/phase458-diplomat-magnum-opus-omar-twsbi-swipe-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-8ld-objective-deepen-hero-100-parker-frontie/checkpoint.db",
);

test("Phase 458 deepens Diplomat Magnum, Opus 88 Omar and TWSBI Swipe on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase458-diplomat-opus-twsbi-"),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase458-diplomat-opus-twsbi-test",
  ...
  try {
    ...
    assert.equal(phase458DiplomatMagnumOpusOmarTwsbiSwipeDepthPacks.length, 3);
    for (const pack of phase458DiplomatMagnumOpusOmarTwsbiSwipeDepthPacks) {
    ...
    await assert.rejects(
      applyPhase458DiplomatMagnumOpusOmarTwsbiSwipeDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase458DiplomatMagnumOpusOmarTwsbiSwipeDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE458_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE458_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase458DiplomatMagnumOpusOmarTwsbiSwipeDepthPacks.find(
    ...
    const replay = await applyPhase458DiplomatMagnumOpusOmarTwsbiSwipeDepth(
      client,
      options,
    );
