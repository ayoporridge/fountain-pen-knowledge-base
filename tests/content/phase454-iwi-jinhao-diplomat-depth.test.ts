...
import { applyPhase454IwiJinhaoDiplomatDepth } from "../../scripts/apply-phase454-iwi-jinhao-diplomat-depth";
import {
  PHASE454_MODEL_IDS,
  phase454IwiJinhaoDiplomatDepthPacks,
} from "../../scripts/data/phase454-iwi-jinhao-diplomat-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-7ft-objective-deepen-existing-parker-premier/checkpoint.db",
);

test("Phase 454 deepens IWI Laureate, Jinhao X159 and Diplomat Viper on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase454-iwi-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase454-iwi-jinhao-diplomat-test",
  ...
  try {
    ...
    assert.equal(phase454IwiJinhaoDiplomatDepthPacks.length, 3);
    ...
    await assert.rejects(
      applyPhase454IwiJinhaoDiplomatDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase454IwiJinhaoDiplomatDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE454_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE454_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase454IwiJinhaoDiplomatDepthPacks.find(
    ...
    const replay = await applyPhase454IwiJinhaoDiplomatDepth(client, options);
