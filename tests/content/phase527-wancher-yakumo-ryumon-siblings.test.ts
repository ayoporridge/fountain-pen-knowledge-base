...
import {
  type ApplyPhase527Options,
  applyPhase527WancherYakumoRyumonSiblings,
} from "../../scripts/apply-phase527-wancher-yakumo-ryumon-siblings";
import {
  PHASE527_TARGETS,
  PHASE527_WANCHER_BRAND_ID,
  phase527WancherYakumoRyumonPacks,
} from "../../scripts/data/phase527-wancher-yakumo-ryumon-siblings";
import {
...
test("Phase 527 publishes three sourced Wancher Yakumo Ryumon siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase527-wancher-")),
  );
  ...
  const options: ApplyPhase527Options = {
    workspaceRoot: ROOT,
    reviewer: "phase527-wancher-yakumo-ryumon-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE527_TARGETS.length, 3);
    assert.equal(phase527WancherYakumoRyumonPacks.length, 3);
    for (const pack of phase527WancherYakumoRyumonPacks) {
      ...
      assert.match(svg, /Amaterasu|Susano|Orochi/);
    }
    await assert.rejects(
      applyPhase527WancherYakumoRyumonSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase527WancherYakumoRyumonSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase527WancherYakumoRyumonPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase527WancherYakumoRyumonPacks.entries()) {
      const target = PHASE527_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Yakumo|Ryumon|Urushi/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE527_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE527_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase527WancherYakumoRyumonSiblings(
      client,
      options,
    );
    assert.deepEqual(
