...
import { applyPhase460DiplomatTravellerEsteemClrDepth } from "../../scripts/apply-phase460-diplomat-traveller-esteem-clr-depth";
import {
  PHASE460_DIPLOMAT_IDS,
  phase460DiplomatTravellerEsteemClrDepthPacks,
} from "../../scripts/data/phase460-diplomat-traveller-esteem-clr-depth";
import {
...
test("Phase 460 deepens Diplomat Traveller, Esteem and CLR on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(os.tmpdir(), "fpkg-phase460-diplomat-traveller-esteem-clr-"),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase460-diplomat-traveller-esteem-clr-test",
  ...
  try {
    ...
    assert.equal(phase460DiplomatTravellerEsteemClrDepthPacks.length, 3);
    for (const pack of phase460DiplomatTravellerEsteemClrDepthPacks) {
      assert.ok(
    ...
    await assert.rejects(
      applyPhase460DiplomatTravellerEsteemClrDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase460DiplomatTravellerEsteemClrDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE460_DIPLOMAT_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE460_DIPLOMAT_IDS)) {
      const pack = phase460DiplomatTravellerEsteemClrDepthPacks.find(
    ...
    const replay = await applyPhase460DiplomatTravellerEsteemClrDepth(
      client,
      options,
    );
