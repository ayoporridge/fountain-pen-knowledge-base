...
import { applyPhase285PelikanM101NContent } from "../../scripts/apply-phase285-pelikan-m101n-content";
import {
  PHASE285_M101N_ID,
  PHASE285_M101N_SLUG,
  PHASE285_PELIKAN_ID,
  phase285PelikanM101NPacks,
} from "../../scripts/data/phase285-pelikan-m101n";
import {
...
test("Phase 285 publishes Pelikan M101N on an owned copy", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const protectedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase285-pelikan-protected-")),
  );
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase285-pelikan-owned-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase285-pelikan-m101n-test",
  ...
  try {
    ...
    assert.equal(phase285PelikanM101NPacks.length, 2);
    const pack = phase285PelikanM101NPacks.find(
      (item) => item.entityId === PHASE285_M101N_ID,
    );
    assert.ok(pack);
    assert.ok(
    ...
    await assert.rejects(
      () =>
        applyPhase285PelikanM101NContent(client, {
          ...options,
          env: { ...options.env, FPKG_DATABASE_URL: "libsql://remote" },
        }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase285PelikanM101NContent(client, options);
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      [
        [PHASE285_PELIKAN_ID, "published"],
        [PHASE285_M101N_ID, "published"],
      ],
    );
    const row = (
      await client.execute({
        sql: "SELECT id,type,slug,name,body_md FROM public_entities WHERE id=?",
        args: [PHASE285_M101N_ID],
      })
    ).rows[0];
    assert.deepEqual(
      ...
      [
        ...
        "pen",
        PHASE285_M101N_SLUG,
        "Pelikan M101N Special & Limited Editions",
      ],
    );
    ...
    const hash = await computePublicationContentHash(client, PHASE285_M101N_ID);
    assert.deepEqual(
      (
        await client.execute({
          sql: "SELECT review_kind,status FROM entity_content_reviews WHERE entity_id=? AND content_hash=? ORDER BY review_kind",
          args: [PHASE285_M101N_ID, hash],
        })
      ...
      Number(
        (
          await client.execute({
            ...
            args: [PHASE285_M101N_ID, PHASE285_PELIKAN_ID],
          })
        ).rows[0]?.value,
      ),
      1,
      ...
      Number(
        (
          await client.execute({
            ...
            args: [PHASE285_PELIKAN_ID, PHASE285_M101N_ID],
          })
        ).rows[0]?.value,
      ),
      1,
    ...
    assert.ok(
      (await applyPhase285PelikanM101NContent(client, options)).entities.every(
