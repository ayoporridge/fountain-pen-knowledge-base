...
import { applyPhase484ChiltonWingflowDuke551Hero616Depth } from "../../scripts/apply-phase484-chilton-wingflow-duke551-hero616-depth";
import {
  PHASE484_IDS,
  phase484ChiltonWingflowDuke551Hero616DepthPacks,
} from "../../scripts/data/phase484-chilton-wingflow-duke551-hero616-depth";
import {
...
test("Phase 484 deepens Chilton Wing-flow, Duke 551 Confucius and Hero 616 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase484-chilton-duke-hero-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase484-chilton-duke-hero-test",
  ...
  try {
    ...
    assert.equal(phase484ChiltonWingflowDuke551Hero616DepthPacks.length, 3);
    for (const pack of phase484ChiltonWingflowDuke551Hero616DepthPacks) {
    ...
    await assert.rejects(
      applyPhase484ChiltonWingflowDuke551Hero616Depth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase484ChiltonWingflowDuke551Hero616Depth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE484_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE484_IDS)) {
      const pack = phase484ChiltonWingflowDuke551Hero616DepthPacks.find(
    ...
    const replay = await applyPhase484ChiltonWingflowDuke551Hero616Depth(
      client,
      options,
    );
