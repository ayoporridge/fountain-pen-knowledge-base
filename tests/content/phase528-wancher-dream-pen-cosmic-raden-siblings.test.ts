...
import {
  type ApplyPhase528Options,
  applyPhase528WancherDreamPenCosmicRadenSiblings,
} from "../../scripts/apply-phase528-wancher-dream-pen-cosmic-raden-siblings";
import {
  PHASE528_TARGETS,
  PHASE528_WANCHER_BRAND_ID,
  phase528WancherDreamPenCosmicRadenPacks,
} from "../../scripts/data/phase528-wancher-dream-pen-cosmic-raden-siblings";
import {
...
test("Phase 528 publishes five sourced Wancher Dream Pen Cosmic Raden siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase528-wancher-")),
  );
  ...
  const options: ApplyPhase528Options = {
    workspaceRoot: ROOT,
    reviewer: "phase528-wancher-dream-pen-cosmic-raden-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE528_TARGETS.length, 5);
    assert.equal(phase528WancherDreamPenCosmicRadenPacks.length, 5);
    for (const pack of phase528WancherDreamPenCosmicRadenPacks) {
      ...
      assert.match(svg, /Nebula|Meteor Shower|Comets|Asteroid Belt|Supernova/);
    }
    await assert.rejects(
      applyPhase528WancherDreamPenCosmicRadenSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase528WancherDreamPenCosmicRadenSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase528WancherDreamPenCosmicRadenPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase528WancherDreamPenCosmicRadenPacks.entries()) {
      const target = PHASE528_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(
        ...
        /Cosmic|Raden|Maki-e|Urushi/,
      );
      assert.match(String(publicRow?.body_md ?? ""), /18K|Wancher/);
      assert.match(
        String(publicRow?.body_md ?? ""),
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE528_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE528_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase528WancherDreamPenCosmicRadenSiblings(
      client,
      options,
    );
    assert.deepEqual(
