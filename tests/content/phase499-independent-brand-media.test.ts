...
import {
  type ApplyPhase499Options,
  applyPhase499IndependentBrandMedia,
  PHASE499_MEDIA_TARGETS,
  PHASE499_MODEL_MEDIA_TARGETS,
} from "../../scripts/apply-phase499-independent-brand-media";
import {
...
test("Phase 499 separates five independent-brand primary media rows", {
  timeout: 420_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL_CATALOG);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase499-media-")),
  );
  ...
  const options: ApplyPhase499Options = {
    workspaceRoot: ROOT,
    reviewer: "phase499-independent-brand-media-test",
  ...
  const oldPaths: string[] = PHASE499_MODEL_MEDIA_TARGETS.map(
    (target) => target[3],
  );
  ...
  try {
    ...
    const modelBefore = await client.execute({
      sql: `SELECT ma.entity_id,ma.id,ma.local_path,ma.source_item_id,ma.title FROM media_assets ma WHERE ma.id IN (${PHASE499_MODEL_MEDIA_TARGETS.map(() => "?").join(",")}) ORDER BY ma.id`,
      args: PHASE499_MODEL_MEDIA_TARGETS.map((target) => target[2]),
    });
    ...
    await assert.rejects(
      applyPhase499IndependentBrandMedia(client, {
        ...options,
        env: {
          NODE_ENV: "test",
    ...
    const first = await applyPhase499IndependentBrandMedia(client, options);
    assert.equal(first.changed, true);
    assert.deepEqual(
      first.entities.map((entity) => entity.outcome),
      new Array(PHASE499_MEDIA_TARGETS.length).fill("published"),
    );
    ...
    const second = await applyPhase499IndependentBrandMedia(client, options);
    assert.equal(second.changed, false);
    assert.deepEqual(
      second.entities.map((entity) => entity.outcome),
      new Array(PHASE499_MEDIA_TARGETS.length).fill("noop"),
    );
