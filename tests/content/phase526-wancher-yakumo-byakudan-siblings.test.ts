...
import {
  type ApplyPhase526Options,
  applyPhase526WancherYakumoByakudanSiblings,
} from "../../scripts/apply-phase526-wancher-yakumo-byakudan-siblings";
import {
  PHASE526_TARGETS,
  PHASE526_WANCHER_BRAND_ID,
  phase526WancherYakumoByakudanPacks,
} from "../../scripts/data/phase526-wancher-yakumo-byakudan-siblings";
import {
...
test("Phase 526 publishes three sourced Wancher Yakumo Byakudan siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase526-wancher-")),
  );
  ...
  const options: ApplyPhase526Options = {
    workspaceRoot: ROOT,
    reviewer: "phase526-wancher-yakumo-byakudan-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE526_TARGETS.length, 3);
    assert.equal(phase526WancherYakumoByakudanPacks.length, 3);
    for (const pack of phase526WancherYakumoByakudanPacks) {
      ...
      assert.match(svg, /Yae|Black|Red/);
    }
    await assert.rejects(
      applyPhase526WancherYakumoByakudanSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase526WancherYakumoByakudanSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase526WancherYakumoByakudanPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase526WancherYakumoByakudanPacks.entries()) {
      const target = PHASE526_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Yakumo|Byakudan|Urushi/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE526_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE526_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase526WancherYakumoByakudanSiblings(
      client,
      options,
    );
    assert.deepEqual(
