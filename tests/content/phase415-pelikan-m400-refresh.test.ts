...
import {
  type ApplyPhase415Options,
  applyPhase415PelikanM400Refresh,
  PHASE415_M400_ID,
  PHASE415_M400_SLUG,
  PHASE415_PELIKAN_BRAND_ID,
} from "../../scripts/apply-phase415-pelikan-m400-refresh";
import { phase415PelikanM400RefreshPacks } from "../../scripts/data/phase415-pelikan-m400-refresh";
...
test("Phase 415 deepens the existing Pelikan Souverän M400 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const keepAlive = setInterval(() => undefined, 1_000);
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase415-pelikan-m400-")),
  );
  ...
  const options: ApplyPhase415Options = {
    workspaceRoot: ROOT,
    reviewer: "phase415-pelikan-m400-refresh-test",
  ...
  try {
    ...
    assert.equal(pack.entityId, PHASE415_M400_ID);
    assert.equal(pack.expectedSlug, PHASE415_M400_SLUG);
    ...
    assert.match(svg, /Souverän M400/);
    assert.match(svg, /非产品照片/);
    assert.match(svg, /不按比例/);

    await assert.rejects(
      applyPhase415PelikanM400Refresh(client, { ...options, reviewer: " " }),
      /reviewer must not be empty/,
    );
    await assert.rejects(
      applyPhase415PelikanM400Refresh(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase415PelikanM400Refresh(client, options);
    assert.deepEqual(
    ...
    assert.equal(page?.slug, PHASE415_M400_SLUG);
    assert.equal(page?.name, "Pelikan Souverän M400");
    ...
    const spec = (
      await rows(
        client,
        "SELECT brand_entity_id,nib,fill_system,material,dimensions,weight,price_range,status FROM model_specs WHERE entity_id=?",
        [PHASE415_M400_ID],
      )
    )[0];
    assert.equal(spec?.brand_entity_id, PHASE415_PELIKAN_BRAND_ID);
    assert.match(String(spec?.nib), /14K.*585.*EF.*F.*M.*B/);
    assert.match(String(spec?.fill_system), /活塞/);
    assert.match(String(spec?.material), /cellulose acetate|树脂|条纹/);
    assert.match(String(spec?.dimensions), /12\.7|11\.7/);
    assert.match(String(spec?.weight), /14\.9|14\.0|15\.3/);
    assert.match(String(spec?.price_range), /€430|430/);
    assert.match(String(spec?.status), /Old Style|New Style|1997/);
    ...
    assert.ok(
      variants.filter((row) => row.parent_variant_id !== null).length >= 8,
    );
    ...
    assert.equal(
      await scalar(
        ...
        [PHASE415_M400_ID, PHASE415_PELIKAN_BRAND_ID],
      ),
      1,
    );
    assert.equal(
      await scalar(
        ...
        [PHASE415_PELIKAN_BRAND_ID, PHASE415_M400_ID],
      ),
      1,
    );
    assert.equal(
    ...
    const replay = await applyPhase415PelikanM400Refresh(client, options);
    assert.deepEqual(replay.entities, [
      { entityId: PHASE415_M400_ID, outcome: "noop", contentHash },
    ]);
