...
import { applyPhase451SheafferDepth } from "../../scripts/apply-phase451-sheaffer-touchdown-craftsman-depth";
import {
  PHASE451_MODEL_IDS,
  phase451SheafferTouchdownCraftsmanPacks,
} from "../../scripts/data/phase451-sheaffer-touchdown-craftsman-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-6tj-sheaffer-balance-snorkel-pfm-depth-owned-checkpoint/checkpoint.db",
);

test("Phase 451 deepens Sheaffer Touchdown TM and Craftsman variants on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase451-sheaffer-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase451-sheaffer-test",
  ...
  try {
    ...
    assert.equal(phase451SheafferTouchdownCraftsmanPacks.length, 3);
    for (const pack of phase451SheafferTouchdownCraftsmanPacks) {
    ...
    await assert.rejects(
      applyPhase451SheafferDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase451SheafferDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE451_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE451_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase451SheafferTouchdownCraftsmanPacks.find(
    ...
    const replay = await applyPhase451SheafferDepth(client, options);
