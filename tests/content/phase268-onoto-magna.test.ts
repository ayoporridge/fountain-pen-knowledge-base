...
import { applyPhase268OnotoMagnaContent } from "../../scripts/apply-phase268-onoto-magna-content";
import {
  PHASE268_MAGNA_ID,
  PHASE268_ONOTO_BRAND_ID,
  phase268OnotoMagnaPacks,
} from "../../scripts/data/phase268-onoto-magna";
import {
...
test("Phase 268 publishes Onoto Magna on an owned copy", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase268-onoto-magna-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase268-onoto-magna-test",
  ...
  try {
    ...
    assert.equal(phase268OnotoMagnaPacks.length, 2);
    const modelPack = phase268OnotoMagnaPacks.find(
      (pack) => pack.entityId === PHASE268_MAGNA_ID,
    );
    assert.ok(modelPack);
    assert.ok(
    ...
    await assert.rejects(
      () =>
        applyPhase268OnotoMagnaContent(client, {
          ...options,
          env: { ...options.env, FPKG_DATABASE_URL: "libsql://remote" },
        }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase268OnotoMagnaContent(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      [PHASE268_ONOTO_BRAND_ID, PHASE268_MAGNA_ID],
    );
    const row = (
      await client.execute({
        sql: "SELECT id,type,slug,name,body_md FROM public_entities WHERE id=?",
        args: [PHASE268_MAGNA_ID],
      })
    ).rows[0];
    assert.deepEqual(
      [row?.id, row?.type, row?.slug, row?.name],
      [PHASE268_MAGNA_ID, "pen", "onoto-magna", "Onoto Magna"],
    );
    ...
    const hash = await computePublicationContentHash(client, PHASE268_MAGNA_ID);
    assert.deepEqual(
      (
        await client.execute({
          sql: "SELECT review_kind,status FROM entity_content_reviews WHERE entity_id=? AND content_hash=? ORDER BY review_kind",
          args: [PHASE268_MAGNA_ID, hash],
        })
      ...
      Number(
        (
          await client.execute({
            ...
            args: [PHASE268_MAGNA_ID, PHASE268_ONOTO_BRAND_ID],
          })
        ).rows[0]?.value,
      ),
      1,
      ...
      Number(
        (
          await client.execute({
            ...
            args: [
              `rev-phase268-made-by-${PHASE268_MAGNA_ID}`,
              PHASE268_ONOTO_BRAND_ID,
              PHASE268_MAGNA_ID,
            ],
          })
    ...
    assert.ok(
      (await applyPhase268OnotoMagnaContent(client, options)).entities.every(
