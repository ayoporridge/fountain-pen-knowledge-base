...
import { applyPhase434BrandDepthRefresh } from "../../scripts/apply-phase434-brand-depth-refresh";
import {
  PHASE434_BRAND_IDS,
  phase434BrandDepthRefreshPacks,
} from "../../scripts/data/phase434-brand-depth-refresh";
import {
...
test("Phase 434 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase434-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase434-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase434BrandDepthRefreshPacks.length, 5);
    for (const pack of phase434BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase434BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase434BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE434_BRAND_IDS),
    );
    ...
    const replay = await applyPhase434BrandDepthRefresh(client, options);
