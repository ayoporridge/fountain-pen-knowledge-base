...
import {
  type ApplyPhase578Options,
  applyPhase578WancherTsuikinKanhizakura,
} from "../../scripts/apply-phase578-wancher-tsuikin-kanhizakura-depth";
import {
  PHASE578_TARGET_ID,
  PHASE578_WANCHER_BRAND_ID,
  phase578WancherTsuikinKanhizakuraPacks,
} from "../../scripts/data/phase578-wancher-tsuikin-kanhizakura-depth";
import {
...
test("Phase 578 deepens Wancher Tsuikin Kanhizakura on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const protectedHash = sha256(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase578-wancher-tsuikin-")),
  );
  ...
  const options: ApplyPhase578Options = {
    workspaceRoot: ROOT,
    reviewer: "phase578-wancher-tsuikin-kanhizakura-depth-test",
  ...
  try {
    ...
    assert.ok((pack.claims?.length ?? 0) >= 20);
    assert.ok((pack.variants?.length ?? 0) >= 10);
    for (const phrase of [
      "9327396454615",
      "tsuikin-kanhizakuras",
      "WF-TSOU-SAK-RD",
      "US$6,886",
      "Ebonite",
      "Red Urushi",
    ...
    assert.match(svg, /Kanhizakura/);

    const existing = await rows(
      client,
      "SELECT id,type,slug,name FROM entities WHERE id=? OR slug=? ORDER BY id",
      [PHASE578_TARGET_ID, pack.expectedSlug],
    );
    assert.deepEqual(
      existing.map((row) => [row.id, row.type, row.slug, row.name]),
      [[PHASE578_TARGET_ID, "pen", pack.expectedSlug, pack.canonicalName]],
    );
    await assert.rejects(
      applyPhase578WancherTsuikinKanhizakura(client, {
        ...options,
        env: {
          ...options.env,
    ...
    const first = await applyPhase578WancherTsuikinKanhizakura(client, options);
    assert.equal(first.entityId, PHASE578_TARGET_ID);
    ...
    assert.match(
      String(state?.source),
      /^curated-content:phase578-wancher-tsuikin-kanhizakura-depth-v1:/,
    );
      ...
      Number(
        (
          await rows(
            ...
            [PHASE578_TARGET_ID, PHASE578_WANCHER_BRAND_ID],
          )
        )[0]?.n,
      ),
      1,
      ...
      Number(
        (
          await rows(
            ...
            [PHASE578_WANCHER_BRAND_ID, PHASE578_TARGET_ID],
          )
        )[0]?.n,
      ),
      1,
    ...
    const replay = await applyPhase578WancherTsuikinKanhizakura(
      client,
      options,
    );
