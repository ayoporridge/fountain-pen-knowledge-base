...
import {
  type ApplyPhase144Options,
  applyPhase144OttoHuttContent,
} from "../../scripts/apply-phase144-otto-hutt-design04-design07-content";
import {
  PHASE144_IDS,
  PHASE144_SLUGS,
  phase144Packs,
} from "../../scripts/data/phase144-otto-hutt-design04-design07-batch";
import {
...
test("Phase 144 publishes Otto Hutt brand, design04 and design07 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const keepAlive = setInterval(() => undefined, 1_000);
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase144-")),
  );
  ...
  const options: ApplyPhase144Options = {
    workspaceRoot: ROOT_ALIAS,
    reviewer: "phase144-test",
  ...
  try {
    ...
    assert.equal(phase144Packs.length, 3);
    for (const pack of phase144Packs) {
    ...
    await assert.rejects(
      () => applyPhase144OttoHuttContent(client, { ...options, reviewer: " " }),
      /reviewer must not be empty/,
    );
    await assert.rejects(
      () =>
        applyPhase144OttoHuttContent(client, {
          ...options,
          env: {
            NODE_ENV: "test",
            TURSO_DATABASE_URL: "",
    ...
    const first = await applyPhase144OttoHuttContent(client, options);
    assert.deepEqual(
      ...
      Number(
        (
          await rows(
            ...
            [PHASE144_IDS.design04, PHASE144_IDS.brand],
          )
        )[0]?.n,
      ),
      1,
      ...
      Number(
        (
          await rows(
            client,
            "SELECT count(*) n FROM entity_links WHERE source_id=? AND target_id=? AND link_type='made_by'",
            [PHASE144_IDS.design07, PHASE144_IDS.brand],
          )
        )[0]?.n,
      ),
      1,
    ...
    assert.equal(
      Number(
        (
          await rows(
            client,
            "SELECT count(*) n FROM entity_links WHERE source_id=? AND target_id IN (?,?) AND link_type='reverse'",
            [PHASE144_IDS.brand, PHASE144_IDS.design04, PHASE144_IDS.design07],
          )
        )[0]?.n,
      ),
      2,
    );
    const replay = await applyPhase144OttoHuttContent(client, options);
    assert.deepEqual(
    ...
    const identities = await rows(
      ...
      [PHASE144_SLUGS.brand, PHASE144_SLUGS.design04, PHASE144_SLUGS.design07],
    );
    assert.deepEqual(
      identities,
      [
        { id: PHASE144_IDS.brand, type: "brand", slug: PHASE144_SLUGS.brand },
        {
          id: PHASE144_IDS.design04,
          type: "pen",
          slug: PHASE144_SLUGS.design04,
        },
        {
          id: PHASE144_IDS.design07,
          type: "pen",
          slug: PHASE144_SLUGS.design07,
        },
