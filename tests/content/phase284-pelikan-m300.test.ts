...
import { applyPhase284PelikanM300Content } from "../../scripts/apply-phase284-pelikan-m300-content";
import {
  PHASE284_M300_ID,
  PHASE284_M300_SLUG,
  PHASE284_PELIKAN_ID,
  phase284PelikanM300Packs,
} from "../../scripts/data/phase284-pelikan-m300";
import {
...
test("Phase 284 publishes Pelikan Souverän M300 on an owned copy", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const protectedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase284-pelikan-protected-")),
  );
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase284-pelikan-owned-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase284-pelikan-m300-test",
  ...
  try {
    ...
    assert.equal(phase284PelikanM300Packs.length, 2);
    const pack = phase284PelikanM300Packs.find(
      (item) => item.entityId === PHASE284_M300_ID,
    );
    assert.ok(pack);
    assert.ok(
    ...
    await assert.rejects(
      () =>
        applyPhase284PelikanM300Content(client, {
          ...options,
          env: { ...options.env, FPKG_DATABASE_URL: "libsql://remote" },
        }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase284PelikanM300Content(client, options);
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      [
        [PHASE284_PELIKAN_ID, "published"],
        [PHASE284_M300_ID, "published"],
      ],
    );
    const row = (
      await client.execute({
        sql: "SELECT id,type,slug,name,body_md FROM public_entities WHERE id=?",
        args: [PHASE284_M300_ID],
      })
    ).rows[0];
    assert.deepEqual(
      [row?.id, row?.type, row?.slug, row?.name],
      [PHASE284_M300_ID, "pen", PHASE284_M300_SLUG, "Pelikan Souverän M300"],
    );
    ...
    const hash = await computePublicationContentHash(client, PHASE284_M300_ID);
    assert.deepEqual(
      (
        await client.execute({
          sql: "SELECT review_kind,status FROM entity_content_reviews WHERE entity_id=? AND content_hash=? ORDER BY review_kind",
          args: [PHASE284_M300_ID, hash],
        })
      ...
      Number(
        (
          await client.execute({
            ...
            args: [PHASE284_M300_ID, PHASE284_PELIKAN_ID],
          })
        ).rows[0]?.value,
      ),
      1,
      ...
      Number(
        (
          await client.execute({
            ...
            args: [PHASE284_PELIKAN_ID, PHASE284_M300_ID],
          })
        ).rows[0]?.value,
      ),
      1,
    ...
    assert.ok(
      (await applyPhase284PelikanM300Content(client, options)).entities.every(
