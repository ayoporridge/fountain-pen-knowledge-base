...
} from "../../scripts/data/phase524-wancher-yakumo-chijimi-oita";
import {
...
test("Phase 524 publishes four sourced Wancher Yakumo Chijimi/Oita siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase524-wancher-")),
  );
  ...
  const options: ApplyPhase524Options = {
    workspaceRoot: ROOT,
    reviewer: "phase524-wancher-yakumo-chijimi-oita-test",
  ...
  try {
    ...
    assert.equal(PHASE524_TARGETS.length, 4);
    assert.equal(phase524WancherYakumoChijimiOitaPacks.length, 4);
    for (const pack of phase524WancherYakumoChijimiOitaPacks) {
      ...
      assert.match(svg, /Shirohebi|Nishikihebi|Aodaisho|Kurozan/);
    }
    await assert.rejects(
      applyPhase524WancherYakumoChijimiOita(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase524WancherYakumoChijimiOita(client, options);
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase524WancherYakumoChijimiOitaPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase524WancherYakumoChijimiOitaPacks.entries()) {
      const target = PHASE524_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Yakumo|Oita|Urushi/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE524_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE524_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase524WancherYakumoChijimiOita(client, options);
    assert.deepEqual(
