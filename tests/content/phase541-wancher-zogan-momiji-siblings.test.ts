...
import {
  type ApplyPhase541Options,
  applyPhase541WancherZoganMomijiSiblings,
} from "../../scripts/apply-phase541-wancher-zogan-momiji-siblings";
import {
  PHASE541_TARGETS,
  PHASE541_WANCHER_BRAND_ID,
  phase541WancherZoganMomijiSiblingPacks,
} from "../../scripts/data/phase541-wancher-zogan-momiji-siblings";
import {
...
test("Phase 541 publishes three sourced Wancher Zogan Momiji siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase541-wancher-")),
  );
  ...
  const options: ApplyPhase541Options = {
    workspaceRoot: ROOT,
    reviewer: "phase541-wancher-zogan-momiji-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE541_TARGETS.length, 3);
    assert.equal(phase541WancherZoganMomijiSiblingPacks.length, 3);
    const mediaPaths = new Set<string>();
    for (const pack of phase541WancherZoganMomijiSiblingPacks) {
      ...
      assert.match(svg, /Momiji|Zogan/);
    }
    await assert.rejects(
      applyPhase541WancherZoganMomijiSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase541WancherZoganMomijiSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase541WancherZoganMomijiSiblingPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase541WancherZoganMomijiSiblingPacks.entries()) {
      const target = PHASE541_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Momiji|Zogan/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE541_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE541_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase541WancherZoganMomijiSiblings(
      client,
      options,
    );
    assert.deepEqual(
