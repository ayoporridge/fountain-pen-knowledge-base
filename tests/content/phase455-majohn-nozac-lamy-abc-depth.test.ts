...
import { applyPhase455MajohnNozacLamyAbcDepth } from "../../scripts/apply-phase455-majohn-nozac-lamy-abc-depth";
import {
  PHASE455_MODEL_IDS,
  phase455MajohnNozacLamyAbcDepthPacks,
} from "../../scripts/data/phase455-majohn-nozac-lamy-abc-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-7ol-objective-deepen-existing-iwi-jinhao-diplomat/checkpoint.db",
);

test("Phase 455 deepens Majohn V1, Conklin Nozac and LAMY abc on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase455-majohn-nozac-lamy-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase455-majohn-nozac-lamy-abc-test",
  ...
  try {
    ...
    assert.equal(phase455MajohnNozacLamyAbcDepthPacks.length, 3);
    ...
    await assert.rejects(
      applyPhase455MajohnNozacLamyAbcDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase455MajohnNozacLamyAbcDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE455_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE455_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase455MajohnNozacLamyAbcDepthPacks.find(
    ...
    const replay = await applyPhase455MajohnNozacLamyAbcDepth(client, options);
