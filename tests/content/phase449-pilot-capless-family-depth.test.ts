...
import { applyPhase449ModelDepth } from "../../scripts/apply-phase449-pilot-capless-family-depth";
import {
  PHASE449_MODEL_IDS,
  phase449PilotCaplessFamilyPacks,
} from "../../scripts/data/phase449-pilot-capless-family-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-6f2-opus88-leonardo-model-depth-owned-checkpoint/checkpoint.db",
);

test("Phase 449 deepens Pilot Capless family on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase449-capless-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase449-pilot-capless-test",
  ...
  try {
    ...
    assert.equal(phase449PilotCaplessFamilyPacks.length, 3);
    for (const pack of phase449PilotCaplessFamilyPacks) {
    ...
    await assert.rejects(
      applyPhase449ModelDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase449ModelDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE449_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE449_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase449PilotCaplessFamilyPacks.find(
    ...
    const replay = await applyPhase449ModelDepth(client, options);
