...
import {
  type ApplyPhase534Options,
  applyPhase534WancherUrushiESiblings,
} from "../../scripts/apply-phase534-wancher-urushi-e-siblings";
import {
  PHASE534_TARGETS,
  PHASE534_WANCHER_BRAND_ID,
  phase534WancherUrushiEPacks,
} from "../../scripts/data/phase534-wancher-urushi-e-siblings";
import {
...
test("Phase 534 publishes three sourced Wancher Urushi-e siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase534-wancher-")),
  );
  ...
  const options: ApplyPhase534Options = {
    workspaceRoot: ROOT,
    reviewer: "phase534-wancher-urushi-e-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE534_TARGETS.length, 3);
    assert.equal(phase534WancherUrushiEPacks.length, 3);
    for (const pack of phase534WancherUrushiEPacks) {
      ...
      assert.match(svg, /Urushi-e|Dragon|Tiger|Persimmon/);
    }
    await assert.rejects(
      applyPhase534WancherUrushiESiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase534WancherUrushiESiblings(client, options);
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase534WancherUrushiEPacks.map((pack) => [pack.entityId, "published"]),
    );
    for (const [index, pack] of phase534WancherUrushiEPacks.entries()) {
      const target = PHASE534_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Urushi-e|Urushi|漆绘/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE534_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE534_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase534WancherUrushiESiblings(client, options);
    assert.deepEqual(
