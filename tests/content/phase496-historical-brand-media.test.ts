...
import {
  type ApplyPhase496Options,
  applyPhase496HistoricalBrandMedia,
  PHASE496_MEDIA_TARGETS,
  PHASE496_MODEL_MEDIA_TARGETS,
} from "../../scripts/apply-phase496-historical-brand-media";
import {
...
test("Phase 496 separates five historical brand primary media rows", {
  timeout: 420_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL_CATALOG);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase496-media-")),
  );
  ...
  const options: ApplyPhase496Options = {
    workspaceRoot: ROOT,
    reviewer: "phase496-historical-brand-media-test",
  ...
  const oldPaths: string[] = PHASE496_MODEL_MEDIA_TARGETS.map(
    (target) => target[3],
  );
  ...
  try {
    ...
    const modelBefore = await client.execute({
      sql: `SELECT ma.entity_id,ma.id,ma.local_path,ma.source_item_id,ma.title FROM media_assets ma WHERE ma.id IN (${PHASE496_MODEL_MEDIA_TARGETS.map(() => "?").join(",")}) ORDER BY ma.id`,
      args: PHASE496_MODEL_MEDIA_TARGETS.map((target) => target[2]),
    });
    ...
    await assert.rejects(
      applyPhase496HistoricalBrandMedia(client, {
        ...options,
        env: {
          NODE_ENV: "test",
    ...
    const first = await applyPhase496HistoricalBrandMedia(client, options);
    assert.equal(first.changed, true);
    assert.deepEqual(
      first.entities.map((entity) => entity.outcome),
      new Array(PHASE496_MEDIA_TARGETS.length).fill("published"),
    );
    ...
    const second = await applyPhase496HistoricalBrandMedia(client, options);
    assert.equal(second.changed, false);
    assert.deepEqual(
      second.entities.map((entity) => entity.outcome),
      new Array(PHASE496_MEDIA_TARGETS.length).fill("noop"),
    );
