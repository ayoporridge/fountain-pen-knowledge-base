...
import { applyPhase443BrandDepthRefresh } from "../../scripts/apply-phase443-brand-depth-refresh";
import {
  PHASE443_BRAND_IDS,
  phase443BrandDepthRefreshPacks,
} from "../../scripts/data/phase443-brand-depth-refresh";
import {
...
test("Phase 443 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase443-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase443-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase443BrandDepthRefreshPacks.length, 5);
    for (const pack of phase443BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase443BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase443BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE443_BRAND_IDS),
    );
    ...
    const replay = await applyPhase443BrandDepthRefresh(client, options);
