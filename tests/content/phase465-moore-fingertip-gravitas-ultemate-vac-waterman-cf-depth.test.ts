...
import { applyPhase465MooreFingertipGravitasUltemateVacWatermanCfDepth } from "../../scripts/apply-phase465-moore-fingertip-gravitas-ultemate-vac-waterman-cf-depth";
import {
  PHASE465_IDS,
  phase465MooreFingertipGravitasUltemateVacWatermanCfDepthPacks,
} from "../../scripts/data/phase465-moore-fingertip-gravitas-ultemate-vac-waterman-cf-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-ajc-montegrappa-elmo-02-asvine-v200-sailor-k/checkpoint.db",
);

test("Phase 465 deepens selected Moore Fingertip, Gravitas Ultemate Vac and Waterman C/F on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(
      path.join(
        os.tmpdir(),
        "fpkg-phase465-moore-fingertip-gravitas-ultemate-vac-waterman-cf-",
      ),
    ),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase465-moore-fingertip-gravitas-ultemate-vac-waterman-cf-test",
  ...
  try {
    ...
    assert.equal(
      phase465MooreFingertipGravitasUltemateVacWatermanCfDepthPacks.length,
      3,
    );
    for (const pack of phase465MooreFingertipGravitasUltemateVacWatermanCfDepthPacks) {
      assert.ok(
    ...
    await assert.rejects(
      applyPhase465MooreFingertipGravitasUltemateVacWatermanCfDepth(client, {
        ...options,
    ...
    const first =
      await applyPhase465MooreFingertipGravitasUltemateVacWatermanCfDepth(
        client,
        options,
      );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE465_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE465_IDS)) {
      const pack =
        phase465MooreFingertipGravitasUltemateVacWatermanCfDepthPacks.find(
