...
import { applyPhase456ViscontiLavaColorLamyCp1Lily910Depth } from "../../scripts/apply-phase456-visconti-lava-color-lamy-cp1-lily-910-depth";
import {
  PHASE456_MODEL_IDS,
  phase456ViscontiLavaColorLamyCp1Lily910DepthPacks,
} from "../../scripts/data/phase456-visconti-lava-color-lamy-cp1-lily-910-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-7y3-objective-deepen-majohn-nozac-lamy-abc/checkpoint.db",
);

test("Phase 456 deepens Visconti Lava Color, LAMY cp1 and Lily 910 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase456-visconti-lamy-lily-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase456-visconti-lamy-lily-test",
  ...
  try {
    ...
    assert.equal(phase456ViscontiLavaColorLamyCp1Lily910DepthPacks.length, 3);
    ...
    await assert.rejects(
      applyPhase456ViscontiLavaColorLamyCp1Lily910Depth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase456ViscontiLavaColorLamyCp1Lily910Depth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE456_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE456_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase456ViscontiLavaColorLamyCp1Lily910DepthPacks.find(
    ...
    const replay = await applyPhase456ViscontiLavaColorLamyCp1Lily910Depth(
      client,
      options,
    );
