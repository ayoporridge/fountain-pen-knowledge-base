...
import { applyPhase490TwsbiDiamondGoDepth } from "../../scripts/apply-phase490-twsbi-diamond-go-depth";
import {
  PHASE490_IDS,
  phase490TwsbiDiamondGoDepthPacks,
} from "../../scripts/data/phase490-twsbi-diamond-go-depth";
import {
...
test("Phase 490 deepens TWSBI Diamond 580, Diamond 580ALR and GO on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase490-twsbi-diamond-go-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase490-twsbi-diamond-go-test",
  ...
  try {
    ...
    assert.equal(phase490TwsbiDiamondGoDepthPacks.length, 3);
    for (const pack of phase490TwsbiDiamondGoDepthPacks) {
      ...
      assert.ok(
        new Set(pack.sources.map((item) => item.independenceGroup)).size >= 2,
        `${pack.expectedSlug} needs independent source groups`,
      );
    }
    await assert.rejects(
      applyPhase490TwsbiDiamondGoDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase490TwsbiDiamondGoDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE490_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE490_IDS)) {
      const pack = phase490TwsbiDiamondGoDepthPacks.find(
    ...
    const replay = await applyPhase490TwsbiDiamondGoDepth(client, options);
