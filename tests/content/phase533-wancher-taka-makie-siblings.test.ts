...
import {
  type ApplyPhase533Options,
  applyPhase533WancherTakaMakieSiblings,
} from "../../scripts/apply-phase533-wancher-taka-makie-siblings";
import {
  PHASE533_TARGETS,
  PHASE533_WANCHER_BRAND_ID,
  phase533WancherTakaMakiePacks,
} from "../../scripts/data/phase533-wancher-taka-makie-siblings";
import {
...
test("Phase 533 publishes three sourced Wancher Taka Maki-e siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase533-wancher-")),
  );
  ...
  const options: ApplyPhase533Options = {
    workspaceRoot: ROOT,
    reviewer: "phase533-wancher-taka-makie-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE533_TARGETS.length, 3);
    assert.equal(phase533WancherTakaMakiePacks.length, 3);
    for (const pack of phase533WancherTakaMakiePacks) {
      ...
      assert.match(svg, /Taka|Maki-e|Mejiro|Narihira|Praying Mantis/);
    }
    await assert.rejects(
      applyPhase533WancherTakaMakieSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase533WancherTakaMakieSiblings(client, options);
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase533WancherTakaMakiePacks.map((pack) => [pack.entityId, "published"]),
    );
    for (const [index, pack] of phase533WancherTakaMakiePacks.entries()) {
      const target = PHASE533_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Taka|Maki-e|蒔绘/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE533_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE533_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase533WancherTakaMakieSiblings(client, options);
    assert.deepEqual(
