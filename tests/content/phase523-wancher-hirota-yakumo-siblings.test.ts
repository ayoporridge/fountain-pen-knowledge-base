...
import {
  type ApplyPhase523Options,
  applyPhase523WancherHirotaYakumoSiblings,
} from "../../scripts/apply-phase523-wancher-hirota-yakumo-siblings";
import {
  PHASE523_TARGETS,
  PHASE523_WANCHER_BRAND_ID,
  phase523WancherHirotaYakumoPacks,
} from "../../scripts/data/phase523-wancher-hirota-yakumo-siblings";
import {
...
test("Phase 523 publishes four sourced Wancher Hirota/Yakumo siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase523-wancher-")),
  );
  ...
  const options: ApplyPhase523Options = {
    workspaceRoot: ROOT,
    reviewer: "phase523-wancher-hirota-yakumo-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE523_TARGETS.length, 4);
    assert.equal(phase523WancherHirotaYakumoPacks.length, 4);
    for (const pack of phase523WancherHirotaYakumoPacks) {
      ...
      assert.match(svg, /Sabi Nuri|Chawan|Momo|Ryuusei|Chijimi/);
    }
    await assert.rejects(
      applyPhase523WancherHirotaYakumoSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase523WancherHirotaYakumoSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase523WancherHirotaYakumoPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase523WancherHirotaYakumoPacks.entries()) {
      const target = PHASE523_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Hirota|Yakumo|Urushi/);
      assert.match(String(publicRow?.body_md ?? ""), /JoWo|Shogun/);
      assert.match(
        String(publicRow?.body_md ?? ""),
        /converter|Converter|Sailor Standard/,
      );
      assert.doesNotMatch(
        String(publicRow?.body_md ?? ""),
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE523_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase523WancherHirotaYakumoSiblings(
      client,
      options,
    );
    assert.deepEqual(
