...
import { applyPhase438BrandDepthRefresh } from "../../scripts/apply-phase438-brand-depth-refresh";
import {
  PHASE438_BRAND_IDS,
  phase438BrandDepthRefreshPacks,
} from "../../scripts/data/phase438-brand-depth-refresh";
import {
...
test("Phase 438 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase438-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase438-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase438BrandDepthRefreshPacks.length, 5);
    for (const pack of phase438BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase438BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase438BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE438_BRAND_IDS),
    );
    ...
    const replay = await applyPhase438BrandDepthRefresh(client, options);
