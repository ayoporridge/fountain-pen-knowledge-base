...
import { applyPhase442BrandDepthRefresh } from "../../scripts/apply-phase442-brand-depth-refresh";
import {
  PHASE442_BRAND_IDS,
  phase442BrandDepthRefreshPacks,
} from "../../scripts/data/phase442-brand-depth-refresh";
import {
...
test("Phase 442 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase442-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase442-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase442BrandDepthRefreshPacks.length, 5);
    for (const pack of phase442BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase442BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase442BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE442_BRAND_IDS),
    );
    ...
    const replay = await applyPhase442BrandDepthRefresh(client, options);
