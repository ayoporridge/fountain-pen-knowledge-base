...
import {
  type ApplyPhase145Options,
  applyPhase145WaldmannTuscanyContent,
} from "../../scripts/apply-phase145-waldmann-tuscany-content";
import {
  PHASE145_IDS,
  PHASE145_SLUGS,
  phase145Packs,
} from "../../scripts/data/phase145-waldmann-tuscany-batch";
import {
...
test("Phase 145 publishes Waldmann Tuscany on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const keepAlive = setInterval(() => undefined, 1_000);
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase145-")),
  );
  ...
  const options: ApplyPhase145Options = {
    workspaceRoot: ROOT_ALIAS,
    reviewer: "phase145-test",
  ...
  try {
    ...
    assert.equal(phase145Packs.length, 2);
    for (const pack of phase145Packs) {
    ...
    await assert.rejects(
      () =>
        applyPhase145WaldmannTuscanyContent(client, {
          ...options,
          reviewer: " ",
        }),
    ...
    const first = await applyPhase145WaldmannTuscanyContent(client, options);
    assert.deepEqual(
      ...
      Number(
        (
          await rows(
            ...
            [PHASE145_IDS.tuscany, PHASE145_IDS.brand],
          )
        )[0]?.n,
      ),
      1,
      ...
      Number(
        (
          await rows(
            ...
            [PHASE145_IDS.brand, PHASE145_IDS.tuscany],
          )
        )[0]?.n,
      ),
      1,
    );
    const replay = await applyPhase145WaldmannTuscanyContent(client, options);
    assert.deepEqual(
    ...
    const identities = await rows(
      ...
      [PHASE145_SLUGS.brand, PHASE145_SLUGS.tuscany],
    );
    assert.deepEqual(identities, [
      { id: PHASE145_IDS.brand, type: "brand", slug: PHASE145_SLUGS.brand },
      { id: PHASE145_IDS.tuscany, type: "pen", slug: PHASE145_SLUGS.tuscany },
    ]);
