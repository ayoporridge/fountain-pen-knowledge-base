...
import {
  type ApplyPhase532Options,
  applyPhase532WancherRisingSunMakieSiblings,
} from "../../scripts/apply-phase532-wancher-rising-sun-makie-siblings";
import {
  PHASE532_TARGETS,
  PHASE532_WANCHER_BRAND_ID,
  phase532WancherRisingSunMakiePacks,
} from "../../scripts/data/phase532-wancher-rising-sun-makie-siblings";
import {
...
test("Phase 532 publishes two sourced Wancher Rising Sun Maki-e siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase532-wancher-")),
  );
  ...
  const options: ApplyPhase532Options = {
    workspaceRoot: ROOT,
    reviewer: "phase532-wancher-rising-sun-makie-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE532_TARGETS.length, 2);
    assert.equal(phase532WancherRisingSunMakiePacks.length, 2);
    for (const pack of phase532WancherRisingSunMakiePacks) {
      ...
      assert.match(svg, /Sansui|Dragon|Rising Sun/);
    }
    await assert.rejects(
      applyPhase532WancherRisingSunMakieSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase532WancherRisingSunMakieSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase532WancherRisingSunMakiePacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase532WancherRisingSunMakiePacks.entries()) {
      const target = PHASE532_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE532_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE532_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase532WancherRisingSunMakieSiblings(
      client,
      options,
    );
    assert.deepEqual(
