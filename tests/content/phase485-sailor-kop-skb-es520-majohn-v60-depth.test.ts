...
import { applyPhase485SailorKopSkbEs520MajohnV60Depth } from "../../scripts/apply-phase485-sailor-kop-skb-es520-majohn-v60-depth";
import {
  PHASE485_IDS,
  phase485SailorKopSkbEs520MajohnV60DepthPacks,
} from "../../scripts/data/phase485-sailor-kop-skb-es520-majohn-v60-depth";
import {
...
test("Phase 485 deepens Sailor Professional Gear KOP, SKB ES-520 and Majohn V60 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase485-sailor-skb-majohn-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase485-sailor-skb-majohn-test",
  ...
  try {
    ...
    assert.equal(phase485SailorKopSkbEs520MajohnV60DepthPacks.length, 3);
    for (const pack of phase485SailorKopSkbEs520MajohnV60DepthPacks) {
    ...
    await assert.rejects(
      applyPhase485SailorKopSkbEs520MajohnV60Depth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase485SailorKopSkbEs520MajohnV60Depth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE485_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE485_IDS)) {
      const pack = phase485SailorKopSkbEs520MajohnV60DepthPacks.find(
    ...
    const replay = await applyPhase485SailorKopSkbEs520MajohnV60Depth(
      client,
      options,
    );
