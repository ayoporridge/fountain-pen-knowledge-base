...
import {
  type ApplyPhase513Options,
  applyPhase513WancherKieiRemainingSiblings,
} from "../../scripts/apply-phase513-wancher-kiei-remaining-siblings";
import {
  PHASE513_TARGETS,
  PHASE513_WANCHER_BRAND_ID,
  phase513WancherKieiRemainingSiblingPacks,
} from "../../scripts/data/phase513-wancher-kiei-remaining-siblings";
import {
...
test("Phase 513 publishes two sourced Wancher Kiei sibling SKUs on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase513-wancher-")),
  );
  ...
  const options: ApplyPhase513Options = {
    workspaceRoot: ROOT,
    reviewer: "phase513-wancher-kiei-remaining-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE513_TARGETS.length, 2);
    assert.equal(phase513WancherKieiRemainingSiblingPacks.length, 2);
    for (const pack of phase513WancherKieiRemainingSiblingPacks) {
      ...
      assert.match(svg, /Kiei/);
      assert.match(svg, /Urushi/);
    }
    await assert.rejects(
      applyPhase513WancherKieiRemainingSiblings(client, {
        ...options,
        env: {
          ...options.env,
    ...
    const first = await applyPhase513WancherKieiRemainingSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase513WancherKieiRemainingSiblingPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [
      index,
      pack,
    ] of phase513WancherKieiRemainingSiblingPacks.entries()) {
      const target = PHASE513_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Kiei|季映/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE513_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE513_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase513WancherKieiRemainingSiblings(
      client,
      options,
    );
    assert.deepEqual(
