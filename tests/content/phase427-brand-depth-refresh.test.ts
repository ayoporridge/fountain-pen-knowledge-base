...
import { applyPhase427BrandDepthRefresh } from "../../scripts/apply-phase427-brand-depth-refresh";
import {
  PHASE427_BRAND_IDS,
  phase427BrandDepthRefreshPacks,
} from "../../scripts/data/phase427-brand-depth-refresh";
import {
...
test("Phase 427 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase427-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase427-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase427BrandDepthRefreshPacks.length, 5);
    for (const pack of phase427BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase427BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase427BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE427_BRAND_IDS),
    );
    ...
    const replay = await applyPhase427BrandDepthRefresh(client, options);
