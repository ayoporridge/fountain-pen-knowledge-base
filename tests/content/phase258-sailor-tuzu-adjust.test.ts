...
import { applyPhase258SailorTuzuAdjustContent } from "../../scripts/apply-phase258-sailor-tuzu-adjust-content";
import {
  PHASE258_SAILOR_BRAND_ID,
  PHASE258_TUZU_ID,
  PHASE258_TUZU_SLUG,
  phase258SailorTuzuAdjustPacks,
} from "../../scripts/data/phase258-sailor-tuzu-adjust";
import {
...
test("Phase 258 publishes Sailor TUZU ADJUST with exact Sailor topology", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase258-sailor-tuzu-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase258-sailor-tuzu-adjust-test",
  ...
  try {
    ...
    assert.equal(phase258SailorTuzuAdjustPacks.length, 2);
    const pack = phase258SailorTuzuAdjustPacks.find(
      (item) => item.entityId === PHASE258_TUZU_ID,
    );
    assert.ok(pack);
    assert.ok(
    ...
    await assert.rejects(
      applyPhase258SailorTuzuAdjustContent(client, {
        ...options,
    ...
    assert.deepEqual(
      (
        await applyPhase258SailorTuzuAdjustContent(client, options)
      ).entities.map((item) => [item.entityId, item.outcome]),
      [
        [PHASE258_SAILOR_BRAND_ID, "published"],
        [PHASE258_TUZU_ID, "published"],
      ],
    );
    const row = (
    ...
    assert.deepEqual(
      ...
      [PHASE258_TUZU_ID, "pen", PHASE258_TUZU_SLUG],
    );
    ...
    for (const pattern of [
      /TUZU ADJUST/,
      /10°|10 度/,
      /Rotating Nib Feature/i,
      /Natural Fit Grip/i,
      /不锈钢/,
      /F\/M\/B/,
      /cartridge.*converter|墨囊.*转换器/i,
      /recycled PC/i,
      /135/,
      /20\.0/,
      /Forge/,
    ...
    assert.equal(
      (
        await client.execute({
          ...
          args: [PHASE258_SAILOR_BRAND_ID, PHASE258_TUZU_ID],
        })
      ).rows[0]?.value,
      1,
    );
    ...
    assert.equal(
      (
        await client.execute({
          ...
          args: [PHASE258_TUZU_ID, localPath],
        })
      ).rows[0]?.value,
      1,
    );
    const hash = await computePublicationContentHash(client, PHASE258_TUZU_ID);
    assert.deepEqual(
      (
        await client.execute({
          sql: "SELECT review_kind,status FROM entity_content_reviews WHERE entity_id=? AND content_hash=? ORDER BY review_kind",
          args: [PHASE258_TUZU_ID, hash],
        })
