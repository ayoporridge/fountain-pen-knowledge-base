...
import { applyPhase468WatermanCommandoPatricianTaperiteDepth } from "../../scripts/apply-phase468-waterman-commando-patrician-taperite-depth";
import {
  PHASE468_IDS,
  phase468WatermanCommandoPatricianTaperiteDepthPacks,
} from "../../scripts/data/phase468-waterman-commando-patrician-taperite-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-ctt-deepen-existing-majohn-m2-cleo-skribent-/checkpoint.db",
);

test("Phase 468 deepens Waterman Commando, Patrician and Taperite on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase468-waterman-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase468-waterman-commando-patrician-taperite-test",
  ...
  try {
    ...
    assert.equal(phase468WatermanCommandoPatricianTaperiteDepthPacks.length, 3);
    for (const pack of phase468WatermanCommandoPatricianTaperiteDepthPacks) {
      assert.ok(
      ...
      const body =
        fs
          .readFileSync(path.join(ROOT, pack.markdownFile), "utf8")
          .match(/## body_md\n([\s\S]*?)(?=\n## )/)?.[1] ?? "";
      assert.ok(body.length >= 2_600, `${pack.expectedSlug} body is too short`);
      assert.ok(
    ...
    await assert.rejects(
      applyPhase468WatermanCommandoPatricianTaperiteDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase468WatermanCommandoPatricianTaperiteDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE468_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE468_IDS)) {
      const pack = phase468WatermanCommandoPatricianTaperiteDepthPacks.find(
    ...
    const replay = await applyPhase468WatermanCommandoPatricianTaperiteDepth(
      client,
      options,
    );
