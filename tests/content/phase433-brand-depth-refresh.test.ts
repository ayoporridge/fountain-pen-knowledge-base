...
import { applyPhase433BrandDepthRefresh } from "../../scripts/apply-phase433-brand-depth-refresh";
import {
  PHASE433_BRAND_IDS,
  phase433BrandDepthRefreshPacks,
} from "../../scripts/data/phase433-brand-depth-refresh";
import {
...
test("Phase 433 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase433-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase433-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase433BrandDepthRefreshPacks.length, 5);
    for (const pack of phase433BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase433BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase433BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE433_BRAND_IDS),
    );
    ...
    const replay = await applyPhase433BrandDepthRefresh(client, options);
