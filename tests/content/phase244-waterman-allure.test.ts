...
import {
  type ApplyPhase244Options,
  applyPhase244WatermanAllureContent,
} from "../../scripts/apply-phase244-waterman-allure-content";
import {
  PHASE244_PEN_ID,
  PHASE244_PEN_SLUG,
  PHASE244_WATERMAN_ID,
  phase244WatermanAllurePacks,
} from "../../scripts/data/phase244-waterman-allure";
import {
...
test("Phase 244 publishes Waterman Allure without mixing writing modes", {
  timeout: 900_000,
}, async () => {
  const keepAlive = setInterval(() => undefined, 1_000);
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase244-waterman-")),
  );
  ...
  const options: ApplyPhase244Options = {
    workspaceRoot: ROOT_ALIAS,
    reviewer: "phase244-waterman-allure-test",
  ...
  try {
    ...
    assert.equal(phase244WatermanAllurePacks.length, 2);
    const pack = phase244WatermanAllurePacks.find(
      (candidate) => candidate.entityId === PHASE244_PEN_ID,
    );
    assert.ok(pack);
    ...
    await assert.rejects(
      () =>
        applyPhase244WatermanAllureContent(client, {
          ...options,
          reviewer: " ",
        }),
    ...
    const first = await applyPhase244WatermanAllureContent(client, options);
    assert.deepEqual(
    ...
    assert.deepEqual(
      ...
      [[PHASE244_PEN_ID, "pen", PHASE244_PEN_SLUG]],
    );
    ...
    assert.equal(
      await scalar(
        ...
        [PHASE244_PEN_ID, PHASE244_WATERMAN_ID],
      ),
      1,
    );
    assert.equal(
      await scalar(
        ...
        [PHASE244_WATERMAN_ID, PHASE244_PEN_ID],
      ),
      1,
    );
    assert.equal(
    ...
    const replay = await applyPhase244WatermanAllureContent(client, options);
    assert.deepEqual(
