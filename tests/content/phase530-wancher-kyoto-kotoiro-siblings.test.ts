...
import {
  type ApplyPhase530Options,
  applyPhase530WancherKyotoKotoiroSiblings,
} from "../../scripts/apply-phase530-wancher-kyoto-kotoiro-siblings";
import {
  PHASE530_TARGETS,
  PHASE530_WANCHER_BRAND_ID,
  phase530WancherKyotoKotoiroPacks,
} from "../../scripts/data/phase530-wancher-kyoto-kotoiro-siblings";
import {
...
test("Phase 530 publishes five sourced Wancher Kyoto Kotoiro siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase530-wancher-")),
  );
  ...
  const options: ApplyPhase530Options = {
    workspaceRoot: ROOT,
    reviewer: "phase530-wancher-kyoto-kotoiro-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE530_TARGETS.length, 5);
    assert.equal(phase530WancherKyotoKotoiroPacks.length, 5);
    for (const pack of phase530WancherKyotoKotoiroPacks) {
      ...
      assert.match(
        svg,
        /Cherry Blossom|Fushimi Inari|Byodoin|Arashiyama|Kinkakuji/,
      );
    }
    await assert.rejects(
      applyPhase530WancherKyotoKotoiroSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase530WancherKyotoKotoiroSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase530WancherKyotoKotoiroPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase530WancherKyotoKotoiroPacks.entries()) {
      const target = PHASE530_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Kyoto|Urushi|金箔/);
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE530_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE530_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase530WancherKyotoKotoiroSiblings(
      client,
      options,
    );
    assert.deepEqual(
