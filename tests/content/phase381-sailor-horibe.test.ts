...
import { applyPhase381SailorHoribeContent } from "../../scripts/apply-phase381-sailor-horibe-content";
import {
  PHASE381_HORIBE_ID,
  PHASE381_HORIBE_SLUG,
  PHASE381_SAILOR_BRAND_ID,
  phase381SailorHoribePacks,
} from "../../scripts/data/phase381-sailor-horibe-yahei";
import {
...
test("Phase 381 publishes Sailor Horibe Yahei Kanemaru 10-9891 on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase381-sailor-horibe-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase381-sailor-horibe-test",
  ...
  try {
    ...
    const pack = phase381SailorHoribePacks.find(
    ...
    await assert.rejects(
      applyPhase381SailorHoribeContent(client, {
        ...options,
        env: {
          ...options.env,
    ...
    const first = await applyPhase381SailorHoribeContent(client, options);
    assert.equal(
      first.entities.find((item) => item.entityId === PHASE381_HORIBE_ID)
        ?.outcome,
      "published",
    );
    const entity = (
      await client.execute({
        ...
        args: [PHASE381_HORIBE_ID],
      })
    ).rows[0];
    assert.equal(String(entity?.type), "pen");
    assert.equal(String(entity?.slug), PHASE381_HORIBE_SLUG);
    assert.equal(
      String(entity?.name),
      "写乐 Sailor KOP Maki-e Ukiyo-e Horibe Yahei Kanemaru（10-9891）",
    );
    const body = String(entity?.body_md ?? "");
    for (const pattern of [
      /Horibe Yahei Kanemaru/i,
      /10-9891/,
      /歌川国芳|Utagawa Kuniyoshi/i,
      /堀部矢兵卫/,
      /赤穗|Ako/i,
      /Chushingura/i,
      /KOP/,
    ...
    assert.equal(String(spec?.brand_entity_id), PHASE381_SAILOR_BRAND_ID);
      ...
      Number(
        (
          await client.execute({
            ...
            args: [PHASE381_HORIBE_ID, PHASE381_SAILOR_BRAND_ID],
          })
        ).rows[0]?.n,
      ),
      1,
      ...
      Number(
        (
          await client.execute({
            ...
            args: [PHASE381_SAILOR_BRAND_ID, PHASE381_HORIBE_ID],
          })
        ).rows[0]?.n,
      ),
      1,
    );
    const replay = await applyPhase381SailorHoribeContent(client, options);
    assert.equal(
      replay.entities.find((item) => item.entityId === PHASE381_HORIBE_ID)
        ?.outcome,
      "noop",
    );
