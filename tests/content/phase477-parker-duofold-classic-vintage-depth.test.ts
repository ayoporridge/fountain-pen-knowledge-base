...
import { applyPhase477ParkerDuofoldClassicVintageDepth } from "../../scripts/apply-phase477-parker-duofold-classic-vintage-depth";
import {
  PHASE477_IDS,
  phase477ParkerDuofoldClassicVintageDepthPacks,
} from "../../scripts/data/phase477-parker-duofold-classic-vintage-depth";
import {
...
test("Phase 477 deepens Parker Duofold vintage and Classic Centennial on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase477-parker-duofold-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase477-parker-duofold-test",
  ...
  try {
    ...
    assert.equal(phase477ParkerDuofoldClassicVintageDepthPacks.length, 2);
    const baselineCounts = new Map<
      string,
      { aliases: number; refs: number; variants: number }
    >();
    for (const pack of phase477ParkerDuofoldClassicVintageDepthPacks) {
    ...
    await assert.rejects(
      applyPhase477ParkerDuofoldClassicVintageDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase477ParkerDuofoldClassicVintageDepth(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE477_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE477_IDS)) {
      const pack = phase477ParkerDuofoldClassicVintageDepthPacks.find(
    ...
    const replay = await applyPhase477ParkerDuofoldClassicVintageDepth(
      client,
      options,
    );
