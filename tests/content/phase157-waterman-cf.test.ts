...
import {
  type ApplyPhase157Options,
  applyPhase157WatermanCfContent,
} from "../../scripts/apply-phase157-waterman-cf-content";
import {
  PHASE157_IDS,
  phase157WatermanCfPacks,
} from "../../scripts/data/phase157-waterman-cf";
import {
...
test("Phase 157 publishes canonical Waterman C/F on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  ...
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase157-")),
  );
  ...
  const options: ApplyPhase157Options = {
    workspaceRoot: ROOT_ALIAS,
    reviewer: "phase157-test",
  ...
  try {
    ...
    for (const pack of phase157WatermanCfPacks) {
    ...
    await assert.rejects(
      () =>
        applyPhase157WatermanCfContent(client, { ...options, reviewer: " " }),
      /reviewer must not be empty/,
    );
    await assert.rejects(
      () =>
        applyPhase157WatermanCfContent(client, {
          ...options,
          env: {
            NODE_ENV: "test",
            TURSO_DATABASE_URL: "",
    ...
    const first = await applyPhase157WatermanCfContent(client, options);
    assert.deepEqual(
    ...
    const replay = await applyPhase157WatermanCfContent(client, options);
    assert.deepEqual(
      ...
      Number(
        (
          await rows(
            ...
            [PHASE157_IDS.cf, PHASE157_IDS.brand],
          )
        )[0]?.n,
      ),
      1,
