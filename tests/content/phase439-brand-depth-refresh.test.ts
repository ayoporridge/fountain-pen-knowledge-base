...
import { applyPhase439BrandDepthRefresh } from "../../scripts/apply-phase439-brand-depth-refresh";
import {
  PHASE439_BRAND_IDS,
  phase439BrandDepthRefreshPacks,
} from "../../scripts/data/phase439-brand-depth-refresh";
import {
...
test("Phase 439 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase439-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase439-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase439BrandDepthRefreshPacks.length, 5);
    for (const pack of phase439BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase439BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase439BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE439_BRAND_IDS),
    );
    ...
    const replay = await applyPhase439BrandDepthRefresh(client, options);
