...
import { applyPhase430BrandDepthRefresh } from "../../scripts/apply-phase430-brand-depth-refresh";
import {
  PHASE430_BRAND_IDS,
  phase430BrandDepthRefreshPacks,
} from "../../scripts/data/phase430-brand-depth-refresh";
import {
...
test("Phase 430 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase430-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase430-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase430BrandDepthRefreshPacks.length, 5);
    for (const pack of phase430BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase430BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase430BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE430_BRAND_IDS),
    );
    ...
    const replay = await applyPhase430BrandDepthRefresh(client, options);
