...
import {
  type ApplyPhase515Options,
  applyPhase515WancherRadenSiblings,
} from "../../scripts/apply-phase515-wancher-raden-siblings";
import {
  PHASE515_TARGETS,
  PHASE515_WANCHER_BRAND_ID,
  phase515WancherRadenSiblingPacks,
} from "../../scripts/data/phase515-wancher-raden-siblings";
import {
...
test("Phase 515 publishes two sourced Wancher Raden sibling SKUs on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase515-wancher-")),
  );
  ...
  const options: ApplyPhase515Options = {
    workspaceRoot: ROOT,
    reviewer: "phase515-wancher-raden-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE515_TARGETS.length, 2);
    assert.equal(phase515WancherRadenSiblingPacks.length, 2);
    for (const pack of phase515WancherRadenSiblingPacks) {
      ...
      assert.match(svg, /Raden/);
      assert.match(svg, pack.entityId.includes("shuryu") ? /Shuryu/ : /Sourin/);
    }
    await assert.rejects(
      applyPhase515WancherRadenSiblings(client, {
        ...options,
        env: {
          ...options.env,
    ...
    const first = await applyPhase515WancherRadenSiblings(client, options);
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase515WancherRadenSiblingPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase515WancherRadenSiblingPacks.entries()) {
      const target = PHASE515_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(String(publicRow?.body_md ?? ""), /Raden|螺钿/);
      ...
      if (target.key === "shuryu")
        assert.match(String(publicRow?.body_md ?? ""), /序列号|Titanium/);
      const publication = (
        await client.execute({
      ...
      assert.equal(
        (
          await client.execute({
            ...
            args: [pack.entityId, PHASE515_WANCHER_BRAND_ID],
          })
        ).rows[0]?.n,
        1,
      );
      assert.equal(
        (
          await client.execute({
            ...
            args: [PHASE515_WANCHER_BRAND_ID, pack.entityId],
          })
        ).rows[0]?.n,
        1,
      );
    ...
    const replay = await applyPhase515WancherRadenSiblings(client, options);
    assert.deepEqual(
