...
test("Phase 521 publishes four sourced Wancher Japanese lacquer siblings on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase521-wancher-")),
  );
  ...
  const options: ApplyPhase521Options = {
    workspaceRoot: ROOT,
    reviewer: "phase521-wancher-japanese-lacquer-siblings-test",
  ...
  try {
    ...
    assert.equal(PHASE521_TARGETS.length, 4);
    assert.equal(phase521WancherJapaneseLacquerPacks.length, 4);
    for (const pack of phase521WancherJapaneseLacquerPacks) {
      ...
      assert.match(svg, /Kurozan|Midori|Oboro|Tsubomi/);
    }
    await assert.rejects(
      applyPhase521WancherJapaneseLacquerSiblings(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase521WancherJapaneseLacquerSiblings(
      client,
      options,
    );
    assert.deepEqual(
      first.entities.map((item) => [item.entityId, item.outcome]),
      phase521WancherJapaneseLacquerPacks.map((pack) => [
        pack.entityId,
        "published",
      ]),
    );
    for (const [index, pack] of phase521WancherJapaneseLacquerPacks.entries()) {
      const target = PHASE521_TARGETS[index];
      assert.ok(target);
      const publicRow = (
      ...
      assert.match(
        ...
        /Urushi|Bokashi|Tamamushi|Kurozan/,
      );
    ...
    const replay = await applyPhase521WancherJapaneseLacquerSiblings(
      client,
      options,
    );
    assert.deepEqual(
