...
import { applyPhase445BrandDepth } from "../../scripts/apply-phase445-postal-nakaya-morrison-brand-depth";
import {
  PHASE445_BRAND_IDS,
  phase445BrandDepthPacks,
} from "../../scripts/data/phase445-postal-nakaya-morrison-brand-depth";
import {
...
test("Phase 445 restores three historical and handmade brand entries on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase445-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase445-postal-nakaya-morrison-test",
  ...
  try {
    ...
    assert.equal(phase445BrandDepthPacks.length, 3);
    for (const pack of phase445BrandDepthPacks) {
    ...
    await assert.rejects(
      applyPhase445BrandDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase445BrandDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE445_BRAND_IDS),
    );
    ...
    const replay = await applyPhase445BrandDepth(client, options);
