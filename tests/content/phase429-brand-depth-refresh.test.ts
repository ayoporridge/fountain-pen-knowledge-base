...
import { applyPhase429BrandDepthRefresh } from "../../scripts/apply-phase429-brand-depth-refresh";
import {
  PHASE429_BRAND_IDS,
  phase429BrandDepthRefreshPacks,
} from "../../scripts/data/phase429-brand-depth-refresh";
import {
...
test("Phase 429 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase429-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase429-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase429BrandDepthRefreshPacks.length, 5);
    for (const pack of phase429BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase429BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase429BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE429_BRAND_IDS),
    );
    ...
    const replay = await applyPhase429BrandDepthRefresh(client, options);
