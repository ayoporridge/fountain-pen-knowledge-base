...
import { applyPhase471WatermanCfCharlestonDepth } from "../../scripts/apply-phase471-waterman-cf-charleston-depth";
import {
  PHASE471_IDS,
  phase471WatermanCfCharlestonDepthPacks,
} from "../../scripts/data/phase471-waterman-cf-charleston-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-dro-phase-470-deepen-existing-waterman-ideal/checkpoint.db",
);

test("Phase 471 deepens Waterman C/F and Charleston on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase471-waterman-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase471-waterman-cf-charleston-test",
  ...
  try {
    ...
    assert.equal(phase471WatermanCfCharlestonDepthPacks.length, 2);
    for (const pack of phase471WatermanCfCharlestonDepthPacks) {
    ...
    await assert.rejects(
      applyPhase471WatermanCfCharlestonDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase471WatermanCfCharlestonDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE471_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE471_IDS)) {
      const pack = phase471WatermanCfCharlestonDepthPacks.find(
    ...
    const replay = await applyPhase471WatermanCfCharlestonDepth(
      client,
      options,
    );
