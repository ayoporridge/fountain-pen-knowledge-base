...
import { applyPhase432BrandDepthRefresh } from "../../scripts/apply-phase432-brand-depth-refresh";
import {
  PHASE432_BRAND_IDS,
  phase432BrandDepthRefreshPacks,
} from "../../scripts/data/phase432-brand-depth-refresh";
import {
...
test("Phase 432 restores five rich brand entries and their reverse navigation on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const snapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase432-brand-depth-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase432-brand-depth-test",
  ...
  try {
    ...
    assert.equal(phase432BrandDepthRefreshPacks.length, 5);
    for (const pack of phase432BrandDepthRefreshPacks) {
    ...
    await assert.rejects(
      applyPhase432BrandDepthRefresh(client, {
        ...options,
    ...
    const first = await applyPhase432BrandDepthRefresh(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE432_BRAND_IDS),
    );
    ...
    const replay = await applyPhase432BrandDepthRefresh(client, options);
