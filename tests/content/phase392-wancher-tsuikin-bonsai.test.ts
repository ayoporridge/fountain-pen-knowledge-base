...
import {
  type ApplyPhase392Options,
  applyPhase392WancherBonsaiContent,
} from "../../scripts/apply-phase392-wancher-tsuikin-bonsai-content";
import {
  PHASE392_BONSAI_ID,
  PHASE392_WANCHER_BRAND_ID,
  phase392WancherBonsaiPacks,
} from "../../scripts/data/phase392-wancher-tsuikin-bonsai";
import {
...
test("Phase 392 publishes Wancher Ryukyu Tsuikin Bonsai on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const keepAlive = setInterval(() => undefined, 1_000);
  const protectedSnapshot = snapshotCatalogFiles(REAL);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase392-wancher-bonsai-")),
  );
  ...
  const options: ApplyPhase392Options = {
    workspaceRoot: ROOT,
    reviewer: "phase392-wancher-bonsai-test",
  ...
  try {
    ...
    assert.equal(phase392WancherBonsaiPacks.length, 2);
    const pack = phase392WancherBonsaiPacks.find(
      (candidate) => candidate.entityId === PHASE392_BONSAI_ID,
    );
    assert.ok(pack);
    ...
    await assert.rejects(
      applyPhase392WancherBonsaiContent(client, {
        ...options,
        env: {
          ...options.env,
          NODE_ENV: "test",
    ...
    const first = await applyPhase392WancherBonsaiContent(client, options);
    assert.deepEqual(
      first.entities.map((entity) => entity.outcome),
      ["published", "published"],
    );
    for (const candidate of phase392WancherBonsaiPacks) {
      const state = (
        await rows(
          client,
    ...
    for (const pattern of [
      ...
      /Kodachi/,
      /Jowo #6/i,
      /Ebonite feed.*Jowo #6|Jowo #6.*Ebonite feed/i,
    ...
    assert.match(String(spec?.nib), /Keiryu.*Kodachi.*Jowo #6/i);
    assert.match(String(spec?.fill_system), /cartridge.*converter/i);
    assert.match(String(spec?.material), /Ebonite.*Tamesukashi.*Tsuikin/i);
    assert.match(String(spec?.dimensions), /未公布/);
    assert.match(String(spec?.weight), /未公布/);
    assert.match(String(spec?.price_range), /900/);
    assert.match(String(spec?.status), /Original.*Tamesukashi/);
    ...
    assert.deepEqual(
      ...
      [
        `${PHASE392_BONSAI_ID}:${PHASE392_WANCHER_BRAND_ID}:made_by`,
        `${PHASE392_WANCHER_BRAND_ID}:${PHASE392_BONSAI_ID}:reverse`,
      ],
    );

    const replay = await applyPhase392WancherBonsaiContent(client, options);
    assert.deepEqual(
