...
import {
  type ApplyPhase497Options,
  applyPhase497AdditionalBrandMedia,
  PHASE497_MEDIA_TARGETS,
  PHASE497_MODEL_MEDIA_TARGETS,
} from "../../scripts/apply-phase497-additional-brand-media";
import {
...
test("Phase 497 separates five additional brand primary media rows", {
  timeout: 420_000,
}, async () => {
  const protectedSnapshot = snapshotCatalogFiles(REAL_CATALOG);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase497-media-")),
  );
  ...
  const options: ApplyPhase497Options = {
    workspaceRoot: ROOT,
    reviewer: "phase497-additional-brand-media-test",
  ...
  const oldPaths: string[] = PHASE497_MODEL_MEDIA_TARGETS.map(
    (target) => target[3],
  );
  ...
  try {
    ...
    const modelBefore = await client.execute({
      sql: `SELECT ma.entity_id,ma.id,ma.local_path,ma.source_item_id,ma.title FROM media_assets ma WHERE ma.id IN (${PHASE497_MODEL_MEDIA_TARGETS.map(() => "?").join(",")}) ORDER BY ma.id`,
      args: PHASE497_MODEL_MEDIA_TARGETS.map((target) => target[2]),
    });
    ...
    await assert.rejects(
      applyPhase497AdditionalBrandMedia(client, {
        ...options,
        env: {
          NODE_ENV: "test",
    ...
    const first = await applyPhase497AdditionalBrandMedia(client, options);
    assert.equal(first.changed, true);
    assert.deepEqual(
      first.entities.map((entity) => entity.outcome),
      new Array(PHASE497_MEDIA_TARGETS.length).fill("published"),
    );
    ...
    const second = await applyPhase497AdditionalBrandMedia(client, options);
    assert.equal(second.changed, false);
    assert.deepEqual(
      second.entities.map((entity) => entity.outcome),
      new Array(PHASE497_MEDIA_TARGETS.length).fill("noop"),
    );
