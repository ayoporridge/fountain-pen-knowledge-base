...
import { applyPhase450SheafferDepth } from "../../scripts/apply-phase450-sheaffer-balance-snorkel-pfm-depth";
import {
  PHASE450_MODEL_IDS,
  phase450SheafferBalanceSnorkelPfmPacks,
} from "../../scripts/data/phase450-sheaffer-balance-snorkel-pfm-depth";
import {
...
const BASE_CHECKPOINT = path.join(
  ROOT,
  ".planning/quick/260804-6np-pilot-capless-family-depth-owned-checkpoint/checkpoint.db",
);

test("Phase 450 deepens Sheaffer Balance, Snorkel and PFM on an owned checkpoint", {
  timeout: 900_000,
}, async () => {
  const realSnapshot = snapshotCatalogFiles(REAL);
  const baseSnapshot = snapshotCatalogFiles(BASE_CHECKPOINT);
  const ownedRoot = fs.realpathSync.native(
    fs.mkdtempSync(path.join(os.tmpdir(), "fpkg-phase450-sheaffer-")),
  );
  ...
  const options = {
    workspaceRoot: ROOT,
    reviewer: "phase450-sheaffer-test",
  ...
  try {
    ...
    assert.equal(phase450SheafferBalanceSnorkelPfmPacks.length, 3);
    for (const pack of phase450SheafferBalanceSnorkelPfmPacks) {
    ...
    await assert.rejects(
      applyPhase450SheafferDepth(client, {
        ...options,
        env: { ...options.env, TURSO_DATABASE_URL: "libsql://remote" },
      }),
      /refuses inherited remote database selection/,
    );
    const first = await applyPhase450SheafferDepth(client, options);
    assert.deepEqual(
      first.entities.map((item) => item.entityId),
      Object.values(PHASE450_MODEL_IDS),
    );
    for (const [label, modelId] of Object.entries(PHASE450_MODEL_IDS)) {
      const entity = (
      ...
      const pack = phase450SheafferBalanceSnorkelPfmPacks.find(
    ...
    const replay = await applyPhase450SheafferDepth(client, options);
