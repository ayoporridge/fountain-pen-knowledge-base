-- Phase 21: reviewed taxonomy identity substrate and publication contract v3.
-- Migration ordering is deliberate: every dependent trigger/view is removed
-- before either publication table is rebuilt, then recreated below.
...
DROP TRIGGER IF EXISTS fact_scope_variant_insert_guard;
DROP TRIGGER IF EXISTS fact_scope_variant_update_guard;
DROP TRIGGER IF EXISTS publication_fact_scope_insert;
DROP TRIGGER IF EXISTS publication_fact_scope_update_old;
DROP TRIGGER IF EXISTS publication_fact_scope_update_new;
DROP TRIGGER IF EXISTS publication_fact_scope_delete;
DROP TRIGGER IF EXISTS publication_spec_field_evidence_insert;
DROP TRIGGER IF EXISTS publication_spec_field_evidence_update_old;
DROP TRIGGER IF EXISTS publication_spec_field_evidence_update_new;
DROP TRIGGER IF EXISTS publication_spec_field_evidence_delete;
DROP TRIGGER IF EXISTS publication_claim_evidence_insert;
DROP TRIGGER IF EXISTS publication_claim_evidence_update_old;
DROP TRIGGER IF EXISTS publication_claim_evidence_update_new;
DROP TRIGGER IF EXISTS publication_claim_evidence_delete;
DROP TRIGGER IF EXISTS publication_fact_conflict_insert;
DROP TRIGGER IF EXISTS publication_fact_conflict_update_old;
DROP TRIGGER IF EXISTS publication_fact_conflict_update_new;
DROP TRIGGER IF EXISTS publication_fact_conflict_delete;
DROP TRIGGER IF EXISTS publication_fact_conflict_member_insert;
DROP TRIGGER IF EXISTS publication_fact_conflict_member_update_old;
DROP TRIGGER IF EXISTS publication_fact_conflict_member_update_new;
DROP TRIGGER IF EXISTS publication_fact_conflict_member_delete;
DROP TRIGGER IF EXISTS publication_content_review_update;
DROP TRIGGER IF EXISTS publication_content_review_delete;
DROP TRIGGER IF EXISTS publication_entity_id_immutable;
DROP TRIGGER IF EXISTS publication_story_id_immutable;
DROP TRIGGER IF EXISTS publication_model_spec_id_immutable;
DROP TRIGGER IF EXISTS publication_model_variant_id_immutable;
DROP TRIGGER IF EXISTS publication_claim_id_immutable;
DROP TRIGGER IF EXISTS publication_citation_id_immutable;
DROP TRIGGER IF EXISTS publication_source_item_id_immutable;
DROP TRIGGER IF EXISTS publication_source_registry_id_immutable;
DROP TRIGGER IF EXISTS publication_entity_reference_id_immutable;
DROP TRIGGER IF EXISTS publication_timeline_event_id_immutable;
DROP TRIGGER IF EXISTS publication_media_asset_id_immutable;
DROP TRIGGER IF EXISTS publication_entity_link_id_immutable;
DROP TRIGGER IF EXISTS publication_fact_scope_id_immutable;
DROP TRIGGER IF EXISTS publication_spec_field_evidence_id_immutable;
DROP TRIGGER IF EXISTS publication_claim_evidence_id_immutable;
DROP TRIGGER IF EXISTS publication_fact_conflict_id_immutable;
DROP TRIGGER IF EXISTS publication_fact_conflict_member_id_immutable;
DROP TRIGGER IF EXISTS publication_lifecycle_entity_id_immutable;
DROP TRIGGER IF EXISTS publication_content_revision_revoke_reviews;
DROP TRIGGER IF EXISTS publication_published_snapshot_immutable;
...
-- Preserve every legacy review row as immutable audit history. A v2 hash is
-- valid only while revoked; all live review states require contract v3.
CREATE TABLE entity_content_reviews_v3 (
  ...
  content_hash TEXT NOT NULL CHECK (
    ...
    AND (
      substr(content_hash, 1, 10) = 'sha256:v3:'
      OR (
        substr(content_hash, 1, 10) = 'sha256:v2:'
        AND status = 'revoked'
      )
    )
  ),
...
INSERT INTO entity_content_reviews_v3 (
  id, entity_id, review_kind, content_hash, status, reviewer, reviewed_at,
  note, created_at, updated_at
)
SELECT
  id, entity_id, review_kind, content_hash, 'revoked', reviewer, reviewed_at,
  note, created_at, updated_at
FROM entity_content_reviews;

DROP TABLE entity_content_reviews;
ALTER TABLE entity_content_reviews_v3 RENAME TO entity_content_reviews;
...
CREATE TABLE entity_publications_v3 (
  ...
  approved_content_hash TEXT CHECK (
    approved_content_hash IS NULL OR (
      ...
      AND substr(approved_content_hash, 1, 10) = 'sha256:v3:'
  ...
  reviewed_contract_version INTEGER CHECK (
    reviewed_contract_version IS NULL OR reviewed_contract_version = 3
  ),
  reviewed_by TEXT,
  reviewed_at TEXT,
  published_at TEXT,
...
INSERT INTO entity_publications_v3 (
  entity_id, status, depth_tier, quality_score, blockers_json,
...
SELECT
  entity_id,
  CASE WHEN status = 'published' THEN 'in_review' ELSE status END,
  depth_tier,
  quality_score,
  '["contract_v3_review_required"]',
  NULL,
  content_revision + 1,
  NULL,
  NULL,
...
ALTER TABLE entity_publications_v3 RENAME TO entity_publications;
...
-- Reviewed aliases carry their public scope and document-level provenance.
ALTER TABLE entity_aliases ADD COLUMN alias_kind TEXT NOT NULL DEFAULT 'alias'
CHECK (
  alias_kind IN (
    'alias', 'regional_name', 'former_name', 'licensed_name', 'producer_name'
  )
);
ALTER TABLE entity_aliases ADD COLUMN market TEXT
CHECK (market IS NULL OR trim(market) != '');
ALTER TABLE entity_aliases ADD COLUMN valid_from TEXT
CHECK (valid_from IS NULL OR trim(valid_from) != '');
ALTER TABLE entity_aliases ADD COLUMN valid_to TEXT
CHECK (valid_to IS NULL OR trim(valid_to) != '');
ALTER TABLE entity_aliases ADD COLUMN source_item_id TEXT
REFERENCES source_items(id) ON DELETE SET NULL;
ALTER TABLE entity_aliases ADD COLUMN review_status TEXT NOT NULL DEFAULT 'pending'
CHECK (review_status IN ('pending', 'approved', 'rejected'));

CREATE INDEX idx_entity_aliases_reviewed
  ON entity_aliases(entity_id, review_status, alias_kind);

CREATE TRIGGER entity_alias_validity_insert_guard
...
CREATE TRIGGER entity_alias_validity_update_guard
...
-- Replay-safe migration batches and row-level actions retain exact identity
-- decisions without making them part of the public payload.
CREATE TABLE taxonomy_batches (
  id TEXT PRIMARY KEY NOT NULL,
...
CREATE TABLE taxonomy_actions (
  id TEXT PRIMARY KEY NOT NULL,
  batch_id TEXT NOT NULL REFERENCES taxonomy_batches(id) ON DELETE CASCADE,
  source_row_key TEXT NOT NULL CHECK (trim(source_row_key) != ''),
  action_kind TEXT NOT NULL CHECK (
    action_kind IN (
      'create', 'merge', 'split', 'rename', 'alias', 'retire', 'variant'
    )
  ),
  action_checksum TEXT NOT NULL CHECK (
    length(action_checksum) = 64
    AND action_checksum NOT GLOB '*[^0-9a-f]*'
  ),
  ...
  status TEXT NOT NULL DEFAULT 'staged' CHECK (
    status IN ('staged', 'applied', 'skipped', 'failed')
  ),
  note TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  UNIQUE(batch_id, source_row_key),
  UNIQUE(batch_id, action_checksum)
);

CREATE INDEX idx_taxonomy_actions_batch
  ON taxonomy_actions(batch_id, action_kind, status);

CREATE TABLE entity_lineage (
  id TEXT PRIMARY KEY NOT NULL,
  batch_id TEXT NOT NULL REFERENCES taxonomy_batches(id) ON DELETE CASCADE,
  action_id TEXT REFERENCES taxonomy_actions(id) ON DELETE SET NULL,
  ...
  lineage_kind TEXT NOT NULL CHECK (
    lineage_kind IN ('rename', 'merge', 'split', 'retire', 'successor')
  ),
  ...
  CHECK (
    lineage_kind != 'split'
    OR (fallback_reason IS NOT NULL AND trim(fallback_reason) != '')
  ),
  UNIQUE(batch_id, source_entity_id, target_entity_id, lineage_kind)
);

CREATE INDEX idx_entity_lineage_source
  ON entity_lineage(source_entity_id, lineage_kind);
CREATE INDEX idx_entity_lineage_target
  ON entity_lineage(target_entity_id, lineage_kind);
...
CREATE TABLE entity_redirects (
  ...
  redirect_kind TEXT NOT NULL CHECK (
    redirect_kind IN ('permanent', 'hard_404')
  ),
  fallback_reason TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  CHECK (
    (source_path GLOB '/brand/*' OR source_path GLOB '/pen/*')
    AND substr(source_path, 2) NOT GLOB '*/*/*'
    AND source_path NOT GLOB '*[?#]*'
  ),
  CHECK (
    (redirect_kind = 'hard_404' AND target_path IS NULL)
    OR (
      redirect_kind = 'permanent'
      AND target_path IS NOT NULL
      AND (target_path GLOB '/brand/*' OR target_path GLOB '/pen/*')
      AND substr(target_path, 2) NOT GLOB '*/*/*'
...
CHECK (
  variant_kind IN (
    'variant', 'edition_group', 'color', 'material', 'nib', 'market_sku'
  )
);
ALTER TABLE model_variants ADD COLUMN parent_variant_id TEXT
REFERENCES model_variants(id) ON DELETE SET NULL;
ALTER TABLE model_variants ADD COLUMN product_code TEXT
CHECK (product_code IS NULL OR trim(product_code) != '');
ALTER TABLE model_variants ADD COLUMN market TEXT
CHECK (market IS NULL OR trim(market) != '');

CREATE INDEX idx_model_variants_parent
  ON model_variants(parent_variant_id, model_entity_id);

CREATE TRIGGER model_variant_parent_insert_guard
BEFORE INSERT ON model_variants
WHEN NEW.parent_variant_id IS NOT NULL
BEGIN
  SELECT CASE WHEN NOT EXISTS (
    SELECT 1 FROM model_variants parent
    WHERE parent.id = NEW.parent_variant_id
      AND parent.model_entity_id = NEW.model_entity_id
      AND parent.variant_kind = 'edition_group'
  ) THEN RAISE(ABORT, 'taxonomy_guard: invalid variant parent') END;
  SELECT CASE WHEN NEW.variant_kind NOT IN ('color', 'material', 'nib', 'market_sku')
    THEN RAISE(ABORT, 'taxonomy_guard: invalid variant child kind') END;
END;

CREATE TRIGGER model_variant_parent_update_guard
BEFORE UPDATE OF parent_variant_id, model_entity_id, variant_kind
ON model_variants
WHEN NEW.parent_variant_id IS NOT NULL
BEGIN
  SELECT CASE WHEN NOT EXISTS (
  ...
  WITH RECURSIVE ancestor(id) AS (
    SELECT NEW.parent_variant_id
    UNION ALL
    SELECT parent.parent_variant_id
    FROM model_variants parent
    JOIN ancestor ON parent.id = ancestor.id
    WHERE parent.parent_variant_id IS NOT NULL
  )
...
CREATE TRIGGER publication_entity_alias_update
AFTER UPDATE ON entity_aliases
WHEN OLD.review_status = 'approved' OR NEW.review_status = 'approved'
BEGIN
  UPDATE entity_publications
  SET content_revision = content_revision + 1,
      status = CASE WHEN status = 'published' THEN 'in_review' ELSE status END,
...
CREATE TRIGGER publication_entity_alias_delete
BEFORE DELETE ON entity_aliases
WHEN OLD.review_status = 'approved'
BEGIN
  UPDATE entity_publications
  SET content_revision = content_revision + 1,
      status = CASE WHEN status = 'published' THEN 'in_review' ELSE status END,
...
CREATE TRIGGER publication_entity_tag_insert
...
CREATE TRIGGER publication_entity_tag_update
AFTER UPDATE ON entity_tags
WHEN OLD.entity_id IS NOT NEW.entity_id OR OLD.tag_id IS NOT NEW.tag_id
BEGIN
  UPDATE entity_publications
  SET content_revision = content_revision + 1,
      status = CASE WHEN status = 'published' THEN 'in_review' ELSE status END,
...
CREATE TRIGGER publication_entity_tag_delete
...
CREATE TRIGGER publication_entity_alias_id_immutable
BEFORE UPDATE OF id ON entity_aliases
WHEN OLD.id IS NOT NEW.id
BEGIN
  SELECT RAISE(ABORT, 'publication_guard: entity_aliases.id is immutable');
END;

CREATE TRIGGER taxonomy_batch_id_immutable
BEFORE UPDATE OF id ON taxonomy_batches
WHEN OLD.id IS NOT NEW.id
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_guard: taxonomy_batches.id is immutable');
END;

CREATE TRIGGER taxonomy_action_id_immutable
BEFORE UPDATE OF id ON taxonomy_actions
WHEN OLD.id IS NOT NEW.id
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_guard: taxonomy_actions.id is immutable');
END;

CREATE TRIGGER entity_lineage_id_immutable
BEFORE UPDATE OF id ON entity_lineage
WHEN OLD.id IS NOT NEW.id
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_guard: entity_lineage.id is immutable');
END;

CREATE TRIGGER entity_redirect_id_immutable
BEFORE UPDATE OF id ON entity_redirects
WHEN OLD.id IS NOT NEW.id
BEGIN
  SELECT RAISE(ABORT, 'taxonomy_guard: entity_redirects.id is immutable');
END;
...
SELECT id, 3, 'missing_publication', 'entity', id, id
FROM governed_entities
WHERE publication_entity_id IS NULL
UNION ALL
SELECT id, 3, 'missing_summary', 'entity', id, id
FROM governed_entities
WHERE summary IS NULL OR trim(summary) = ''
UNION ALL
SELECT governed.id, 3, 'missing_published_story', 'entity', governed.id, governed.id
...
UNION ALL
SELECT governed.id, 3, 'multiple_published_stories', 'entity', governed.id, governed.id
FROM governed_entities governed
WHERE governed.type IN ('brand', 'pen')
  AND 1 < (
    SELECT count(*)
...
UNION ALL
SELECT story.entity_id, 3, 'deprecated_story_present', 'story', story.id, story.id
FROM stories story
JOIN governed_entities governed ON governed.id = story.entity_id
WHERE story.status = 'deprecated'
UNION ALL
SELECT id, 3, 'missing_approved_content_hash', 'publication', id, id
FROM governed_entities
WHERE publication_entity_id IS NOT NULL
  AND approved_content_hash IS NULL
UNION ALL
SELECT id, 3, 'invalid_approved_content_hash', 'publication', id, id
FROM governed_entities
WHERE approved_content_hash IS NOT NULL
  AND (
    length(approved_content_hash) != 74
    OR substr(approved_content_hash, 1, 10) != 'sha256:v3:'
    OR substr(approved_content_hash, 11) GLOB '*[^0-9a-f]*'
  )
UNION ALL
SELECT id, 3, 'stale_reviewed_revision', 'publication', id, id
FROM governed_entities
WHERE publication_entity_id IS NOT NULL
  AND (
...
UNION ALL
SELECT id, 3, 'stale_contract_version', 'publication', id, id
FROM governed_entities
WHERE publication_entity_id IS NOT NULL
  AND (
    reviewed_contract_version IS NULL
    OR reviewed_contract_version != 3
  )
UNION ALL
SELECT id, 3, 'missing_reviewer', 'publication', id, id
FROM governed_entities
WHERE publication_entity_id IS NOT NULL
  AND (reviewed_by IS NULL OR trim(reviewed_by) = '')
UNION ALL
SELECT id, 3, 'missing_reviewed_at', 'publication', id, id
FROM governed_entities
WHERE publication_entity_id IS NOT NULL
  AND (reviewed_at IS NULL OR trim(reviewed_at) = '')
UNION ALL
SELECT id, 3, 'missing_published_at', 'publication', id, id
FROM governed_entities
WHERE publication_entity_id IS NOT NULL
  AND publication_status = 'published'
  AND (published_at IS NULL OR trim(published_at) = '')
UNION ALL
SELECT owner.entity_id, 3, 'pending_claim_present', 'claim', claim.id, claim.id
FROM claims claim
JOIN publication_payload_claim_entities owner ON owner.claim_id = claim.id
JOIN governed_entities governed ON governed.id = owner.entity_id
WHERE claim.review_status IN ('pending', 'needs_source')
UNION ALL
SELECT owner.entity_id, 3, 'approved_claim_unclassified', 'claim', claim.id, claim.id
FROM claims claim
JOIN publication_payload_claim_entities owner ON owner.claim_id = claim.id
JOIN governed_entities governed ON governed.id = owner.entity_id
WHERE claim.review_status = 'approved'
...
UNION ALL
SELECT governed.id, 3, 'missing_approved_core_claim', 'claim', governed.id, governed.id
FROM governed_entities governed
WHERE governed.type IN ('brand', 'pen')
  AND NOT EXISTS (
    SELECT 1
...
UNION ALL
SELECT spec.entity_id, 3, 'spec_needs_source', 'model_spec', spec.id, spec.id
...
UNION ALL
SELECT governed.id, 3, 'missing_primary_or_archive_group', 'source_group', governed.id, governed.id
FROM governed_entities governed
WHERE governed.type IN ('brand', 'pen')
  AND coalesce((
...
UNION ALL
SELECT governed.id, 3, 'missing_professional_secondary_group', 'source_group', governed.id, governed.id
FROM governed_entities governed
WHERE governed.type IN ('brand', 'pen')
  AND coalesce((
...
UNION ALL
SELECT governed.id, 3, 'missing_approved_primary_media', 'media_asset', governed.id, governed.id
FROM governed_entities governed
WHERE governed.type IN ('brand', 'pen')
  AND NOT EXISTS (
    SELECT 1
...
UNION ALL
SELECT governed.id, 3, 'missing_fact_review', 'content_review', governed.id, 'fact'
FROM governed_entities governed
...
UNION ALL
SELECT governed.id, 3, 'missing_language_review', 'content_review', governed.id, 'language'
FROM governed_entities governed
WHERE governed.publication_entity_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM publication_v2_current_reviews review
...
UNION ALL
SELECT governed.id, 3, 'missing_media_review', 'content_review', governed.id, 'media'
FROM governed_entities governed
WHERE governed.publication_entity_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM publication_v2_current_reviews review
...
UNION ALL
SELECT governed.id, 3, 'missing_publication_review', 'content_review', governed.id, 'publication'
FROM governed_entities governed
WHERE governed.publication_entity_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM publication_v2_current_reviews review
...
UNION ALL
SELECT entity.id, 3, 'missing_made_by', 'entity_link', entity.id, entity.id
FROM entities entity
...
UNION ALL
SELECT entity.id, 3, 'multiple_made_by', 'entity_link', entity.id, entity.id
FROM entities entity
WHERE entity.type = 'pen'
  AND 1 < (
    SELECT count(*)
...
WHERE publication.status = 'published'
  AND readiness.contract_version = 3
...
WHEN OLD.model_entity_id IS NOT NEW.model_entity_id
  ...
  OR OLD.parent_variant_id IS NOT NEW.parent_variant_id
...
WHEN NEW.link_type IN (
  'made_by', 'member_of_series', 'marketed_under_licensed_brand'
)
BEGIN
  UPDATE entity_publications
  SET content_revision = content_revision + 1,
      status = CASE WHEN status = 'published' THEN 'in_review' ELSE status END,
      updated_at = datetime('now')
  WHERE entity_id IN (NEW.source_id, NEW.target_id);
END;
...
BEGIN
  ...
  SET content_revision = content_revision + 1,
      ...
      updated_at = datetime('now')
  WHERE entity_id IN (
    OLD.source_id, OLD.target_id, NEW.source_id, NEW.target_id
  );
END;

CREATE TRIGGER publication_made_by_link_delete
BEFORE DELETE ON entity_links
WHEN OLD.link_type IN (
  'made_by', 'member_of_series', 'marketed_under_licensed_brand'
)
BEGIN
  UPDATE entity_publications
  SET content_revision = content_revision + 1,
      status = CASE WHEN status = 'published' THEN 'in_review' ELSE status END,
      updated_at = datetime('now')
  WHERE entity_id IN (OLD.source_id, OLD.target_id);
END;
