...
```text
/goal 对 /Users/xz/CodeBuddy/fountain-pen-graph 执行安全与架构加固。
...
```text
/goal 在 /Users/xz/CodeBuddy/fountain-pen-graph 中补全品牌和概念实体。
...
```text
/goal 对 /Users/xz/CodeBuddy/fountain-pen-graph 统一暗色模式设计系统。
...
```text
/goal 在 /Users/xz/CodeBuddy/fountain-pen-graph 中修复 UI 排版和空状态问题。
...
```text
/goal 在 /Users/xz/CodeBuddy/fountain-pen-graph 中提升代码质量。
...
```text
/goal 对 /Users/xz/CodeBuddy/fountain-pen-graph 进行 SEO 和性能优化。
...
```text
/goal 在 /Users/xz/CodeBuddy/fountain-pen-graph 中扮演挑剔的新用户，
...
```
Goal 1（安全加固） → Goal 2（数据补全） → Goal 3（暗色模式） → Goal 4（UI打磨） → Goal 5（代码质量） → Goal 6（SEO性能） → Goal 7（综合审计）
```

理由：先堵安全漏洞（P0），再补数据（内容是核心价值），然后统一视觉（用户直接感知），再重构代码（可持续性），最后补 SEO 和做兜底审计。
